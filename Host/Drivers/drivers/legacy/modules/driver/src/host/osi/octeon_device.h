...
struct cavium_wk {
	...
	struct delayed_work work_2;
	struct delayed_work work_mvmgmt;
	//struct delayed_work first_insmod_task;
	void *ctxptr;
	u64 ctxul;
};  
struct cavium_wq {
	struct workqueue_struct *wq;
	struct workqueue_struct *wq_2;
	struct workqueue_struc *wq_mvmgmt;
	struct cavium_wk wk;
...
#define IRQ_NAME_OFF(i) ((i) * INTRNAMSIZ)
   ...
   /** Application Context */
    ...
    bool facility_conf_init_error;
    bool facility_conf_init_done;
	...
	int is_reset;
	int is_set;
	volatile int is_down;
	int had_set ;
	struct delayed_work first_insmod_task;
	//struct workqueue_struct *first_insmod_wq;
