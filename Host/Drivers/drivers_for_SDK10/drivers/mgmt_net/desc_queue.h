...
#define OTXCN_DESC_PTR_DIRECT   0
#define OTXCN_DESC_PTR_INDIRECT 1

/* hw desc ptr */
struct otxcn_hw_desc_ptr {
	union {
		uint64_t u64;
		struct {
...
struct otxcn_hw_descq {
	...
	struct otxcn_hw_desc_ptr desc_arr[];
} __packed;

#define OTXCN_DESC_ARR_ENTRY_OFFSET(i) \
	(sizeof(struct otxcn_hw_descq) + (i * sizeof(struct otxcn_hw_desc_ptr)))
