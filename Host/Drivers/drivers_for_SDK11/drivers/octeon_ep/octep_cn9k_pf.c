...
static void octep_init_config_cn93_pf(struct octep_device *oct)
{
    ...
    //zcf = (void __iomem *)oct->mmio[2].hw_addr + 1;
    //dev_info(&pdev->dev, "hw_addr = %p, zcf=%p, size = %u\n", (void __iomem *)oct->mmio[2].hw_addr, zcf, sizeof(void __iomem *));
    //dev_info(&pdev->dev, "CN93_PEM_BAR4_INDEX_OFFSET=0x%llx, link = %u, CTRL_MBOX_SZ = 0x%lx, result = 0x%lx\n", 
    //    CN93_PEM_BAR4_INDEX_OFFSET, link, CTRL_MBOX_SZ, link * CTRL_MBOX_SZ);

    dev_info(&pdev->dev, "hw_addr = %p, barmem_addr=%p\n", 
        (void __iomem *)oct->mmio[2].hw_addr, conf->ctrl_mbox_cfg.barmem_addr);
