...
static void cn93_vf_dump_q_regs(struct octep_vf_device *oct, int qno)
{
	...
	dev_info(dev, "R[%d]_MBOX_PF_VF_INT[0x%llx]: 0x%016llx\n",
		 qno, CN93_VF_SDP_R_MBOX_PF_VF_INT(qno),
		 octep_vf_read_csr64(oct, CN93_VF_SDP_R_MBOX_PF_VF_INT(qno)));
}
...
static int cn93_vf_reset_iq(struct octep_vf_device *oct, int q_no)
{
	...
	dev_dbg(&oct->pdev->dev, "cn93_vf_reset_iq: Reset VF IQ-%d\n", q_no);
...
static void cn93_vf_reset_oq(struct octep_vf_device *oct, int q_no)
{
	...
	dev_dbg(&oct->pdev->dev, "cn93_vf_reset_oq: Reset VF IQ-%d\n", q_no);
	/* Disable Output (Rx) Ring */
	octep_vf_write_csr64(oct, CN93_VF_SDP_R_OUT_ENABLE(q_no), val);

	/* Clear count CSRs */
	val = octep_vf_read_csr(oct, CN93_VF_SDP_R_OUT_CNTS(q_no));
	octep_vf_write_csr64(oct, CN93_VF_SDP_R_OUT_CNTS(q_no), val & 0xFFFFFFFF);
...
static void octep_vf_reset_io_queues_cn93(struct octep_vf_device *oct)
{
	...
	dev_dbg(&pdev->dev, "Reset OCTEP_CN93 VF IO Queues, count = %u\n", CFG_GET_PORTS_ACTIVE_IO_RINGS(oct->conf));
