				      ...
				      enum rte_crypto_op_sess_type sess_type, void *mdata)
{
	...
	if (qp->ca.vector_sz) {
		...
		agq = roc_sso_hwgrp_agq_from_tag(&qp->evdev->sso, ec_mdata->response_info.queue_id,
						 tag_mask, 0);
		if (agq < 0) {
			plt_err("Invalid aggregator queue id");
			roc_sso_hwgrp_agq_release(&qp->evdev->sso,
						  ec_mdata->response_info.queue_id);
			return agq;
		}
