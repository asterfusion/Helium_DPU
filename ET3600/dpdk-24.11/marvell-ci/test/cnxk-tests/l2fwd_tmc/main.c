...
static void
l2fwd_event_parse_mode(const char *optarg, struct l2fwd_resources *rsrc)
{
...
static void
l2fwd_event_parse_eventq_sched(const char *optarg, struct l2fwd_resources *rsrc)
{
...
print_stats(struct l2fwd_resources *rsrc)
{
	...
	for (port_id = 0; port_id < RTE_MAX_ETHPORTS; port_id++) {
		...
		printf("\nStatistics for port %u ------------------------------"
		       ...
		       port_id, rsrc->port_stats[port_id].tx,
...
signal_handler(int signum)
{
	...
	if (signum == SIGINT || signum == SIGTERM) {
		printf("\n\nSignal %d received, preparing to exit...\n",
		       signum);
		__atomic_store_n(&rsrc->force_quit, 1, __ATOMIC_RELEASE);
	}
}
...
main(int argc, char **argv)
{
	...
	if (tmc_isol_init() != 0)
		rte_panic("Failed to enable task isolation\n");
	...
	nb_mbufs =
		RTE_MAX(nb_ports * (RTE_TEST_RX_DESC_DEFAULT +
				    RTE_TEST_TX_DESC_DEFAULT + MAX_PKT_BURST +
				    rte_lcore_count() * MEMPOOL_CACHE_SIZE),
			8192U);

	/* Create the mbuf pool. 8< */
	rsrc->pktmbuf_pool = rte_pktmbuf_pool_create(
