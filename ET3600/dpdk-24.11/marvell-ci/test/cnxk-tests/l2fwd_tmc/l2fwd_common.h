...
static inline void
l2fwd_init_thread_tmc(void)
{
	/* Setup thread-level task isolation mode:*/
	if (tmc_isol_thr_init() < 0)
		printf("Thread %d failed to setup the isolation mode.\n",
		       rte_lcore_id());

	if (tmc_isol_thr_enter() < 0)
		printf("Thread %d failed to enter the isolation mode.\n",
		       rte_lcore_id());
}
