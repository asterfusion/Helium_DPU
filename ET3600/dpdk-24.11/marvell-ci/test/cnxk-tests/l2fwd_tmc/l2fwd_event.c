...
l2fwd_event_service_enable(uint32_t service_id)
{
	...
	while (slcore_count--) {
		...
		if (rte_service_map_lcore_set(
			    service_id, slcore_array[slcore_count], 0) != 0)
			return -ENOENT;
...
l2fwd_event_service_setup(struct l2fwd_resources *rsrc)
{
	...
	for (i = 0; i < evt_rsrc->tx_adptr.nb_tx_adptr; i++) {
		ret = rte_event_eth_tx_adapter_caps_get(
			evt_rsrc->event_d_id, evt_rsrc->tx_adptr.tx_adptr[i],
			&caps);
		if (ret < 0)
			rte_panic("Failed to get Rx adapter[%d] caps\n",
		...
		const uint32_t flags)
{
	...
	if (timer_period > 0)
		__atomic_fetch_add(&rsrc->port_stats[mbuf->port].rx, 1,
				   __ATOMIC_RELAXED);
	mbuf->port = dst_port;
	...
	if (timer_period > 0)
		__atomic_fetch_add(&rsrc->port_stats[mbuf->port].tx, 1,
				   __ATOMIC_RELAXED);
}

static __rte_always_inline void
l2fwd_event_loop_single(struct l2fwd_resources *rsrc, const uint32_t flags)
{
	struct l2fwd_event_resources *evt_rsrc = rsrc->evt_rsrc;
...
static __rte_always_inline void
l2fwd_event_loop_burst(struct l2fwd_resources *rsrc, const uint32_t flags)
{
	struct l2fwd_event_resources *evt_rsrc = rsrc->evt_rsrc;
	const int port_id = l2fwd_get_free_event_port(evt_rsrc);
	const uint8_t tx_q_id =
...
static __rte_always_inline void
l2fwd_event_loop(struct l2fwd_resources *rsrc, const uint32_t flags)
{
...
{
	l2fwd_event_loop(rsrc, L2FWD_EVENT_TX_DIRECT | L2FWD_EVENT_SINGLE);
}
...
{
	l2fwd_event_loop(rsrc, L2FWD_EVENT_UPDT_MAC | L2FWD_EVENT_TX_DIRECT |
				       L2FWD_EVENT_SINGLE);
}
...
{
	l2fwd_event_loop(rsrc, L2FWD_EVENT_UPDT_MAC | L2FWD_EVENT_TX_ENQ |
				       L2FWD_EVENT_SINGLE);
}
		       ...
		       const uint64_t timer_period, const uint32_t flags)
{
	...
	if (vec->attr_valid) {
		...
		for (i = 0, j = 1; i < vec->nb_elem; i++, j++) {
			...
			if (flags & L2FWD_EVENT_UPDT_MAC)
				l2fwd_mac_updating(mbufs[i], vec->port,
