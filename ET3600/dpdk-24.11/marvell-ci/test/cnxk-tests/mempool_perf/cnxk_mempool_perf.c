...
/*
 * CNXK Mempool performance
 * ========================
 *
 * This is a lean version of the default mempool_perf test in DPDK.
 * The default mempool perf test runs multiple combinations of tests
 * all of which is not required for cnxk.
 * This test trims down the combinations of tests to the minimum required
 * runs.
 */
...
{
	unsigned int bulk_tab_get[] = { CACHE_LINE_BURST, 32, 0 };
	unsigned int bulk_tab_put[] = { CACHE_LINE_BURST, 32, 0 };
...
REGISTER_TEST_COMMAND(cnxk_mempool_perf, test_mempool_perf);
