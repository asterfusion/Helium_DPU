...
#ifndef __included_bihash_72_8_h__
#define __included_bihash_72_8_h__
...
typedef struct
{
  u64 key[9];
  u64 value;
} clib_bihash_kv_72_8_t;

static inline void
clib_bihash_mark_free_72_8 (clib_bihash_kv_72_8_t *v)
{
  v->value = 0xFEEDFACE8BADF00DULL;
}

static inline int
clib_bihash_is_free_72_8 (const clib_bihash_kv_72_8_t * v)
{
...
static inline u64
clib_bihash_hash_72_8 (const clib_bihash_kv_72_8_t * v)
{
#ifdef clib_crc32c_uses_intrinsics
  return clib_crc32c ((u8 *) v->key, 64);
#else
  u64 tmp = v->key[0] ^ v->key[1] ^ v->key[2] ^ v->key[3] ^ v->key[4]
    ^ v->key[5] ^ v->key[6] ^ v->key[7] ^ v->key[8];
  return clib_xxhash (tmp);
#endif
}

static inline u8 *
format_bihash_kvp_72_8 (u8 * s, va_list * args)
{
  clib_bihash_kv_72_8_t *v = va_arg (*args, clib_bihash_kv_72_8_t *);

  s = format (s, "key %llu %llu %llu %llu %llu %llu %llu %llu %llu value %llu", v->key[0],
...
#endif /* __included_bihash_72_8_h__ */
