/*
 * geosite_periodic.c - skeleton plug-in periodic function
 *
...
static uword
geosite_periodic_process (vlib_main_t * vm,
	                  vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  geosite_main_t *pm = &geosite_main;
  f64 now;
  f64 timeout = 10.0;
  uword *event_data = 0;
  uword event_type;
...
void geosite_create_periodic_process (geosite_main_t *gmp)
{
  ...
  gmp->periodic_node_index = vlib_process_create (gmp->vlib_main,
    "geosite-periodic-process",
    geosite_periodic_process, 16 /* log2_n_stack_bytes */);
}
