		  ...
		  vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
	...
	{
          u32 next0 = GEOSITE_NEXT_INTERFACE_OUTPUT;
          u32 next1 = GEOSITE_NEXT_INTERFACE_OUTPUT;
...
#undef _
  ...
  vlib_node_increment_counter (vm, geosite_node.index, 
                               GEOSITE_ERROR_SWAPPED, pkts_swapped);
...
VLIB_REGISTER_NODE (geosite_node) = 
{
  ...
  .next_nodes = {
        [GEOSITE_NEXT_INTERFACE_OUTPUT] = "interface-output",
  },
};
#endif /* CLIB_MARCH_VARIANT */
/* *INDENT-ON* */
