...
static u32
unformat_onp_pktio_rq_poll_algo (unformat_input_t *sub_input, va_list *args)
{
  cnxk_pktio_rq_poll_algo_t *algo;

  algo = va_arg (*args, cnxk_pktio_rq_poll_algo_t *);

  /* clang-format off */
  if (0)
    ;
#define _(val, alg, str, desc)                                                \
  else if (unformat (sub_input, #str))                                        \
    *algo = CNXK_PKTIO_RQ_POLL_ALGO_##alg;
  ...
  if (sub_input)
    {
      ...
      uuid_clear (pktioconf->uuid_token);
      ...
      while (unformat_check_input (sub_input) != UNFORMAT_END_OF_INPUT)
	{
	  ...
	  else if (unformat (sub_input, "vf-token %s", &uuid_string))
	    {
	      error =
		onp_uuid_parse ((char *) uuid_string, pktioconf->uuid_token);
	      if (error)
		return clib_error_return (error, "onp_uuid_parse failed");
	    }
	  else if (unformat (sub_input, "rxq-poll-algo %U",
			     unformat_onp_pktio_rq_poll_algo,
...
clib_error_t *
onp_pktio_inl_inb_ipsec_flow_enable (vlib_main_t *vm)
{
  vnet_main_t *vnm = vnet_get_main ();
  onp_main_t *om = onp_get_main ();
  vnet_flow_t flow = { 0 };
  ...
  if (pm->is_inl_ipsec_flow_enabled)
    return NULL;

  vec_foreach (pktio, om->onp_pktios)
    {
      if (cnxk_drv_pktio_is_inl_dev (vm, pktio->cnxk_pktio_index))
	continue;
      ...
      vnet_flow_enable (vnm, flow_index, pktio->hw_if_index);
    }

  pm->is_inl_ipsec_flow_enabled = 1;
  return NULL;
}
...
#undef _
  ...
  drv_pktio_index = cnxk_drv_pktio_init (vm, &pconf->pktio_pci_addr,
					 pconf->uuid_token, &pci_handle);
  ...
  if (!cnxk_drv_pktio_is_inl_dev (vm, drv_pktio_index))
    {
	...
	{
	  /*
	   * Clear IPsec send queue configuration since pktio does not
	   * support inline outbound IPsec functionality
	   */
	  eth_config.n_tx_ipsec_queues = 0;
	  pconf->n_tx_ipsec_q = 0;
	}
...
#undef _
  ...
  if (om->onp_conf->onp_schedconf.is_sched_config_enabled)
    tx_conf.txq_offloads |= CNXK_PKTIO_TX_OFF_FLAG_SCHED_EN;
  ...
  rx_conf.rxq_poll_algo = pktio->rxq_poll_algo;
  rx_conf.rxq_max_poll_retries = pktio->n_rxq_poll_retries;
  rx_conf.rxq_min_vec_size = pktio->rxq_min_vec_size;
  rx_conf.rxq_max_vec_size = pktio->rxq_max_vec_size;

  onp_pktio_notice ("eth%d rq (algo: %u, retry: %u, min: %u, max: %u)",
		    pktio->onp_pktio_index, rx_conf.rxq_poll_algo,
		    rx_conf.rxq_max_poll_retries, rx_conf.rxq_min_vec_size,
		    rx_conf.rxq_max_vec_size);
  ...
  vec_validate_init_empty_aligned (pktio->onp_pktio_txqs, pktio->n_tx_q - 1,
			     ...
			     int is_assign_node)
{
  ...
  pktio->onp_pktio_rxqs[rq_index].vnet_hw_rxq_index =
...
onp_pktio_txqs_fp_set (vlib_main_t *vm, u32 onp_pktio_index, int is_enable)
{
  ...
  if (is_enable)
      ...
      vec_foreach_index (txq, pktio->onp_pktio_txqs)
	{
	  ...
	  if (!pktio->txq_init_done)
	    pktio->onp_pktio_txqs[txq].vnet_hw_txq_index =
	      vnet_hw_if_register_tx_queue (vnm, pktio->hw_if_index, txq);
	}
