			...
			u8 is_trace_enable)
{
  ...
  cnxk_drv_pkt_process_and_enq_to_next_march (vm, node, ptd, n_rx_packets,
					      next_index, &is_single_next);
  ...
  if (PREDICT_FALSE (is_trace_enable &&
      ...
      while (n_trace && (n_left > 0))
	{
	  bufs = ptd->buffers;
#if CLIB_DEBUG > 0
	  if (vlib_trace_buffer (
		vm, node, ptd->next1[count] ? ptd->next1[count] : next_index,
		bufs[count], 1))
#else
	  if (vlib_trace_buffer (
		vm, node, ptd->next1[count] ? ptd->next1[count] : next_index,
		...
		vlib_add_trace (vm, node, bufs[count], sizeof t0[0]);
	      ...
	      t0->next_node_index =
		ptd->next1[count] ? ptd->next1[count] : next_index,
	      t0->queue_index = qid;
