...
/*
 * Symmetric RSS key for Toeplitz hash.
 * This key ensures that hash(src_ip, dst_ip, src_port, dst_port) ==
 * hash(dst_ip, src_ip, dst_port, src_port), so bidirectional traffic
 * of the same flow is steered to the same RX queue.
 * Based on the paper "Scalable TCP Session Monitoring with Symmetric
 * Receive-side Scaling", extended to 48 bytes for Marvell CNXK.
 */
const u8 cnxk_pktio_symmetric_rss_key[CNXK_PKTIO_RSS_KEY_LEN] = {
  ...
  if (hdr)
    {
      ...
      if (hdr->match_id)
	s = format (s, "\n%UPKT_RX_FDIR 0x%x", format_white_space, indent + 2,
		    hdr->match_id);
    }
  return s;
}
...
cnxk_pktio_nix_dev_init (cnxk_pktio_t *pktio, cnxk_plt_pci_device_t *dev)
{
  ...
  if (roc_model_is_cn10k ())
    ...
    clib_memcpy (&pktio_ops->fops, &eth_9k_ops, sizeof (cnxk_pktio_ops_t));
  else
    ASSERT (0);
...
i32
cnxk_pktio_init (vlib_main_t *vm, vlib_pci_addr_t *addr, uuid_t uid_token,
		 vlib_pci_dev_handle_t *pci_handle)
{
  cnxk_plt_pci_device_t *dev;
  cnxk_pktio_t *pktio;

  dev = cnxk_pci_dev_probe (vm, addr, uid_token, pci_handle);
  if (!dev)
    {
  ...
  switch (dev->id.device_id)
    {
    case PCI_DEVID_CNXK_RVU_NIX_INL_PF:
    case PCI_DEVID_CNXK_RVU_NIX_INL_VF:
      pktio = cnxk_pktio_alloc ();
      return cn10k_pktio_inl_dev_init (pktio, dev);
...
cnxk_pktio_start (vlib_main_t *vm, cnxk_pktio_t *dev)
{
  ...
  if (pm->inl_dev.is_enabled)
    {
      rv = roc_nix_inl_rq_ena_dis (nix, true);
      if (rv)
	{
	  cnxk_pktio_err ("roc_nix_inl_rq_ena_dis failed with '%s' error",
			  roc_error_msg_get (rv));
	  return -1;
	}
    }
...
cnxk_pktio_stop (vlib_main_t *vm, cnxk_pktio_t *dev)
{
  ...
  if (pm->inl_dev.is_enabled)
    {
      rv = roc_nix_inl_rq_ena_dis (nix, false);
      if (rv)
	{
	  cnxk_pktio_err ("roc_nix_inl_rq_ena_dis failed with '%s' error",
			  roc_error_msg_get (rv));
...
i32
cnxk_drv_pktio_is_inl_dev_enabled (vlib_main_t *vm)
{
  cnxk_pktio_main_t *pm;

  pm = cnxk_pktio_get_main ();
  ASSERT (pm->inl_dev.is_enabled <= 1);

  return pm->inl_dev.is_enabled;
}
...
i32
cnxk_drv_pktio_init (vlib_main_t *vm, vlib_pci_addr_t *addr, uuid_t uuid_token,
		     vlib_pci_dev_handle_t *phandle)
{
  ASSERT (vm->thread_index == 0);

  return cnxk_pktio_init (vm, addr, uuid_token, phandle);
}
...
i32
cnxk_drv_pktio_flow_inl_dev_update (vnet_main_t *vnm, vnet_flow_dev_op_t op,
				    u32 dev_instance, vnet_flow_t *flow,
				    uword *private_data)
{
  cnxk_pktio_ops_map_t *ops_map = cnxk_pktio_get_pktio_ops (dev_instance);

  return (ops_map->fops.pktio_flow_inl_dev_update (vnm, op, &ops_map->pktio,
						   flow, private_data));
}
...
u32
cnxk_drv_pktio_flow_query (vlib_main_t *vm, u32 pktio_index, uword flow_index,
...
u8
cnxk_drv_pktio_is_inl_dev (vlib_main_t *vm, u16 pktio_index)
{
  cnxk_pktio_ops_map_t *ops_map = cnxk_pktio_get_pktio_ops (pktio_index);

  return ops_map->fops.pktio_is_inl_dev (vm, &ops_map->pktio);
}
