...
#include <onp/drv/modules/pktio/pktio_fp_tx_cn9k.h>
...
#define _(trace, o_cksum, name)                                               \
                                                                              \
i32                                                                           \
CLIB_MULTIARCH_FN(cn9k_recv_pkts_##name) (vlib_main_t *vm,                    \
...
CLIB_MARCH_FN_REGISTRATION(cn9k_recv_pkts_##name);                            \
                                                                              \
i32                                                                           \
CLIB_MULTIARCH_FN(cn9k_recv_mseg_pkts_##name) (vlib_main_t *vm,               \
              vlib_node_runtime_t *node, u32 rxq, u16 rx_pkts,                \
              cnxk_per_thread_data_t *ptd)                                    \
{                                                                             \
  u64 rx_off_flags = o_cksum ? CNXK_PKTIO_RX_OFF_FLAG_OUTER_CKSUM : 0;        \
...
CLIB_MARCH_FN_REGISTRATION(cn9k_recv_mseg_pkts_##name);
...
#define _(inl_ipsec, fat_flow, o_cksum, desc_sz, name)                        \
                                                                              \
i32                                                                           \
CLIB_MULTIARCH_FN(cn9k_send_pkts_##name) (vlib_main_t *vm,                    \
                                    vlib_node_runtime_t *node, u32 txq,       \
                                    u16 tx_pkts, cnxk_per_thread_data_t *ptd) \
{                                                                             \
  u64 tx_off_flags = o_cksum ? CNXK_PKTIO_TX_OFF_FLAG_OUTER_CKSUM : 0;        \
                                                                              \
  if(inl_ipsec) return 0;                                                     \
                                                                              \
  return cn9k_pkts_send (vm, node, txq, tx_pkts, ptd, desc_sz,                \
                         0, tx_off_flags);                                    \
}                                                                             \
CLIB_MARCH_FN_REGISTRATION(cn9k_send_pkts_##name);                            \
                                                                              \
i32                                                                           \
CLIB_MULTIARCH_FN(cn9k_send_mseg_pkts_##name) (vlib_main_t *vm,               \
                                    vlib_node_runtime_t *node, u32 txq,       \
                                    u16 tx_pkts, cnxk_per_thread_data_t *ptd) \
{                                                                             \
  u64 tx_off_flags = o_cksum ? CNXK_PKTIO_TX_OFF_FLAG_OUTER_CKSUM : 0;        \
  ...
  if(inl_ipsec) return 0;                                                     \
                                                                              \
  return cn9k_pkts_send (vm, node, txq, tx_pkts, ptd, desc_sz + 8,            \
                         0,                                                   \
                         tx_off_flags | CNXK_PKTIO_TX_OFF_FLAG_MSEG);         \
}                                                                             \
CLIB_MARCH_FN_REGISTRATION(cn9k_send_mseg_pkts_##name);
...
i32
cn9k_pktio_pkts_recv (vlib_main_t *vm, vlib_node_runtime_t *node, u32 rxq,
...
i32
cn9k_pktio_pkts_send (vlib_main_t *vm, vlib_node_runtime_t *node, u32 txq,
		      u16 tx_pkts, cnxk_per_thread_data_t *ptd,
		      const u64 fp_flags, const u64 off_flags)
{
  ASSERT (tx_pkts);

  return cn9k_pkts_send (vm, node, txq, tx_pkts, ptd, 16, fp_flags, off_flags);
}

static i32
cn9k_pktio_config (vlib_main_t *vm, cnxk_pktio_t *dev,
		   cnxk_pktio_config_t *config)
{
  ...
  u16 reta[ROC_NIX_RSS_RETA_MAX];
  bool tm_xmit_enable;
  int i, rv;

  rx_max_len = roc_nix_max_pkt_len (nix);
   ...
   * Update the table to include only plain rx queues
   */

  rv = roc_nix_rss_reta_get (nix, ROC_NIX_RSS_GROUP_DEFAULT, reta);
  if (rv)
    {
  ...
  for (i = 0; i < nix->reta_sz; i++)
    reta[i] = i % config->n_rx_queues;

  rv = roc_nix_rss_reta_set (nix, ROC_NIX_RSS_GROUP_DEFAULT, reta);
  if (rv)
    {
      cnxk_pktio_err ("roc_nix_rss_reta_set failed with '%s' error",
		      roc_error_msg_get (rv));
      return -1;
    }
...
static i32
cn9k_pktio_fprq_setup (vlib_main_t *vm, cnxk_pktio_t *dev, u32 qid,
		       ...
		       cnxk_pktio_rq_type_t rxq_type)
{
  ...
  if (rxq_type == CNXK_PKTIO_RQ_TYPE_RSS)
    {
      fprq->rxq_max_poll_retries = conf->rxq_max_poll_retries;
      ...
      fprq->poll_algo = conf->rxq_poll_algo;
    }

  fprq->retry_count = 0;
  fprq->cached_pkts = 0;
  fprq->last_time_since_dequeued = 0.0;
  /* Number of worker core + 1 */
  fprq->work_enqueue_count = 1 + vlib_num_workers ();

  return 0;
}

static i32
cn9k_pktio_rxq_setup (vlib_main_t *vm, cnxk_pktio_t *dev,
		      cnxk_pktio_rxq_conf_t *conf)
{
  u32 n_queues, first_skip, i, n_desc;
  struct roc_nix *nix = &dev->nix;
  ...
  for (i = 0; i < n_queues; i++)
      ...
      if (rv)
	{
      ...
      first_skip = sizeof (vlib_buffer_t) + sizeof (cnxk_pktio_meta_t);
      ...
      if (rv)
	{
	  cnxk_pktio_err ("roc_nix_rq_init failed (rxq=%d) with '%s' error", i,
			  roc_error_msg_get (rv));
	  return -1;
	}
      if (cn9k_pktio_fprq_setup (vm, dev, i, rq, cq, conf, rq_type) < 0)
	return -1;
    }
  return 0;
}
...
static i32
cn9k_pktio_fpsq_setup (cnxk_pktio_t *dev, u32 qid, struct roc_nix_sq *sq,
...
static i32
cn9k_pktio_txq_setup (vlib_main_t *vm, cnxk_pktio_t *dev,
		      cnxk_pktio_txq_conf_t *conf)
{
  ...
  for (i = 0; i < dev->n_tx_queues; i++)
      ...
      if (rv)
	{
      ...
      /* TODO: pass CQ */
      if (cn9k_pktio_fpsq_setup (dev, i, sq, NULL /* cq */, conf) < 0)
	return -1;
    }
  return 0;
}
...
static i32
cn9k_pktio_rxq_fp_set (vlib_main_t *vm, cnxk_pktio_t *dev, u32 rxq_id,
		       cnxk_pktio_rxq_fn_conf_t *rxq_fn_conf)
{
  ...
  /* clang-format off */
#define _(trace, o_cksum, name)                                               \
  rx_func[trace][o_cksum] = CLIB_MARCH_FN_POINTER(cn9k_recv_pkts_##name);

   foreach_pktio_rx_func
#undef _

#define _(trace, o_cksum, name)                                               \
  mseg_rx_func[trace][o_cksum] = CLIB_MARCH_FN_POINTER(cn9k_recv_mseg_pkts_##name);
...
static i32
cn9k_pktio_txq_fp_set (vlib_main_t *vm, cnxk_pktio_t *dev, u32 txq_id,
		       cnxk_pktio_txq_fn_conf_t *txq_fn_conf)
{
  cnxk_drv_pktio_txq_send_func_t mseg_tx_func[2][2][2];
  cnxk_drv_pktio_txq_send_func_t tx_func[2][2][2];
  u64 flags = txq_fn_conf->offload_flags;

  /* clang-format off */
#define _(inl_ipsec, sched_inp, o_cksum, desc_sz, name)                       \
  tx_func[inl_ipsec][sched_inp][o_cksum] = CLIB_MARCH_FN_POINTER(             \
                                          cn9k_send_pkts_##name);            \

    foreach_pktio_tx_func
#undef _

#define _(inl_ipsec, sched_inp, o_cksum, desc_sz, name)                        \
  mseg_tx_func[inl_ipsec][sched_inp][o_cksum] = CLIB_MARCH_FN_POINTER(         \
                                               cn9k_send_mseg_pkts_##name);   \
...
#undef _
  ...
  if (flags & CNXK_PKTIO_TX_OFF_FLAG_MSEG)
    txq_fn_conf->pktio_send_func_ptr =  mseg_tx_func
                                [!!(flags & CNXK_PKTIO_TX_OFF_FLAG_INLINE_IPSEC)]
				[!!(flags & CNXK_PKTIO_TX_OFF_FLAG_SCHED_EN)]
...
static i32
cn9k_pktio_capa_get (vlib_main_t *vm, cnxk_pktio_t *dev,
		     cnxk_pktio_capa_t *capa)
{
  ...
  capa->is_pktio_inl_outbound = 0;
  capa->is_pktio_inl_inbound = 0;
...
u8
cn9k_pktio_is_inl_dev (vlib_main_t *vm, cnxk_pktio_t *dev)
{
  return 0;
}

cnxk_pktio_ops_t eth_9k_ops = {
  ...
  .pktio_rxq_fp_set = cn9k_pktio_rxq_fp_set,
  .pktio_txq_fp_set = cn9k_pktio_txq_fp_set,
  .pktio_flow_query = cnxk_pktio_flow_query,
  .pktio_xstats_get = cnxk_pktio_xstats_get,
  .pktio_is_inl_dev = cn9k_pktio_is_inl_dev,
  .pktio_stats_get = cnxk_pktio_stats_get,
  .pktio_pkts_recv = cn9k_pktio_pkts_recv,
  .pktio_pkts_send = cn9k_pktio_pkts_send,
  .pktio_flow_dump = cnxk_pktio_flow_dump,
  .pktio_rxq_setup = cn9k_pktio_rxq_setup,
  .pktio_txq_setup = cn9k_pktio_txq_setup,
  .pktio_capa_get = cn9k_pktio_capa_get,
  .pktio_mtu_set = cnxk_pktio_mtu_set,
  .pktio_mtu_get = cnxk_pktio_mtu_get,
  .pktio_config = cn9k_pktio_config,
