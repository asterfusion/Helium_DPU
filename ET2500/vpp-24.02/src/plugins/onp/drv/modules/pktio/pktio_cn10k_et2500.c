...
{                                                                             \
  ...
  u64 fp_flags = 0;                                                           \
                                                                              \
  if (sched_inp)                                                              \
     tx_off_flags |= CNXK_PKTIO_TX_OFF_FLAG_SCHED_EN;                         \
									      \
  if (inl_ipsec)                                                              \
    return cn10k_pkts_send_inline_ipsec(vm, node, txq, tx_pkts, ptd,          \
                                 desc_sz, fp_flags, tx_off_flags);            \
  else                                                                        \
		    ...
		    cnxk_pktio_config_t *config)
{
  ...
  tx_queues = config->n_tx_queues + config->n_tx_ipsec_queues;
  ...
  dev->n_tx_ipsec_queues = config->n_tx_ipsec_queues;
  ...
  rv = roc_nix_tm_hierarchy_enable (nix, ROC_NIX_TM_USER, tm_xmit_enable);
  if (rv)
    {
			...
			struct roc_nix_cq *cq, cnxk_pktio_txq_conf_t *txq_conf)
{
  ...
  if (qid < dev->n_tx_queues)
    ...
    {
      pool_get_zero (dev->ipsec_fpsqs, fpsq);
      /* Use IPsec queue offset for allocation */
      if (fpsq != vec_elt_at_index (dev->ipsec_fpsqs, qid - dev->n_tx_queues))
	{
	  cnxk_pktio_err ("Given IPsec txq(%d) is not configured yet",
			  qid - dev->n_tx_queues);
	  return -1;
	}
    }
		       ...
		       cnxk_pktio_txq_conf_t *conf)
{
  ...
  n_tx_queues = dev->n_tx_queues + dev->n_tx_ipsec_queues;
		      ...
		      cnxk_pktio_capa_t *capa)
{
  ...
  if (pm->inl_dev.is_enabled)
    capa->is_pktio_inl_inbound = 1;

  capa->is_pktio_inl_outbound = 1;
...
u8
cn10k_pktio_is_inl_dev (vlib_main_t *vm, cnxk_pktio_t *dev)
{
  return dev->is_inline;
}

i32
cn10k_pktio_flow_inl_dev_update (vnet_main_t *vnm, vnet_flow_dev_op_t op,
				 ...
				 uword *private_data)
{
  ...
  if (!pm->inl_dev.is_enabled)
    {
      cnxk_pktio_err ("Inline device is not initialized");
      return CNXK_UNSUPPORTED_OPERATION;
    }

  return cnxk_pktio_flow_inl_dev_update (vnm, op, dev, flow, private_data);
}

cnxk_pktio_ops_t eth_10k_ops = {
  .pktio_flow_inl_dev_update = cn10k_pktio_flow_inl_dev_update,
  ...
  .pktio_link_advertise_set = cnxk_pktio_link_advertise_set,
  .pktio_link_state_set = cnxk_pktio_link_state_set,
