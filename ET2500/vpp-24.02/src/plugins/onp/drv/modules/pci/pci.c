...
pci_device_id_t cnxk_pci_device_ids[] = {
...
void *
cnxk_pci_dev_probe (vlib_main_t *vm, vlib_pci_addr_t *addr, uuid_t uuid_token,
		    vlib_pci_dev_handle_t *pci_handle)
{
  ...
  error = vlib_pci_bind_to_uio (vm, addr, (char *) "vfio-pci",
				0 /* no force binding */);
  if (error)
    {
      error = clib_error_return (error,
  ...
  error = vlib_pci_device_open_with_uuid (vm, addr, uuid_token,
					  cnxk_pci_device_ids, &handle);
  if (error)
    {
      error = clib_error_return (error,
