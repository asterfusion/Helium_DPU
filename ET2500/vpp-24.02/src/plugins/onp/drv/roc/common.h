...
#define plt_clz32		     __builtin_clz
#define plt_ctz64		     __builtin_ctzll
#define plt_popcount64		     __builtin_popcountll
#define plt_popcount32		     __builtin_popcount
			  ...
			  u32 offset)
{
  ...
  vm = vlib_get_main ();
  error = vlib_pci_read_write_config (vm, dev->pci_handle, VLIB_READ, offset,
				      dst, len);
  if (error)
    {
