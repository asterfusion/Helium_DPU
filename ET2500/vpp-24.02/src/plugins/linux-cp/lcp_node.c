...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  next0 = original_next;
	  if (lcp_packet_parse (vm, b0, LCP_MATCH_CTX_ARP, NULL, &view) &&
	      ...
	      else
		{
		  ...
		  if (action_result.cpu_bi != LCP_PUNT_BUFFER_INVALID)
		    {
		      /* The Linux copy owns the reply.  Mark only the original
		       * packet so arp-reply can suppress VPP's duplicate reply. */
