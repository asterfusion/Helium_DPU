...
lcp_copp_input_feature_set (u32 sw_if_index, u8 enable)
{
  ...
  if (enable)
    {
      if (lcp_copp_input_feature_refcounts[input_sw_if_index]++ == 0)
	vnet_feature_enable_disable ("device-input",
				     "linux-cp-copp-input-init",
				     input_sw_if_index, 1, NULL, 0);
    }
  else
    {
      ASSERT (lcp_copp_input_feature_refcounts[input_sw_if_index] > 0);
      if (--lcp_copp_input_feature_refcounts[input_sw_if_index] == 0)
	vnet_feature_enable_disable ("device-input",
				     "linux-cp-copp-input-init",
...
lcp_copp_features_set (u32 phy_sw_if_index, u8 enable)
{
  ...
  vnet_feature_enable_disable ("ip6-multicast",
			       ...
			       phy_sw_if_index, enable, NULL, 0);
  vnet_feature_enable_disable ("ip4-local", "linux-cp-ip4-local-punt",
			       phy_sw_if_index, enable, NULL, 0);
