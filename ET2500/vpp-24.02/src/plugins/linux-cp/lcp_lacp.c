...
#define foreach_lcp_lacp                                                      \
  _ (DROP, "error-drop")                                                      \
  _ (IO, "interface-output")

typedef enum
{
#define _(sym, str) LCP_LACP_NEXT_##sym,
  foreach_lcp_lacp
#undef _
    LCP_LACP_N_NEXT,
} lcp_lacp_next_t;
...
static u8 *
format_lcp_lacp_trace (u8 *s, va_list *args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  lcp_lacp_trace_t *t = va_arg (*args, lcp_lacp_trace_t *);
...
VLIB_NODE_FN (lcp_lacp_punt_node) (vlib_main_t * vm,
					   ...
					   vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
  {
	  ...
	  u8 len0, len1;
          ...
          next0 = next1 = LCP_LACP_NEXT_DROP;
 ...
 */
/* *INDENT-OFF* */
VLIB_REGISTER_NODE(lcp_lacp_punt_node) = {
  .name = "lcp-lacp-punt",
  ...
  .format_trace = format_lcp_lacp_trace,

  .n_next_nodes = LCP_LACP_N_NEXT,
  .next_nodes =
  {
    [LCP_LACP_NEXT_DROP] = "error-drop",
    [LCP_LACP_NEXT_IO] = "interface-output",
  },
};
