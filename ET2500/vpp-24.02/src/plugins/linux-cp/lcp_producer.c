...
lcp_trap_is_ndp (vl_api_lcp_trap_type_t trap_type)
{
	  ...
	  if (!copp_processed && vnet_buffer2 (b)->trap_id == LCP_TRAP_INVALID &&
	      ...
	      else
		{
		  ...
		  if (action_result.cpu_bi != LCP_PUNT_BUFFER_INVALID)
		    {
		      /* The Linux copy owns NDP processing.  Mark only the
		       * original packet so VPP suppresses duplicate replies/punts. */
		      if (lcp_trap_is_ndp (result.trap_type))
			vnet_buffer2 (b)->lcp_host_copy_done = 1;
      ...
      while (n_left && n_left_to_next)
	{
	  ...
	  if (lcp_packet_parse (vm, b, context, &view) &&
	      ...
	      else
		{
		  ...
		  if (action_result.cpu_bi != LCP_PUNT_BUFFER_INVALID)
		    {
		      /* NDP is classified on the L2 IPv6 feature arc.  Once its
		       * Linux copy exists, suppress duplicate VPP replies/punts. */
		      if (lcp_trap_is_ndp (result.trap_type))
			vnet_buffer2 (b)->lcp_host_copy_done = 1;
		      copies[n_copies++] = action_result.cpu_bi;
		    }
