...
#define CHECK_FIELD(_field, _expected, _fmt)                                  \
  ...
  if (!lcp_packet_parse (vm, b, LCP_MATCH_CTX_IP6, NULL, &view))
    {
  ...
  metadata.ip_protocol = IP_PROTOCOL_UDP;
  metadata.l4_src_port = 50000;
  metadata.l4_dst_port = 161;
  metadata.l4_ports_valid = true;
  if (!lcp_packet_parse (vm, b, LCP_MATCH_CTX_IP6, &metadata, &view))
    {
