...
hqos_reciprocal hqos_reciprocal_value(uint32_t d)
{
    ...
    l = ((d - 1) == 0) ? 0 : 32 - hqos_count_leading_zeros_32((d-1));   
    m = ((1ULL << 32) * ((1ULL << l) - d));
    m /= d;
...
divide_128_div_64_to_64(uint64_t u1, uint64_t u0, uint64_t v, uint64_t *r)
{
    ...
    /* Count leading zeros. */
    s = hqos_count_leading_zeros_64(v);
    if (s > 0) {
        v = v << s;
...
hqos_reciprocal_u64
hqos_reciprocal_value_u64(uint64_t d)
{
    hqos_reciprocal_u64 R;
    uint64_t m;
    uint64_t r;
    int l;

    l = 63 - hqos_count_leading_zeros_64(d);
