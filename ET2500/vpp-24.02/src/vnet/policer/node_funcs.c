...
static u8 *
format_policer_acl_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  policer_acl_trace_t *t = va_arg (*args, policer_acl_trace_t *);

  s = format (s, "Policer-Acl: next %d policer_index %d",
	      t->next_index, t->policer_index);
  return s;
}

#define foreach_policer_acl_error                 \
_(DROP, "Policer acl action drop")

typedef enum
{
#define _(sym,str) POLICER_ACL_ERROR_##sym,
  foreach_policer_acl_error
#undef _
    POLICER_ACL_N_ERROR,
} policer_acl_error_t;

static char *policer_acl_error_strings[] = {
#define _(sym,string) string,
  foreach_policer_acl_error
#undef _
};
...
static inline uword
policer_acl_inline(vlib_main_t* vm,
			 vlib_node_runtime_t * node,
			 vlib_frame_t * frame)
{
  u32 n_left_from, *from, *to_next;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  pi0 = b0->policer_index;
	  pi1 = b1->policer_index;
	  ...
	  if (PREDICT_FALSE (act0 == QOS_ACTION_DROP))
	    {
	      next0 = 0;
	      b0->error = node->errors[POLICER_ACL_ERROR_DROP];
	    }
	  else /* transmit or mark-and-transmit action */
	    {
	      transmitted++;
	  ...
	  if (PREDICT_FALSE (act1 == QOS_ACTION_DROP)) /* drop action */
	    {
	      next1 = 0;
	      b1->error = node->errors[POLICER_ACL_ERROR_DROP];
	    }
	  else /* transmit or mark-and-transmit action */
	    {
	      transmitted++;
...
VLIB_NODE_FN (policer_acl_in_nonip_node) (vlib_main_t * vm,
				  vlib_node_runtime_t * node,
				  vlib_frame_t * frame)
{
...
VLIB_REGISTER_NODE (policer_acl_in_nonip_node) =
{
  .name = "policer-acl-plugin-in-nonip-l2",
  .vector_size = sizeof (u32),
  .format_trace = format_policer_acl_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (policer_acl_error_strings),
  .error_strings = policer_acl_error_strings,
  .n_next_nodes = 1,
  .next_nodes =
  {
    [0] = "error-drop",
...
VNET_FEATURE_INIT (policer_acl_in_l2_nonip_fa_feature, static) =
{
  .arc_name = "l2-input-nonip",
  .node_name = "policer-acl-plugin-in-nonip-l2",
...
VNET_FEATURE_INIT (policer_acl_out_l2_nonip_fa_feature, static) =
{
  .arc_name = "l2-output-nonip",
  .node_name = "policer-acl-plugin-out-nonip-l2",
  .runs_before = VNET_FEATURES ("l2-output-feat-arc-end"),
};

VLIB_NODE_FN (policer_acl_in_l2_ip6_node) (vlib_main_t * vm,
				   vlib_node_runtime_t * node,
				   vlib_frame_t * frame)
{
  return policer_acl_inline (vm, node, frame);
...
VLIB_REGISTER_NODE (policer_acl_in_l2_ip6_node) =
{
  .name = "policer-acl-plugin-in-ip6-l2",
  .vector_size = sizeof (u32),
  .format_trace = format_policer_acl_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (policer_acl_error_strings),
...
VNET_FEATURE_INIT (policer_acl_in_l2_ip6_fa_feature, static) =
{
  .arc_name = "l2-input-ip6",
  .node_name = "policer-acl-plugin-in-ip6-l2",
  .runs_before = VNET_FEATURES ("l2-input-feat-arc-end"),
};

VLIB_NODE_FN (policer_acl_in_l2_ip4_node) (vlib_main_t * vm,
				   vlib_node_runtime_t * node,
				   vlib_frame_t * frame)
{
  return policer_acl_inline (vm, node, frame);
...
VLIB_REGISTER_NODE (policer_acl_in_l2_ip4_node) =
{
  .name = "policer-acl-plugin-in-ip4-l2",
  .vector_size = sizeof (u32),
  .format_trace = format_policer_acl_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (policer_acl_error_strings),
...
VNET_FEATURE_INIT (policer_acl_in_l2_ip4_fa_feature, static) =
{
  .arc_name = "l2-input-ip4",
  .node_name = "policer-acl-plugin-in-ip4-l2",
  .runs_before = VNET_FEATURES ("l2-input-feat-arc-end"),
};

VLIB_NODE_FN (policer_acl_out_l2_ip6_node) (vlib_main_t * vm,
				    vlib_node_runtime_t * node,
				    vlib_frame_t * frame)
{
  return policer_acl_inline (vm, node, frame);
...
VLIB_REGISTER_NODE (policer_acl_out_l2_ip6_node) =
{
  .name = "policer-acl-plugin-out-ip6-l2",
  .vector_size = sizeof (u32),
  .format_trace = format_policer_acl_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (policer_acl_error_strings),
...
VNET_FEATURE_INIT (policer_acl_out_l2_ip6_fa_feature, static) =
{
  .arc_name = "l2-output-ip6",
  .node_name = "policer-acl-plugin-out-ip6-l2",
  .runs_before = VNET_FEATURES ("l2-output-feat-arc-end"),
};

VLIB_NODE_FN (policer_acl_out_l2_ip4_node) (vlib_main_t * vm,
				    vlib_node_runtime_t * node,
				    vlib_frame_t * frame)
{
  return policer_acl_inline (vm, node, frame);
...
VLIB_REGISTER_NODE (policer_acl_out_l2_ip4_node) =
{
  .name = "policer-acl-plugin-out-ip4-l2",
  .vector_size = sizeof (u32),
  .format_trace = format_policer_acl_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (policer_acl_error_strings),
...
VNET_FEATURE_INIT (policer_acl_out_l2_ip4_fa_feature, static) =
{
  .arc_name = "l2-output-ip4",
  .node_name = "policer-acl-plugin-out-ip4-l2",
  .runs_before = VNET_FEATURES ("l2-output-feat-arc-end"),
};

VLIB_NODE_FN (policer_acl_in_fa_ip6_node) (vlib_main_t * vm,
				   vlib_node_runtime_t * node,
				   vlib_frame_t * frame)
{
  return policer_acl_inline (vm, node, frame);
...
VLIB_REGISTER_NODE (policer_acl_in_fa_ip6_node) =
{
  .name = "policer-acl-plugin-in-ip6-fa",
  .vector_size = sizeof (u32),
  .format_trace = format_policer_acl_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (policer_acl_error_strings),
...
VNET_FEATURE_INIT (policer_acl_in_ip6_fa_feature, static) =
{
  .arc_name = "ip6-unicast",
  .node_name = "policer-acl-plugin-in-ip6-fa",
  .runs_before = VNET_FEATURES ("ip6-flow-classify"),
};

VLIB_NODE_FN (policer_acl_in_fa_ip4_node) (vlib_main_t * vm,
				   vlib_node_runtime_t * node,
				   vlib_frame_t * frame)
{
  return policer_acl_inline (vm, node, frame);
...
VLIB_REGISTER_NODE (policer_acl_in_fa_ip4_node) =
{
  .name = "policer-acl-plugin-in-ip4-fa",
  .vector_size = sizeof (u32),
  .format_trace = format_policer_acl_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (policer_acl_error_strings),
...
VNET_FEATURE_INIT (policer_acl_in_ip4_fa_feature, static) =
{
  .arc_name = "ip4-unicast",
  .node_name = "policer-acl-plugin-in-ip4-fa",
  .runs_before = VNET_FEATURES ("ip4-flow-classify"),
};

VLIB_NODE_FN (policer_acl_out_fa_ip6_node) (vlib_main_t * vm,
				    vlib_node_runtime_t * node,
				    vlib_frame_t * frame)
{
  return policer_acl_inline (vm, node, frame);
...
VLIB_REGISTER_NODE (policer_acl_out_fa_ip6_node) =
{
  .name = "policer-acl-plugin-out-ip6-fa",
  .vector_size = sizeof (u32),
  .format_trace = format_policer_acl_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (policer_acl_error_strings),
...
VNET_FEATURE_INIT (policer_acl_out_ip6_fa_feature, static) = {
  .arc_name = "ip6-output",
  .node_name = "policer-acl-plugin-out-ip6-fa",
  .runs_before = VNET_FEATURES ("ip6-dvr-reinject", "interface-output"),
};

VLIB_NODE_FN (policer_acl_out_fa_ip4_node) (vlib_main_t * vm,
				    vlib_node_runtime_t * node,
				    vlib_frame_t * frame)
{
  return policer_acl_inline (vm, node, frame);
...
VLIB_REGISTER_NODE (policer_acl_out_fa_ip4_node) =
{
  .name = "policer-acl-plugin-out-ip4-fa",
  .vector_size = sizeof (u32),
  .format_trace = format_policer_acl_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (policer_acl_error_strings),
...
VNET_FEATURE_INIT (policer_acl_out_ip4_fa_feature, static) = {
  .arc_name = "ip4-output",
  .node_name = "policer-acl-plugin-out-ip4-fa",
  .runs_before = VNET_FEATURES ("ip4-dvr-reinject", "interface-output"),
};

VLIB_NODE_FN (policer_acl_in_sai_nonip_node) (vlib_main_t * vm,
				    vlib_node_runtime_t * node,
				    vlib_frame_t * frame)
{
  return policer_acl_inline (vm, node, frame);
...
VLIB_REGISTER_NODE (policer_acl_in_sai_nonip_node) =
{
  .name = "policer-acl-plugin-in-sai-nonip-l2",
  .vector_size = sizeof (u32),
  .format_trace = format_policer_acl_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (policer_acl_error_strings),
...
VNET_FEATURE_INIT (policer_acl_in_l2_sai_nonip_fa_feature, static) =
{
  .arc_name = "l2-input-nonip",
  .node_name = "policer-acl-plugin-in-sai-nonip-l2",
  .runs_before = VNET_FEATURES ("l2-input-feat-arc-end"),
};

VLIB_NODE_FN (policer_acl_out_sai_nonip_node) (vlib_main_t * vm,
				    vlib_node_runtime_t * node,
				    vlib_frame_t * frame)
{
  return policer_acl_inline (vm, node, frame);
...
VLIB_REGISTER_NODE (policer_acl_out_sai_nonip_node) =
{
  .name = "policer-acl-plugin-out-sai-nonip-l2",
  .vector_size = sizeof (u32),
  .format_trace = format_policer_acl_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,
  .n_errors = ARRAY_LEN (policer_acl_error_strings),
...
VNET_FEATURE_INIT (policer_acl_out_l2_sai_nonip_fa_feature, static) =
{
  .arc_name = "l2-output-nonip",
  .node_name = "policer-acl-plugin-out-sai-nonip-l2",
  .runs_before = VNET_FEATURES ("l2-output-feat-arc-end"),
};
/*
