                      ...
                      int add)
{
    ...
    if(pfx->fp_proto == FIB_PROTOCOL_IP4 && si->ipv4_multicast_mac_flag == false && add)
    {
        ...
        clib_warning("MFIB: Multicast mac address added for ipv4 interface");
    }
    else if(pfx->fp_proto == FIB_PROTOCOL_IP6 && si->ipv6_multicast_mac_flag == false && add)
    {
        ...
        clib_warning("MFIB: Multicast mac address added for ipv6 interface");
    }
    else if(pfx->fp_proto == FIB_PROTOCOL_IP4 && !add)
    {
        si->ipv4_multicast_mac_count--;
        clib_warning("MFIB: ipv4_multicast_mac_count--");
        if(si->ipv4_multicast_mac_count == 0)
        {
            clib_warning("MFIB: ipv4_multicast_mac_count=0");
    ...
    {
        si->ipv6_multicast_mac_count--;
        clib_warning("MFIB: ipv6_multicast_mac_count--");
        if(si->ipv6_multicast_mac_count == 0)
        {
            clib_warning("MFIB: ipv6_multicast_mac_count=0");
    ...
    else if(pfx->fp_proto == FIB_PROTOCOL_IP4 && si->ipv4_multicast_mac_flag == true && add)
    {
        clib_warning("MFIB: Multicast mac address already added for ipv4 interface");
    }
    else if(pfx->fp_proto == FIB_PROTOCOL_IP6 && si->ipv6_multicast_mac_flag == true && add)
    {
        clib_warning("MFIB: Multicast mac address already added for ipv6 interface");
    }
}
