...
/*
 * Wait for a batch alloc operation on a cache line to complete and count the
 * number of pointers.
 */
static inline uint8_t
roc_npa_batch_alloc_count(struct npa_batch_alloc_status_s *status,
			  unsigned int wait_us)
{
	 ...
	 */
	while (((__atomic_load_n((uint8_t *)status, __ATOMIC_RELAXED) >> 5)
	       & 0x3) == ALLOC_CCODE_INVAL)
			       ...
			       unsigned int wait_us)
{
	...
	for (i = 0; i < num; i += (ROC_ALIGN >> 3)) {
		...
		count += roc_npa_batch_alloc_count(status, wait_us);
	}
 ...
 * adjusts the status->count so that a subsequent call to this api can
				      ...
				      unsigned int num, unsigned int *rem)
{
	...
	avail = roc_npa_batch_alloc_count(status, 0);
				 ...
				 unsigned int num)
{
	...
	for (i = 0; i < num; i += (ROC_ALIGN >> 3)) {
		...
		line_count = roc_npa_batch_alloc_count(status, 0);
