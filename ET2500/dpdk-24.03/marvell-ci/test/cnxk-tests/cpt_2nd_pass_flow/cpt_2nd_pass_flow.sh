...
function start_ping_test()
{
	...
	while [ $Y -le $MAX_Y ]; do
		...
		while [ $itr -le $PING_RETRY ]; do
			...
			if [ $check -eq 1 ]; then
				PING_COUNT=$(($PING_COUNT+$PING_PKTS))
				break;
			fi
			((++itr))
		done
...
function run_test()
{
	...
	sync
	HITS=`cat $APP_LOG | egrep -e "(Flow.*Hits)" | grep -Po 'Hits:.*' | cut -d' ' -f2`
	echo "HITS: $HITS"
	if [ "$PING_COUNT" -ne "$HITS" ]; then
		echo "FAILED:Flow hits ($HITS) does not match ping count ($PING_COUNT)"
		exit 1
	else
		echo "cpt_2nd_pass_flow PASSED"
	fi
...
function run_ipsec_secgw_cn10k()
{
	...
	run_test '$DPDK_TEST_BIN -c $IPSEC_COREMASK -a $CDEV_VF -a $INLINE_DEV,ipsec_in_max_spi=128 -a $EVENT_VF -a $LIF2,ipsec_in_max_spi=128 -a $LIF4,ipsec_in_max_spi=128 --file-prefix $PREFIX -- -P -p 0x3 -u 0x1 -f ./ep1_inline_test.cfg --transfer-mode event --event-schedule-type parallel'
	...
	run_test '$DPDK_TEST_BIN -c $IPSEC_COREMASK -a $CDEV_VF -a $INLINE_DEV,ipsec_in_max_spi=128 -a $LIF2,ipsec_in_max_spi=128 -a $LIF4,ipsec_in_max_spi=128 --file-prefix $PREFIX -- -P -p 0x3 -u 0x1 -f ./ep1_inline_test.cfg --transfer-mode poll --config="(0,0,1),(1,0,2)"'
}
