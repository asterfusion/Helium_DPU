...
static int
parse_udp_dst_port(const char *pktlen)
{
	unsigned long len;
	char *end = NULL;

	/* Parse decimal string */
	len = strtoull(pktlen, &end, 10);
...
static int
parse_ipv4_address(const char *token, uint32_t *ipv4)
{
...
#define CMD_LINE_OPT_IP_ADDR	   "ip-addr"
...
static const struct option lgopts[] = {
	...
	{CMD_LINE_OPT_IP_ADDR, 1, 0, CMD_LINE_OPT_IP_ADDR_NUM},
...
parse_args(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argvopt, short_options, lgopts,
		...
		case CMD_LINE_OPT_IP_ADDR_NUM:
			if (parse_ipv4_address(optarg, &ipv4_addr.s_addr) < 0) {
				printf("Invalid ip4 address\n");
				return -1;
			}
			break;
		case CMD_LINE_OPT_DST_PORT_NUM:
			udp_dst_port = parse_udp_dst_port(optarg);
			break;
...
static uint16_t
usr_app_node_process_scalar(struct rte_graph *graph, struct rte_node *node,
...
static struct rte_node_register usr_node = {
	.process = usr_app_node_process_scalar,
	.name = "app_node",
...
main(int argc, char **argv)
{
	...
	usr_node_id = rte_node_udp4_usr_node_add(usr_node.name);
	...
	ret = rte_node_ip4_route_add(htonl(ipv4_addr.s_addr), 32, 0,
				     RTE_NODE_IP4_LOOKUP_NEXT_IP4_LOCAL);

	if (ret < 0)
		rte_exit(EXIT_FAILURE,
			 "Unable to add ip4 route to graph\n");

	rte_node_udp4_dst_port_add(udp_dst_port, usr_node_id);
