...
pktgen_config_ports(void)
{
    ...
    RTE_ETH_FOREACH_DEV(pid)
    {
        ...
        for (int q = 0; q < rt.tx; q++) {
            ...
            int err;
            size_t sz;
            if (pktgen.nb_rx_txd_buffer[pid] == 0) {
                pktgen.nb_rx_txd_buffer[pid] = mbufs_per_port;
                printf("test:::pktgen.nb_rx_txd_buffer[%d]:%d", pid, pktgen.nb_rx_txd_buffer[pid]);
            }
            ...
            info->q[q].tx_mp =
                pktgen_mbuf_pool_create("Default TX", pid, q, pktgen.nb_rx_txd_buffer[pid], sid, cache_size);
            ...
            info->q[q].range_mp =
                pktgen_mbuf_pool_create("Range TX", pid, q, pktgen.nb_rx_txd_buffer[pid], sid, 0);
            ...
            info->q[q].rate_mp = pktgen_mbuf_pool_create("Rate TX", pid, q, pktgen.nb_rx_txd_buffer[pid], sid, 0);
            ...
            info->q[q].seq_mp =
                pktgen_mbuf_pool_create("Sequence TX", pid, q, pktgen.nb_rx_txd_buffer[pid], sid, cache_size);
