...
pktgen_usage(const char *prgname)
{
    printf("Usage: %s [EAL options] -- [-h] [-v] [-P] [-G] [-g host:port] [-T] [-f cmd_file] [-l "
           ...
           "  --set-buffer-per-port [,,] set the number of descriptors in Rx rings "
           "every port. The number must be a multiple of 128\n"
...
pktgen_parse_args(int argc, char **argv)
{
    ...
    static struct option lgopts[] = {{"crc-strip", 0, 0, 0},
                                     ...
                                     {"set-buffer-per-port", required_argument, 0, 'A'},
                                     {NULL, 0, 0, 0}};
    ...
    pktgen.verbose = 0;
    while ((opt = getopt_long(argc, argvopt, "p:m:f:l:s:g:A:hPNGTvjtrc", lgopts, &option_index)) !=
           EOF)
        switch (opt) {
        case 't':
        ...
        case 'h': /* print out the help message */
            pktgen_usage(prgname);
            return -1;
        case 'A': /*set-buffer-per-port*/
            char *input = strdup(optarg);
            printf("%s", input);
            input[strlen(input) - 1] = '\0';
            char *token              = strtok(input + 1, ",");
            int i                    = 0;
            while (token != NULL && i < MAX_STRING) {
                pktgen.nb_rx_txd_buffer[i] =
                    (uint16_t)atoi(token) * DEFAULT_MBUFS_PER_PORT_MULTIPLER;
                token = strtok(NULL, ",");
                i++;
            }
            int count = sizeof(pktgen.nb_rx_txd_buffer) / sizeof(pktgen.nb_rx_txd_buffer[1]);
            for (int i = 0; i < count; i++) {
                printf("pktgen.nb_rx_txd_buffer[%d] = %u\n", i, pktgen.nb_rx_txd_buffer[i]);
            }
            free(input);
            break;
