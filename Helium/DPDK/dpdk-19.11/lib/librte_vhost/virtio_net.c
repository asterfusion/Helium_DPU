...
vhost_dequeue_offload(struct virtio_net_hdr *hdr, struct rte_mbuf *m)
{
	...
	if (l4_hdr && hdr->gso_type != VIRTIO_NET_HDR_GSO_NONE) {
		...
		case VIRTIO_NET_HDR_GSO_TCPV6:
            ...
            //RTE_LOG(ERR, VHOST_DATA, "l4 len %u size %lu----\n", m->l4_len, m->tso_segsz);
			break;
...
static __rte_always_inline int
copy_desc_to_mbuf_max(struct virtio_net *dev, struct vhost_virtqueue *vq,
          ...
          uint16_t max_pkt_len)
{
	...
	while (1) {
        ...
        //ec96 : over max size  --do no copy
        if (cpy_max_len > max_pkt_len) {
            error = 1;  
...
virtio_dev_tx_split_drop(struct virtio_net *dev, struct vhost_virtqueue *vq,
	  struct rte_mempool *mbuf_pool, struct rte_mbuf **pkts, uint16_t count,
      uint16_t *cpy_pkts, uint16_t drop_max_size)
{
	uint16_t i, ipkts;
	...
	for (i = 0; i < count; i++) {
		...
		pkts[ipkts] = virtio_dev_pktmbuf_alloc(dev, mbuf_pool, buf_len);
		if (unlikely(pkts[ipkts] == NULL))
			break;

		err = copy_desc_to_mbuf_max(dev, vq, buf_vec, nr_vec, pkts[ipkts],
				mbuf_pool, drop_max_size);
		if (unlikely(err < 0)) {
			rte_pktmbuf_free(pkts[ipkts]);
			break;
		} else if (err > 0){
			rte_pktmbuf_free(pkts[ipkts]);
            continue;
		...
		if (unlikely(dev->dequeue_zero_copy)) {
			 ...
			 */
			rte_mbuf_refcnt_update(pkts[ipkts], 1);
...
uint16_t
rte_vhost_dequeue_burst_drop(int vid, uint16_t queue_id, 
                    struct rte_mempool *mbuf_pool, struct rte_mbuf **pkts,
                    uint16_t count, uint16_t *cpy_pkts, uint16_t drop_max_size)
{
	...
	if (unlikely(!is_valid_virt_queue_idx(queue_id, 1, dev->nr_vring))) {
		RTE_LOG(ERR, VHOST_DATA, "(%d) %s: invalid virtqueue idx %d/%d.\n",
			dev->vid, __func__, queue_id, dev->nr_vring);
		return 0;
	}
	...
	if (vq_is_packed(dev)) {
		...
		count = virtio_dev_tx_split_drop(dev, vq, mbuf_pool, pkts, count, 
                                         cpy_pkts, drop_max_size);
