...
int
rte_eth_link_set_nowait(uint16_t port_id, struct rte_eth_link *eth_link)
{
	struct rte_eth_dev *dev;
	...
	return rte_eth_linkstatus_set(dev, eth_link);
}

int rte_nix_link_speed_update(uint16_t port_id, struct rte_eth_link *eth_link)
{
	int ret = 0;
	struct rte_eth_dev *dev;
	...
	RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->update_speed, -ENOTSUP);
	ret = (*dev->dev_ops->update_speed)(dev, eth_link->link_speed);
