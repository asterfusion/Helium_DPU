			...
			__attribute__((unused)) void *data)
{
	...
	RTE_ETH_FOREACH_DEV(pid) {
		...
		memset(&link, 0, sizeof(link));
        	...
        	else if (!strcmp(res->value1, "25000"))
        	{
                	link.link_speed = 25000;
        	}
        	...
        	//rte_eth_link_set_nowait(pid, &link);
        	ret = rte_nix_link_speed_update(pid, &link);
		if (ret)
		{
			printf("Bad arguments, set port %d failed\n", pid);
			return;
		}
				...
				__attribute__((unused)) void *data)
{
	...
	memset(&link, 0, sizeof(link));
	rte_eth_link_get_nowait(res->id, &link);
	if (!strcmp(res->value1, "100000"))
	{
		link.link_speed = 100000;
	}
	...
	//rte_eth_link_set_nowait(res->id, &link);
	ret = rte_nix_link_speed_update(res->id, &link);
	if (ret)
	{
		printf("Bad arguments\n");
	}
