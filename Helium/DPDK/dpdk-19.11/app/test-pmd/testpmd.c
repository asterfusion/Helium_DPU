...
set_rx_queue_stats_mapping_registers(portid_t port_id, struct rte_port *port)
{
	...
	for (i = 0; i < nb_rx_queue_stats_mappings; i++) {
		if ((rx_queue_stats_mappings[i].port_id == port_id) &&
			...
			//printf("### mapping port_id %d, queue_id %d, stats %d\n", port_id, rx_queue_stats_mappings[i].queue_id,
	...
	RTE_ETH_FOREACH_DEV(pid) {
		...
		if (port->dcb_flag == 0) {
			if( port->dev_conf.rx_adv_conf.rss_conf.rss_hf != 0)
			{
				...
				//port->dev_conf.rxmode.offloads = DEV_RX_OFFLOAD_RSS_HASH; //set pkts_burst[i]->hash.rss
			}
			else
		...
		rte_eth_devices[pid].data->nb_rx_queues = nb_rxq;
		rte_eth_devices[pid].data->nb_tx_queues = nb_txq;
		//printf("rxmod mq_mode %d, offloads %lx, nb_rxq %d, nb_txq %d\n", port->dev_conf.rxmode.mq_mode, port->dev_conf.rxmode.offloads, nb_rxq, nb_txq); 
