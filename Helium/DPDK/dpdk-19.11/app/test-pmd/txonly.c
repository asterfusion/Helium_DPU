...
static struct rte_tcp_hdr pkt_tcp_hdr; /** tcp header of tx packets */
...
static void
setup_pkt_tcp_ip_headers(struct rte_ipv4_hdr *ip_hdr,
			 ...
			 uint16_t pkt_data_len)
{
	...
	tcp_hdr->src_port = rte_cpu_to_be_16(tx_udp_src_port);
	tcp_hdr->dst_port = rte_cpu_to_be_16(tx_udp_dst_port);
	tcp_hdr->sent_seq = rte_cpu_to_be_32(0x1);
	tcp_hdr->recv_ack = rte_cpu_to_be_32(0x1);
	tcp_hdr->data_off = 0x50;
	tcp_hdr->tcp_flags = 0x10;
	tcp_hdr->rx_win = 0x7210;
	tcp_hdr->cksum = 0;
	tcp_hdr->tcp_urp = 0;
		...
		const uint16_t vlan_tci_outer, const uint64_t ol_flags)
{
	...
	pkt->l4_len = sizeof(pkt_udp_hdr);
#else
	copy_buf_to_pkt(&pkt_tcp_hdr, sizeof(pkt_tcp_hdr), pkt,
			sizeof(struct rte_ether_hdr) +
			sizeof(struct rte_ipv4_hdr));
	pkt->l4_len = sizeof(pkt_tcp_hdr);
	pkt->tso_segsz = 1452;//test tso
#endif
	/*
...
tx_only_begin(__attribute__((unused)) portid_t pi)
{
	...
	setup_pkt_tcp_ip_headers(&pkt_ip_hdr, &pkt_tcp_hdr, pkt_data_len);
#endif
}
