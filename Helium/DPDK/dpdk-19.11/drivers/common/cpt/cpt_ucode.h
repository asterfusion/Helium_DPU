...
cpt_is_algo_supported(struct rte_crypto_sym_xform *xform)
{
	...
	/*
	 * Microcode supports the following combination.
	 * 1. Encryption followed by authentication
	 * 2. Authentication followed by decryption
	 * Auth followed by encryption, decryption followed
	 * by auth is supported only for aes-cbc-hmac-sha1
	 */
		  ...
		  void **prep_req)
{
	...
	opcode.s.minor = 0;
	opcode.s.minor |= (cpt_ctx->auth_enc << CPT_HMAC_FIRST_BIT_POS);
		  ...
		  void **prep_req)
{
	...
	opcode.s.major = CPT_MAJOR_OP_FC;
	opcode.s.minor = 1;
	opcode.s.minor |= (cpt_ctx->dec_auth << CPT_HMAC_FIRST_BIT_POS);
