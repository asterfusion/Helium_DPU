			...
			struct rte_mempool *mp)
{
    ...
    /*EC2004Y set min_rx_buf_size */
    eth_dev->data->min_rx_buf_size = rte_pktmbuf_data_room_size(mp);

	/* Setup scatter mode if needed by jumbo */
	otx2_nix_enable_mseg_on_jumbo(rxq);

	/* Configure NPA pool based BP */
...
static void
nix_queue_cfg_release(struct rte_eth_dev *eth_dev)
{
	struct otx2_eth_dev *dev = otx2_eth_pmd_priv(eth_dev);
	struct otx2_eth_txq **txq;
	struct otx2_eth_rxq **rxq;
...
//FIXME now do not save rxq/txq config . see EC96-66
//		rc = nix_store_queue_cfg_and_then_release(eth_dev);
//		if (rc)
//			goto fail_configure;
        nix_queue_cfg_release(eth_dev);
...
otx2_nix_dev_stop(struct rte_eth_dev *eth_dev)
{
	...
	for (i = 0; i < eth_dev->data->nb_rx_queues; i++) {
		...
		if (rc)
			continue;
        if ( dev->rx_pkt_burst_no_offload != NULL) {
...
static const struct eth_dev_ops otx2_eth_dev_ops = {
    ...
    .update_speed             = otx2_nix_change_speed,
};
...
otx2_eth_dev_init(struct rte_eth_dev *eth_dev)
{
	...
	if (otx2_eth_dev_is_sdp(pci_dev)) {
		if (otx2_dev_is_pf(bdev)) {
			...
			uint8_t rcnt, vf_srn; 
			if (!bdev->vf) {
				if (otx2_get_sdp_sysfs_entry(&rcnt,
						SDP_SYSFS_VF0_RING_ENTRY))
					goto otx2_dev_uninit;
				dev->req_chan_base = NIX_CHAN_SDP_CH_START;
				dev->req_chan_cnt = rcnt;
			} else {
				if (otx2_get_sdp_sysfs_entry(&vf_srn,
						SDP_SYSFS_VF0_RING_ENTRY))
					goto otx2_dev_uninit;
				if (otx2_get_sdp_sysfs_entry(&rcnt,
						SDP_SYSFS_VFX_RING_ENTRY))
					goto otx2_dev_uninit;
				dev->req_chan_base = NIX_CHAN_SDP_CH_START +
					rte_align32pow2(vf_srn) +
					((bdev->vf - 1) * rcnt);
				dev->req_chan_cnt = rcnt;
			}
		}
