...
#define ETHTOOL_SCHAR_MAX    127
#define ETHTOOL_LINK_MODE_MASK_MAX_BITS    32 * ETHTOOL_SCHAR_MAX
#define ETHTOOL_LINK_MODE_MASK_MAX_BYTES   4 * ETHTOOL_SCHAR_MAX
#define OTX2_ETHTOOL_ALL_MODES (BIT_ULL(51) - 1) 
static int ethtool_parse_hex_u32_bitmap(const char *s, unsigned long *advertising_tmp)
{
	const unsigned int nwords = ETHTOOL_SCHAR_MAX;
	size_t slen = strlen(s);
	size_t i;
    	uint32_t result[ETHTOOL_SCHAR_MAX] = {0};
    ...
    memcpy(advertising_tmp, result, sizeof(*advertising_tmp));
    //memcpy(cmd->link_modes.advertising, result, ETHTOOL_LINK_MODE_MASK_MAX_BYTES);
	return 0;
}

int otx2_nix_change_speed(struct rte_eth_dev *eth_dev, uint32_t link_speed)
{
	int ret = 0;
	struct otx2_eth_dev *dev = otx2_eth_pmd_priv(eth_dev);
	...
	unsigned long advertising_tmp = 0;
	unsigned long advertising = 0;

	if (link_speed == 40000)
	{
		ethtool_parse_hex_u32_bitmap("0x4000000", &advertising_tmp);
	}

	else if (100000 == link_speed)
	{
		ethtool_parse_hex_u32_bitmap("0x8000000000", &advertising_tmp);
	}

	else if (25000 == link_speed)
        {       
                ethtool_parse_hex_u32_bitmap("0x200000000", &advertising_tmp);
        }

	else if (10000 == link_speed)
        {       
                ethtool_parse_hex_u32_bitmap("0x100000", &advertising_tmp);
        }

        else if (1000 == link_speed)
        {       
                ethtool_parse_hex_u32_bitmap("0x20000000000", &advertising_tmp);
        }

	advertising = advertising_tmp & (OTX2_ETHTOOL_ALL_MODES);
