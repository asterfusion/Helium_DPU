...
otx2_nix_mtu_set(struct rte_eth_dev *eth_dev, uint16_t mtu)
{
	...
	{
		frame_size += CVM_RAW_FRONT_SIZE;
	}
	/* Check if MTU is within the allowed range */
	if (!otx2_dev_is_sdp(dev) && (frame_size < NIX_MIN_FRS ||
					frame_size > NIX_MAX_FRS))
	{
		//auto change frame_size
		if(frame_size > NIX_MAX_FRS)
		{
			otx2_err("set frame_size bigger than max %d, change to %d\n", frame_size, NIX_MAX_FRS);
			frame_size = NIX_MAX_FRS;
		}
		else if(frame_size < NIX_MIN_FRS)
		{
			otx2_err("set frame_size smaller than min %d, change to %d\n", frame_size, NIX_MIN_FRS);
			frame_size = NIX_MIN_FRS;
		}
		else
			return -EINVAL;
	...
	if(otx2_dev_is_sdp(dev))
	{
		data->mtu = frame_size - NIX_L2_OVERHEAD + 8 - CVM_RAW_FRONT_SIZE; 
	}
	else
	{
		data->mtu = frame_size - NIX_L2_OVERHEAD + 8; 
	}
	otx2_info("[%s:%d] data mtu %d max_rx_pkt_len %d\n", __func__, __LINE__, data->mtu, data->dev_conf.rxmode.max_rx_pkt_len);
...
otx2_nix_recalc_mtu(struct rte_eth_dev *eth_dev)
{
	...
	otx2_info("[%s:%d] mtu %d data %d max_rx_pkt_len %d\n", __func__, __LINE__, mtu, data->mtu, data->dev_conf.rxmode.max_rx_pkt_len);
...
otx2_nix_info_get(struct rte_eth_dev *eth_dev, struct rte_eth_dev_info *devinfo)
{
	...
	devinfo->max_rx_pktlen = 65535;//NIX_MAX_FRS;
	if (otx2_dev_is_sdp(dev)) {
		int buffsz = RTE_MBUF_DEFAULT_BUF_SIZE;

		if ((int)data->min_rx_buf_size > buffsz)
			buffsz = data->min_rx_buf_size;
		buffsz -= RTE_PKTMBUF_HEADROOM;
		devinfo->max_rx_pktlen = 65535;//NIX_RX_NB_SEG_MAX * buffsz;
        }
