...
/* nix_cqe_hdr_s + nix_rx_parse_s + nix_rx_sg_s + nix_iova_s
 * equal to NIX_CQ_ENTRY_SZ
 */
//ec96
//#define NIX_DESC_SZ			128
#define NIX_DESC_SZ			0
...
nix_get_vlib_from_cqe(void *cq, const uint64_t data_off)
{
	...
	buff = *((rte_iova_t *)((uint64_t *)cq + 9));
	/* vlib resides in 2nd cacheline after rte_mbuf as
	 * per vpp configuration
	 */
	return (dpdk_vlib_buffer_t *)((buff - data_off) +
		sizeof(struct rte_mbuf) + NIX_DESC_SZ);
		     ...
		     const uint64_t val, const uint16_t flag)
{
	...
	if (rx->lctype)
		mbuf->l2_len = rx->lcptr - rx->laptr;
	if (rx->ldtype)
		mbuf->l3_len = rx->ldptr - rx->lcptr;
