...
octeontx_vlan_hw_filter(struct octeontx_nic *nic, uint8_t flag)
{
	...
	fltr_conf.port_type = nic->port_type;
	fltr_conf.fltr_conf = flag;

	rc = octeontx_pki_port_vlan_fltr_config(nic->ptbl_id, &fltr_conf);
	if (rc != 0) {
...
octeontx_dev_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on)
{
	...
	fltr_entry.port_type = nic->port_type;
	...
	rc = octeontx_pki_port_vlan_fltr_entry_config(nic->ptbl_id,
						      &fltr_entry);
	if (rc != 0) {
