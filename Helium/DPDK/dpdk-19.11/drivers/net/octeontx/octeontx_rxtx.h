...
#define OCCTX_RX_RSS_HASH_F             BIT(3)
		     ...
		     const uint16_t flags)
{
	...
	dq->wm_read[lcore].count++;
	if (unlikely(dq->wm_read[lcore].bp ||
	            (!(dq->wm_read[lcore].count & 0x1f)))) {
		if (octeontx_read64(dq->wm_cnt_reg)  > 1500) {
			dq->wm_read[lcore].bp = 1;
			return 0;
		} else {
			dq->wm_read[lcore].bp = 0;
		}
	}
	while (count < nb_pkts) {
...
#define RSS_HASH_F	OCCTX_RX_RSS_HASH_F
/* [RSS_HASH] [VLAN_FLTR] [CSUM] [MULTI_SEG] */
#define OCCTX_RX_FASTPATH_MODES						\
R(no_offload,			0, 0, 0, 0,  OCCTX_RX_OFFLOAD_NONE)	\
R(mseg,				0, 0, 0, 1,  MULT_RX_F)			\
R(csum,				0, 0, 1, 0,  CSUM_F)			\
R(csum_mseg,			0, 0, 1, 1,  CSUM_F | MULT_RX_F)		\
R(vlan,				0, 1, 0, 0,  VLAN_FLTR_F)			\
R(vlan_mseg,			0, 1, 0, 1,  VLAN_FLTR_F | MULT_RX_F)	\
R(vlan_csum,			0, 1, 1, 0,  VLAN_FLTR_F | CSUM_F)		\
R(vlan_csum_mseg,		0, 1, 1, 1,  VLAN_FLTR_F | CSUM_F | MULT_RX_F) \
R(hash,				1, 0, 0, 0,  RSS_HASH_F)	\
R(hash_mseg,			1, 0, 0, 1,  RSS_HASH_F | MULT_RX_F)			\
R(hash_csum,			1, 0, 1, 0,  RSS_HASH_F | CSUM_F)			\
R(hash_csum_mseg,		1, 0, 1, 1,  RSS_HASH_F | CSUM_F | MULT_RX_F)		\
R(hash_vlan,			1, 1, 0, 0,  RSS_HASH_F | VLAN_FLTR_F)			\
R(hash_vlan_mseg,		1, 1, 0, 1,  RSS_HASH_F | VLAN_FLTR_F | MULT_RX_F)	\
R(hash_vlan_csum,		1, 1, 1, 0,  RSS_HASH_F | VLAN_FLTR_F | CSUM_F)		\
R(hash_vlan_csum_mseg,		1, 1, 1, 1,  RSS_HASH_F | VLAN_FLTR_F | CSUM_F | MULT_RX_F)
