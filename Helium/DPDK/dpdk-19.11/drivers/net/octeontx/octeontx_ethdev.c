...
uint16_t
rte_octeontx_chan_mask[OCTEONTX_MAX_BGX_PORTS];
uint16_t
				...
				struct rte_vdev_device *dev)
{
	...
	static const char * const octeontx_vdev_valid_params[] = {
		...
		OCTEONTX_VDEV_PCI_PORT_ARG,
		OCTEONTX_VDEV_SW_PORT_ARG,
		NULL
	};
...
octeontx_port_open(struct octeontx_nic *nic)
{
	...
	octeontx_mbox_sdp_port_conf_t sdp_port_conf;
	octeontx_mbox_bgx_port_fifo_cfg_t fifo_cfg;
	int res = 0;

	PMD_INIT_FUNC_TRACE();

	switch (nic->port_type) {
	case OCTTX_PORT_TYPE_NET:
		memset(&bgx_port_conf, 0x0, sizeof(bgx_port_conf));
		res = octeontx_bgx_port_open(nic->ptbl_id, &bgx_port_conf);
		if (res < 0) {
		...
		nic->duplex = ETH_LINK_FULL_DUPLEX;
		nic->bgx	= bgx_port_conf.bgx;
		nic->lmac	= bgx_port_conf.lmac;

		memset(&fifo_cfg, 0x0, sizeof(fifo_cfg));

		res = octeontx_bgx_port_get_fifo_cfg(nic->ptbl_id, &fifo_cfg);
		if (res < 0) {
	...
	case OCTTX_PORT_TYPE_PCI:
		memset(&sdp_port_conf, 0x0, sizeof(sdp_port_conf));
		/* We have single SDP PF port, so vf_id always 0 */
		res = octeontx_sdp_port_open(nic->ptbl_id, &sdp_port_conf);
		if (res < 0) {
			PMD_INIT_LOG(ERR, "Req failed for port open %d", res);
			return res;
		}

		nic->node = sdp_port_conf.node;
		nic->port_ena = sdp_port_conf.enable;
		nic->base_ichan = sdp_port_conf.base_chan;
		nic->base_ochan = sdp_port_conf.base_chan;
		nic->num_ichans = sdp_port_conf.num_chans;
		nic->num_ochans = sdp_port_conf.num_chans;
		break;
	default:
		PMD_INIT_LOG(ERR, "Invalid port type %d", nic->port_type);
		res = -EINVAL;
		break;
	}
...
octeontx_parse_link_speeds(uint32_t link_speeds)
{
	...
	/* TODO:
	 * this is a hack only for 2.5G PHY with retimer;
	 * we need to find proper way to forward these to kernel BGX driver.
	 */
...
octeontx_apply_link_speed(struct rte_eth_dev *dev)
{
	...
	cfg.autoneg = (conf->link_speeds & ETH_LINK_SPEED_FIXED) ? 0 : 1;
	cfg.duplex = octeontx_parse_eth_link_duplex(conf->link_speeds);
	cfg.qlm_mode = (((conf->link_speeds & ETH_LINK_SPEED_1G) ||
			 (conf->link_speeds & ETH_LINK_SPEED_2_5G)) ?
			OCTEONTX_QLM_MODE_SGMII :
			(conf->link_speeds & ETH_LINK_SPEED_10G) ?
...
octeontx_link_status_poll(void *arg)
{
	...
	if (nic->port_type == OCTTX_PORT_TYPE_PCI)
		res = 1;
	else
		res = octeontx_bgx_port_link_status(nic->ptbl_id);
...
octeontx_port_close(struct octeontx_nic *nic)
{
	...
	if (nic->port_type == OCTTX_PORT_TYPE_NET)
		octeontx_bgx_port_close(nic->ptbl_id);
	else if (nic->port_type == OCTTX_PORT_TYPE_PCI)
		octeontx_sdp_port_close(nic->ptbl_id);
...
octeontx_port_start(struct octeontx_nic *nic)
{
	...
	if (nic->port_type == OCTTX_PORT_TYPE_PCI)
		return octeontx_sdp_port_start(nic->ptbl_id);

	return octeontx_bgx_port_start(nic->ptbl_id);
}
...
octeontx_port_stop(struct octeontx_nic *nic)
{
	...
	if (nic->port_type == OCTTX_PORT_TYPE_PCI)
		return octeontx_sdp_port_stop(nic->ptbl_id);

	return octeontx_bgx_port_stop(nic->ptbl_id);
}
...
octeontx_port_promisc_set(struct octeontx_nic *nic, int en)
{
	...
	res = octeontx_bgx_port_promisc_set(nic->ptbl_id, en);
	if (res < 0) {
...
octeontx_port_mcast_set(struct octeontx_nic *nic, int en)
{
	...
	res = octeontx_bgx_port_multicast_set(nic->ptbl_id, en);
	if (res < 0) {
...
octeontx_port_stats(struct octeontx_nic *nic, struct rte_eth_stats *stats)
{
	...
	res = octeontx_bgx_port_stats(nic->ptbl_id, &bgx_stats);
	if (res < 0) {
...
octeontx_port_stats_clr(struct octeontx_nic *nic)
{
	...
	if (nic->port_type == OCTTX_PORT_TYPE_NET)
		return octeontx_bgx_port_stats_clr(nic->ptbl_id);
	return 0;
}
...
octeontx_rx_offload_flags(struct rte_eth_dev *eth_dev)
{
	...
	if (rxmode->mq_mode == ETH_MQ_RX_RSS &&
			(nic->rx_offloads & DEV_RX_OFFLOAD_RSS_HASH))
		flags |= OCCTX_RX_RSS_HASH_F;
...
octeontx_dev_close(struct rte_eth_dev *dev)
{
	...
	if (rte_atomic16_add_return(&evdev_refcnt, -1) == 0) {
...
octeontx_dev_mtu_set(struct rte_eth_dev *eth_dev, uint16_t mtu)
{
	...
	rc = octeontx_pko_send_mtu(nic->pko_vfid, frame_size);
	if (rc)
		return rc;

	rc = octeontx_bgx_port_mtu_set(nic->ptbl_id, frame_size);
	if (rc)
		return rc;
...
octeontx_dev_start(struct rte_eth_dev *dev)
{
	...
	if (ret)
		printf("Failed to set link configuration\n");
	...
	ret = octeontx_pki_port_start(nic->ptbl_id,
				      (pki_port_type_t *)&nic->port_type);
	if (ret < 0) {
...
pki_port_stop_error:
	octeontx_pki_port_stop(nic->ptbl_id,
			       (pki_port_type_t *)&nic->port_type);
channel_stop_error:
...
octeontx_dev_stop(struct rte_eth_dev *dev)
{
	...
	ret = octeontx_pki_port_stop(nic->ptbl_id,
				     (pki_port_type_t *)&nic->port_type);
	if (ret < 0) {
		     ...
		     unsigned int n)
{
	...
	res = octeontx_bgx_port_xstats(nic->ptbl_id, &bgx_stats);
	if (res < 0) {
		octeontx_log_err("failed to get port stats %d", nic->port_id);
		return res;
	}
...
static int
octeontx_dev_xstats_get_names(struct rte_eth_dev *dev,
...
static int
octeontx_dev_xstats_get_names_by_id(struct rte_eth_dev *dev,
...
octeontx_dev_mac_addr_del(struct rte_eth_dev *dev, uint32_t index)
{
	...
	ret = octeontx_bgx_port_mac_del(nic->ptbl_id, index);
	if (ret != 0)
			  ...
			  __rte_unused uint32_t vmdq)
{
	...
	ret = octeontx_bgx_port_mac_add(nic->ptbl_id, mac_addr->addr_bytes,
					index);
	if (ret < 0) {
					...
					struct rte_ether_addr *addr)
{
	...
	ret = octeontx_bgx_port_mac_set(nic->ptbl_id, addr->addr_bytes);
	if (ret == 0) {
		/* Update same mac address to BGX CAM table */
		ret = octeontx_bgx_port_mac_add(nic->ptbl_id, addr->addr_bytes,
						0);
	}
	if (ret < 0) {
		...
		struct rte_eth_dev_info *dev_info)
{
	...
	if (nic->port_type == OCTTX_PORT_TYPE_NET)
		dev_info->max_mac_addrs =
				octeontx_bgx_port_mac_entries_get(nic->ptbl_id);
...
octeontx_dq_info_getter(octeontx_dq_t *dq, void *out)
{
	...
	((octeontx_dq_t *)out)->wm_cnt_reg = dq->wm_cnt_reg;
}
				...
				uint16_t qidx)
{
	...
	memset(&txq->dq.wm_read, 0, sizeof(txq->dq.wm_read));
				...
				struct rte_mempool *mb_pool)
{
	...
	if (!nic->pki.initialized) {
		pktbuf_conf.port_type = nic->port_type;
		pki_hash.port_type = nic->port_type;
		pki_qos.port_type = nic->port_type;
		...
		ret = octeontx_pki_port_pktbuf_config(nic->ptbl_id,
						      &pktbuf_conf);
		if (ret != 0) {
		...
		if (nic->pki.hash_enable) {
			...
			pki_hash.tag_slf = 1;
			pki_hash.tag_prt = 1;
			octeontx_pki_port_hash_config(nic->ptbl_id, &pki_hash);
		}
		...
		ret = octeontx_pki_port_create_qos(nic->ptbl_id, &pki_qos);
		if (ret < 0) {
...
octeontx_dev_set_link_up(struct rte_eth_dev *eth_dev)
{
	...
	rc = octeontx_bgx_port_set_link_state(nic->ptbl_id, true);
	if (rc)
		goto done;
...
octeontx_dev_set_link_down(struct rte_eth_dev *eth_dev)
{
	...
	return octeontx_bgx_port_set_link_state(nic->ptbl_id, false);
}
static int
...
static int
octeontx_create(struct rte_vdev_device *dev, int port, int port_type,
		int ptbl_id, uint8_t evdev, int socket_id)
{
	int res;
	size_t pko_vfid;
	char octtx_name[OCTEONTX_MAX_NAME_LEN];
	char octtx_new_name[OCTEONTX_MAX_NAME_LEN];
	...
	pko_vfid = octeontx_pko_get_vfid(port_type, init_params.pci_port);
	...
	nic->ptbl_id = ptbl_id;
	rte_atomic16_add(&evdev_refcnt, 1);

	res = octeontx_port_open(nic);
	if (res < 0)
		goto err;

	if (port_type == OCTTX_PORT_TYPE_NET) {
		sprintf(octtx_new_name, "%s_net_b%d_l%d", name, nic->bgx, nic->lmac);
		strlcpy(eth_dev->data->name, octtx_new_name, sizeof(eth_dev->data->name));
		octeontx_log_info("%s interface %d name %s changed to %s",
				  __func__, nic->port_id, octtx_name,
				  octtx_new_name);
	} else if (port_type == OCTTX_PORT_TYPE_PCI) {
		if (nic->ptbl_id == (init_params.pci_port - 1))
			sprintf(octtx_new_name, "%s_pci_pf_%d", name, nic->ptbl_id);
		else
			sprintf(octtx_new_name, "%s_pci_vf_%d", name, nic->ptbl_id);
		strlcpy(eth_dev->data->name, octtx_new_name, sizeof(eth_dev->data->name));
		octeontx_log_info("%s interface %d name %s changed to %s",
				  __func__, nic->port_id, octtx_name,
				  octtx_new_name);
	...
	res = octeontx_pki_port_open(ptbl_id, (pki_port_type_t *)&port_type);
	if (res != 0) {
	...
	if ((init_params.sw_port == port) &&
	    (port_type == OCTTX_PORT_TYPE_NET)) {
		pki_prt_cfg_t prt_cfg = {0};

		prt_cfg.port_type = OCTTX_PORT_TYPE_NET;
		prt_cfg.mmask.dsa_parse = 1;
		prt_cfg.dsa_parse = 1;
		res = octeontx_pki_port_config(ptbl_id, &prt_cfg);
	}
	...
	if (port_type == OCTTX_PORT_TYPE_PCI) {
		data->dev_link.link_status = ETH_LINK_UP;
		nic->link_up = 1;
		nic->speed = OCTEONTX_LINK_SPEED_XAUI;
	} else {
	...
	if (nic->port_type == OCTTX_PORT_TYPE_NET) {
		...
		octeontx_bgx_port_mac_add(nic->ptbl_id, nic->mac_addr, 0);
	...
	if (nic->port_type == OCTTX_PORT_TYPE_NET) {
		rte_octeontx_chan_mask[(nic->base_ochan >> 8) & 0x7] = 4;
		rte_octeontx_pchan_map[(nic->base_ochan >> 8) & 0x7]
			[(nic->base_ochan >> 4) & 0xF] = data->port_id;
	} else { /* Assumed Host ports. Fix for LBK */
		int idx;

		rte_octeontx_chan_mask[(nic->base_ochan >> 8) & 0x7] = 0;
		for (idx = nic->base_ochan; idx <
			(nic->base_ochan + nic->num_ochans); idx++) {
			rte_octeontx_pchan_map[(idx >> 8) & 0x7]
					   [(idx >> 0) & 0x3F] = data->port_id;
		}
	}
...
octeontx_probe(struct rte_vdev_device *dev)
{
	...
	int tx_vfcnt, port_id, evdev, qnum, pnum, res, i, num_ports;
	struct rte_event_dev_config dev_conf;
	const char *eventdev_name = "event_octeontx";
	struct rte_event_dev_info info;
	struct rte_eth_dev *eth_dev;
	int bgx_port_count = 0;
	int sdp_port_count = 0;
	...
	num_ports = init_params.nr_port + init_params.pci_port;

	PMD_INIT_LOG(DEBUG, "nr_port %d pci_port %d dsa_port %d\n",
		     init_params.nr_port, init_params.pci_port,
		     init_params.sw_port);
	...
	rte_atomic16_set(&evdev_refcnt, 0);
	...
	for (i = 0; i < num_ports; i++) {
		if (i < init_params.nr_port)
			port_id = octeontx_create(dev, i, OCTTX_PORT_TYPE_NET,
						  bgx_port_count++, evdev,
						  socket_id);
		else
			port_id = octeontx_create(dev, i, OCTTX_PORT_TYPE_PCI,
						  sdp_port_count++, evdev,
						  socket_id);
