			   ...
			   size_t dq_num, octeontx_pko_dq_getter_t getter)
{
	...
	curr.wm_cnt_reg = (void *)((uintptr_t)ctl->pko[dq_vf].bar0 + PKO_VF_DQ_WM_CNT(dq));
...
size_t
octeontx_pko_get_vfid(int port_type, int num_pci_ports)
{
	size_t vf_cnt = octeontx_pko_vf_count();
	size_t vf_idx = 0;

	if (port_type == OCTTX_PORT_TYPE_NET)
		vf_idx = num_pci_ports;

	for (; vf_idx < vf_cnt; vf_idx++) {
