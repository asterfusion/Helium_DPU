...
int
octeontx_pki_port_open(int port, pki_port_type_t *ptype)
{
	uint16_t global_domain = octeontx_get_global_domain();
	pki_port_type_t port_type = *(pki_port_type_t *)ptype;
...
int
octeontx_pki_port_config(int port, pki_prt_cfg_t *prt_cfg)
{
	struct octeontx_mbox_hdr hdr;
	int res;

	pki_prt_cfg_t p_cfg = *(pki_prt_cfg_t *)prt_cfg;
	int len = sizeof(pki_prt_cfg_t);

	hdr.coproc = OCTEONTX_PKI_COPROC;
	hdr.msg = MBOX_PKI_SET_PORT_CONFIG;
	hdr.vfid = port;

	res = octeontx_mbox_send(&hdr, &p_cfg, len, NULL, 0);
	if (res < 0)
		return -EACCES;
