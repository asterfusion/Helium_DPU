			...
			struct otx_ep_droq *droq, int next_fetch)
{
	...
	if (unlikely(!info->length)) {
		...
		//otx_ep_dbg("OCTEON DROQ[%d]: read_idx: %d; "
		//	     "Data not ready yet, Retry; pending=%lu\n",
		//	   droq->q_no, droq->read_idx, droq->pkts_pending);
