		...
		const struct rte_event_eth_rx_adapter_queue_conf *queue_conf)
{
	...
	if (!found) {
		...
		if (edev->available_events < rxq->pool->size) {
			ssovf_log_err("Max available events %" PRIu32
				      " requested events in rxq pool %" PRIu32
				      "",
	...
	ret = octeontx_pki_port_modify_qos(nic->ptbl_id, &pki_qos);
	if (ret < 0)
		...
		const struct rte_eth_dev *eth_dev, int32_t rx_queue_id)
{
	...
	ret = octeontx_pki_port_delete_qos(nic->ptbl_id, &pki_qos);
	if (ret < 0)
