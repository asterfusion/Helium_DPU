...
#undef T
	...
	const event_dequeue_t ssow_deq[2][2][2][2] = {
#define R(name, f3, f2, f1, f0, flags)					\
	[f3][f2][f1][f0] =  ssows_deq_ ##name,
...
#undef R
	...
	dev->dequeue = ssow_deq
		[!!(edev->rx_offload_flags & OCCTX_RX_RSS_HASH_F)]
	...
	const event_dequeue_burst_t ssow_deq_burst[2][2][2][2] = {
#define R(name, f3, f2, f1, f0, flags)					\
	[f3][f2][f1][f0] =  ssows_deq_burst_ ##name,
...
#undef R
	...
	if (edev->is_timeout_deq) {
		const event_dequeue_t ssow_deq_timeout[2][2][2][2] = {
#define R(name, f3, f2, f1, f0, flags)					\
			[f3][f2][f1][f0] =  ssows_deq_timeout_ ##name,
...
#undef R
		...
		const event_dequeue_burst_t ssow_deq_timeout_burst[2][2][2][2] = {
#define R(name, f3, f2, f1, f0, flags)					\
			[f3][f2][f1][f0] =  ssows_deq_timeout_burst_ ##name,
