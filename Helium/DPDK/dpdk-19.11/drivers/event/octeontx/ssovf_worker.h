			  ...
			  const uint16_t flag, const void *lookup_mem)
{
	...
	if (!!(flag & OCCTX_RX_OFFLOAD_CSUM_F))
		mbuf->ol_flags = ssovf_octeontx_rx_olflags_get(lookup_mem,
							       wqe->w[2]);
	if (wqe->s.w2.lcty)
		mbuf->l2_len = wqe->s.w4.lcptr - wqe->s.w4.laptr;
	if (wqe->s.w2.lfty)
		mbuf->l3_len = wqe->s.w4.lfptr - wqe->s.w4.lcptr;
	...
	if (!!(flag & OCCTX_RX_RSS_HASH_F)) {
		mbuf->hash.rss = wqe->s.w1.tag;
		mbuf->ol_flags |= PKT_RX_RSS_HASH;
	}

	mbuf->port = rte_octeontx_pchan_map[(port_info & 0x700) >> 8]
			[(port_info & 0xFF) >>
			 rte_octeontx_chan_mask[(port_info & 0x700) >> 8]];
...
ssows_get_work(struct ssows *ws, struct rte_event *ev, const uint16_t flag)
{
	...
	if (get_work1 && ev->event_type == RTE_EVENT_TYPE_ETHDEV) {
		ev->mbuf = ssovf_octeontx_wqe_to_pkt(get_work1,
				(ev->event >> 16) & 0xFFF, flag, ws->lookup_mem);
