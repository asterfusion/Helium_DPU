...
otx2_cpt_iq_enable(const struct rte_cryptodev *dev,
		   struct otx2_cpt_qp *qp, uint8_t grp_mask, uint8_t pri,
		   uint32_t size_div40)
{
	...
	/* Set sso_pf_func to allow CPT LF to submit to the specified SSO PF*/
	ret = otx2_cpt_af_reg_read(dev, OTX2_CPT_AF_LF_CTL2(qp->id),
				   &af_lf_ctl2.u);
	if (ret)
		return ret;

	sso_pf_func = otx2_sso_pf_func_get();
	af_lf_ctl2.s.sso_pf_func = sso_pf_func;
	ret = otx2_cpt_af_reg_write(dev, OTX2_CPT_AF_LF_CTL2(qp->id),
				    af_lf_ctl2.u);
	if (ret)
		return ret;

	qp->sso_pf_func = sso_pf_func;
