...
#define SDP_PF_DBDF_DOMAIN      2
#define SDP_PF_DBDF_BUS         15
#define SDP_PF_DBDF_DEVICE      0
#define SDP_PF_DBDF_FUNCTION    0
#define SDP_SYSFS_ENTRY_GROUP "sdp_ring_attr"
#define SDP_SYSFS_VF0_RING_ENTRY "sdp_vf0_rings"
#define SDP_SYSFS_VFX_RING_ENTRY "sdp_vfx_rings"
...
nix_lf_alloc(struct otx2_eth_dev *dev, uint32_t nb_rxq, uint32_t nb_txq)
{
    ...
    req->chan_base = dev->req_chan_base;
    req->chan_cnt = dev->req_chan_cnt;
...
static int
otx2_nix_npa_bp_config(struct otx2_eth_rxq *rxq)
{
    ...
    req->aura.shift = 0xb;
    req->aura_mask.shift = ~(req->aura_mask.shift);
    req->aura.bp = 1;
    req->aura_mask.bp = ~(req->aura_mask.bp);
    req->aura.nix0_bpid = dev->fc_info.bpid[0];
    req->aura_mask.nix0_bpid = ~(req->aura_mask.nix0_bpid);

    rc = otx2_mbox_process_msg(lf->mbox, (void *)&rsp);
    if (rc)
        otx2_err("Failed to set aura(%d) backpressure", req->aura_id);
...
nix_sq_init(struct otx2_eth_txq *txq)
{
    ...
    {
        sq->sq.default_chan = dev->tx_chan_base + dev->tx_chan_offset;
        dev->tx_chan_offset++;
        if (dev->tx_chan_offset == dev->tx_chan_cnt)
            dev->tx_chan_offset = 0;
    }
    else
    {
        sq->sq.default_chan = dev->tx_chan_base;
	...
	rc = otx2_mbox_process(mbox);
    if (!rc && otx2_eth_dev_is_sdp(pci_dev)) {
        /* Make NIX Tx honour SDP BP, if asserted */
        ret = otx2_nix_tm_honour_bp(dev, txq->sq, sq->sq.default_chan -
                NIX_CHAN_SDP_CH_START);
        if (ret)
            otx2_err("SDP BP response setting failed\n");
    }
	if (rc < 0)
		return rc;
...
static int otx2_nix_rx_metadata_negotiate(struct rte_eth_dev *dev, uint64_t *feature)
{
    if (dev == NULL || feature == NULL)
    {
        return 1;
    }
    return 0;
...
otx2_nix_configure(struct rte_eth_dev *eth_dev)
{
	...
	if (!otx2_dev_is_sdp(dev) && otx2_dev_is_Ax(dev) &&
...
static const struct eth_dev_ops otx2_eth_dev_ops = {
    ...
    .rx_metadata_negotiate    = otx2_nix_rx_metadata_negotiate,
...
static inline int otx2_get_sdp_sysfs_entry(uint8_t *val,
                       const char *entry)
{
    char f[255] = { 0 }, buf[255];
    int res, fd;

    snprintf(f, sizeof(f), "%s/" PCI_PRI_FMT "/%s/%s",
         rte_pci_get_sysfs_path(), SDP_PF_DBDF_DOMAIN,
         SDP_PF_DBDF_BUS, SDP_PF_DBDF_DEVICE & 0x7,
         SDP_PF_DBDF_FUNCTION & 0x7, SDP_SYSFS_ENTRY_GROUP,
         entry);
    fd = open(f, O_RDONLY);
    if (fd < 0) {
        otx2_err("%s: open failed.", f);
        return -EACCES;
    }
    res = read(fd, buf, sizeof(buf)-1);
    close(fd);
    if (res < 0) {
        otx2_err("%s: read failed.", f);
        return -EACCES;
    }
    buf[res] = '\0';
    *val = atoi(buf);
...
otx2_eth_dev_init(struct rte_eth_dev *eth_dev)
{
	...
	struct rte_pci_device *pci_dev;
	int rc, max_entries;
    struct otx2_dev *bdev;

    bdev = (struct otx2_dev *)dev;
	...
	if (otx2_eth_dev_is_sdp(pci_dev))
    {
        if (otx2_dev_is_pf(bdev))
        {
            otx2_err("SDP PF shouldn't be attached\n");
        ...
        else 
        {
            ...
            if (!bdev->vf) 
            {
                if (otx2_get_sdp_sysfs_entry(&rcnt, SDP_SYSFS_VF0_RING_ENTRY))
                {
                    goto otx2_dev_uninit;
                }
                dev->req_chan_base = NIX_CHAN_SDP_CH_START;
                dev->req_chan_cnt = rcnt;
            } 
            else
            {
                if (otx2_get_sdp_sysfs_entry(&vf_srn, SDP_SYSFS_VF0_RING_ENTRY))
                {
                    goto otx2_dev_uninit;
                }
                if (otx2_get_sdp_sysfs_entry(&rcnt, SDP_SYSFS_VFX_RING_ENTRY))
                {
                    goto otx2_dev_uninit;
                }
                dev->req_chan_base = NIX_CHAN_SDP_CH_START + rte_align32pow2(vf_srn) + ((bdev->vf - 1) * rcnt);
                dev->req_chan_cnt = rcnt;
            }
        }
        dev->sdp_link = true;
        otx2_nix_link_update(eth_dev, 0);
    }
