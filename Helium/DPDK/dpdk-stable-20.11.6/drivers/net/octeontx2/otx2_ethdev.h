...
#define NIX_MAX_SOFT_FRS        65509
#define NIX_MAX_SDP_FRS \
    (NIX_MAX_SOFT_FRS + RTE_ETHER_CRC_LEN - NIX_MAX_VTAG_ACT_SIZE)
...
#define OTX2_SDP_MAX_FRS        65500   //should be same with OTX_EP_MAX_PKT_SZ
...
struct otx2_eth_dev {
	...
	uint16_t sqb_size;
    uint16_t req_chan_base;
	uint16_t rx_chan_base;
	uint16_t tx_chan_base;
    uint8_t req_chan_cnt;
	uint8_t rx_chan_cnt;
	uint8_t tx_chan_cnt;
    uint8_t tx_chan_offset;
