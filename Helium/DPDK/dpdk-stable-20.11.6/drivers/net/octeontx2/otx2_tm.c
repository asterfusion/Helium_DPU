...
int otx2_nix_tm_honour_bp(struct otx2_eth_dev *dev, uint16_t sq,
			   uint16_t chan)
{
	struct otx2_nix_tm_node *tm_node;
	struct otx2_mbox *mbox = dev->mbox;
	...
	/* Cureently only SDP BP is reponded */
	if (otx2_dev_is_sdp(dev)) {
		tm_node = nix_tm_node_search(dev, sq, false);
		if (!tm_node) {
			otx2_err("Unable to find node\n");
			return -EINVAL;
		}
		/* Get the TL4 node */
		tm_node = tm_node->parent->parent;
		if (tm_node->hw_id == UINT32_MAX) {
			otx2_err("Unable to find TL4 node\n");
			return -EINVAL;
		}
		req = otx2_mbox_alloc_msg_nix_txschq_cfg(mbox);
		req->lvl = tm_node->hw_lvl;
		...
		req->reg[0] = NIX_AF_TL4X_SDP_LINK_CFG(tm_node->hw_id);
		req->regval[0] = BIT_ULL(13) | chan;
		req->regval_mask[0] = ~(BIT_ULL(13) | chan);
