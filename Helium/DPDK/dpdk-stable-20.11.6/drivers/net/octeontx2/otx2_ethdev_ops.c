...
otx2_nix_mtu_set(struct rte_eth_dev *eth_dev, uint16_t mtu)
{
    ...
    if(frame_size < NIX_MIN_FRS)
    {
        otx2_err("set frame_size %d smaller than min %d, change to %d\n", frame_size, NIX_MIN_FRS, NIX_MIN_FRS);
        frame_size = NIX_MIN_FRS;
        mtu_tmp = NIX_MIN_FRS - NIX_L2_OVERHEAD;
        eth_dev->data->mtu = mtu_tmp;
        if (dev->configured && otx2_ethdev_is_ptp_en(dev))
        {
            eth_dev->data->mtu -= NIX_TIMESYNC_RX_OFFSET;
        }

        if (otx2_dev_is_sdp(dev))
        {
            eth_dev->data->mtu -= CVM_RAW_FRONT_SIZE;
        }
    }

    if (otx2_dev_is_sdp(dev) && (frame_size > NIX_MAX_SDP_FRS))
    {
        otx2_err("set frame_size %d bigger than max %d, change to %d\n", frame_size, NIX_MAX_SDP_FRS, NIX_MAX_SDP_FRS);
        frame_size = NIX_MAX_SDP_FRS;
        mtu_tmp = NIX_MAX_SDP_FRS - NIX_L2_OVERHEAD;
        eth_dev->data->mtu = mtu_tmp;
        if (dev->configured && otx2_ethdev_is_ptp_en(dev))
        {
            eth_dev->data->mtu -= NIX_TIMESYNC_RX_OFFSET;
    ...
    else if (!otx2_dev_is_sdp(dev) && (frame_size > NIX_MAX_FRS))
    {
        otx2_err("set frame_size %d bigger than max %d, change to %d\n", frame_size, NIX_MAX_FRS, NIX_MAX_FRS);
        frame_size = NIX_MAX_FRS;
        mtu_tmp = NIX_MAX_FRS - NIX_L2_OVERHEAD;
        eth_dev->data->mtu = mtu_tmp;
        if (dev->configured && otx2_ethdev_is_ptp_en(dev))
        {
            eth_dev->data->mtu -= NIX_TIMESYNC_RX_OFFSET;
