...
otx_ep_setup_iq_regs(struct otx_ep_device *otx_ep, uint32_t iq_no)
{
	...
	otx_ep_write64(iq->base_addr_dma, otx_ep->hw_addr, OTX_EP_R_IN_INSTR_BADDR(iq_no));
	otx_ep_write64(iq->nb_desc, otx_ep->hw_addr, OTX_EP_R_IN_INSTR_RSIZE(iq_no));
	 ...
	 * to raise
	 */
	//reg_val = rte_read64(otx_ep->hw_addr + OTX_EP_R_IN_INT_LEVELS(iq_no));
	reg_val = 0xffffffff;
...
otx_ep_setup_oq_regs(struct otx_ep_device *otx_ep, uint32_t oq_no)
{
	...
	otx_ep_write64(droq->desc_ring_dma, otx_ep->hw_addr, OTX_EP_R_OUT_SLIST_BADDR(oq_no));
	otx_ep_write64(droq->nb_desc, otx_ep->hw_addr, OTX_EP_R_OUT_SLIST_RSIZE(oq_no));
	...
	//reg_val = rte_read64(otx_ep->hw_addr + OTX_EP_R_OUT_INT_LEVELS(oq_no));
	otx_ep_write64(0x3fffffffffffffULL, otx_ep->hw_addr,  OTX_EP_R_OUT_INT_LEVELS(oq_no));

	/* Clear PKT_CNT register */
	//otx_ep_write64(0xFFFFFFFFF, (uint8_t *)otx_ep->hw_addr, OTX_EP_R_OUT_PKT_CNT(oq_no));
...
otx_ep_enable_iq(struct otx_ep_device *otx_ep, uint32_t q_no)
{
	 ...
	 */
	otx_ep_write64(0xFFFFFFFF, otx_ep->hw_addr, OTX_EP_R_IN_INSTR_DBELL(q_no));
...
otx_ep_enable_oq(struct otx_ep_device *otx_ep, uint32_t q_no)
{
	 ...
	 * guest reboot. IQ reset does not clear the doorbells.
	 */
	otx_ep_write64(0xFFFFFFFF, otx_ep->hw_addr, OTX_EP_R_OUT_SLIST_DBELL(q_no));
	while (((rte_read64(otx_ep->hw_addr +
				   ...
				   uint16_t q_no __rte_unused)
{
	...
	out_cnts.d64 = 0;
	out_cnts.s.resend = 1;
	otx_ep_write64(out_cnts.d64, otx_epvf->hw_addr, OTX_EP_R_OUT_CNTS(q_no));
