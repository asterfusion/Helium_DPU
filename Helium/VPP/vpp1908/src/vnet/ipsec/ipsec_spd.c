			 ...
			 int is_add)
{
  ...
  vnet_feature_enable_disable ("ip4-unicast", "ipsec-worker-handoff",
			       sw_if_index, is_add, 0, 0);
  
  vnet_feature_enable_disable ("ip6-unicast", "ipsec6-worker-handoff",
            sw_if_index, is_add, 0, 0);

  vlib_node_t *node = vlib_get_node_by_name (vm, (u8 *) "ipsec4-input-feature");

  if(im->fq_input_index == ~0)
      im->fq_input_index = vlib_frame_queue_main_init (node->index, 0);

  vlib_node_t *node6 = vlib_get_node_by_name (vm, (u8 *) "ipsec6-input-feature");

  if(im->fq_input6_index == ~0)
      im->fq_input6_index = vlib_frame_queue_main_init (node6->index, 0);
  return 0;
}
