...
dpdk_process_subseq_segs (vlib_buffer_t * b,struct rte_mbuf *mb, dpdk_per_thread_data_t * ptd)
{
  u8 nb_seg = 1;
  ...
  b->flags |= VLIB_BUFFER_DPDK_RTE_CHAIN_VALID;
  b->total_length_not_including_first_buffer = 0;
  while (nb_seg < mb->nb_segs)
    {
      ASSERT (mb_seg != 0);
      mb_seg->buf_addr = (void *)((u8 *)mb_seg+sizeof(struct rte_mbuf));
      mb_seg->buf_physaddr = (u64)mb_seg->buf_addr;
      mb_seg->pkt_len = 0;
      mb_seg->buf_len = 2432;
      b->total_length_not_including_first_buffer += mb_seg->data_len;
      if(nb_seg ==  (mb->nb_segs-1))
        mb_seg->next = NULL;
      mb_seg = mb_seg->next;
      nb_seg++;
    }
    // clib_warning ("rx total: %d first_len : %d  left_len %d diff addr: %d off: %d \n", mb->pkt_len,mb->data_len,b->total_length_not_including_first_buffer,(u8*)mb->buf_addr-(u8*)mb,mb->data_off);

  ptd->n_buffers_to_free += nb_seg - 1;
  ...
  while (n_left >= 8)
    {
      ...
      mb[0]->buf_addr = (void *)((u8 *)mb[0]+sizeof(struct rte_mbuf)+VLIB_BUFFER_HDR_SIZE);
      mb[1]->buf_addr = (void *)((u8 *)mb[1]+sizeof(struct rte_mbuf)+VLIB_BUFFER_HDR_SIZE);
      mb[2]->buf_addr = (void *)((u8 *)mb[2]+sizeof(struct rte_mbuf)+VLIB_BUFFER_HDR_SIZE);
      mb[3]->buf_addr = (void *)((u8 *)mb[3]+sizeof(struct rte_mbuf)+VLIB_BUFFER_HDR_SIZE);
      mb[0]->buf_physaddr = (u64) mb[0]->buf_addr;
      mb[1]->buf_physaddr = (u64) mb[1]->buf_addr;
      mb[2]->buf_physaddr = (u64) mb[2]->buf_addr;
      mb[3]->buf_physaddr = (u64) mb[3]->buf_addr;
      mb[0]->buf_len = 2176;
      mb[1]->buf_len = 2176;
      mb[2]->buf_len = 2176;
      mb[3]->buf_len = 2176;
      ...
      if (maybe_multiseg)
	{
	  n_bytes += dpdk_process_subseq_segs (b[0], mb[0], ptd);
	  n_bytes += dpdk_process_subseq_segs (b[1], mb[1], ptd);
	  n_bytes += dpdk_process_subseq_segs (b[2], mb[2], ptd);
	  n_bytes += dpdk_process_subseq_segs (b[3], mb[3], ptd);
	}
				...
				vlib_frame_t * f)
{
  ...
  if (ptd->n_buffers_to_free >= OTX2_MEMPOOL_REFILL_DEPLETE_COUNT)
    {
      ptd->n_buffers_to_free -=
	otx2_mempool_refill (vm, ptd->buffer_pool_index,
			     OTX2_MEMPOOL_REFILL_DEPLETE_COUNT,
