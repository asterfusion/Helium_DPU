...
dpdk_validate_rte_mbuf (vlib_main_t * vm, vlib_buffer_t * b,
			int maybe_multiseg , dpdk_per_thread_data_t *ptd)
{
  ...
  mb->pkt_len = mb->nb_segs>1 ? b->total_length_not_including_first_buffer + b->current_length : b->current_length;
  mb->data_off = VLIB_BUFFER_PRE_DATA_SIZE + b->current_data;
  // clib_warning ("tx total: %d first_len : %d  left_len %d diff: %d offset %d\n",mb->pkt_len,mb->data_len,b->total_length_not_including_first_buffer, (u8*)mb->buf_addr- (u8*)mb,mb->data_off);
  ...
  while (maybe_multiseg && (b->flags & VLIB_BUFFER_NEXT_PRESENT))
	...
	rte_pktmbuf_reset (mb);
      ...
      {
        clib_error("multiseg clone is not supported yet\n");
      }
    }
  */
  ptd->n_buffers_to_free -= mb->nb_segs;
					     ...
					     vlib_frame_t * f)
{
  ...
  while (n_left >= 8)
	...
	{
	  dpdk_validate_rte_mbuf (vm, b[0], 1  , ptd);
	  dpdk_validate_rte_mbuf (vm, b[1], 1  , ptd);
	  dpdk_validate_rte_mbuf (vm, b[2], 1  , ptd);
	  dpdk_validate_rte_mbuf (vm, b[3], 1  , ptd);
	}
      else
	{
	  dpdk_validate_rte_mbuf (vm, b[0], 0  , ptd);
	  dpdk_validate_rte_mbuf (vm, b[1], 0  , ptd);
	  dpdk_validate_rte_mbuf (vm, b[2], 0  , ptd);
	  dpdk_validate_rte_mbuf (vm, b[3], 0  , ptd);
	}
			     ...
			     struct vnet_sw_interface_t *st, int is_add)
{
  ...
  if ((xd->pmd != VNET_DPDK_PMD_IXGBEVF) && (xd->pmd != VNET_DPDK_PMD_I40EVF)
     && (xd->pmd != VNET_DPDK_PMD_OCTEONTX2)
   )
    goto done;
