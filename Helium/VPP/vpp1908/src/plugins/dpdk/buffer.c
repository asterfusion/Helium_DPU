...
#define OCTX2_MEMPOOL_NAME "octeontx2_npa"
...
dpdk_buffer_pool_init (vlib_main_t * vm, vlib_buffer_pool_t * bp)
{
  ...
  /* normal mempool */
  name = format (name, "%s pool %u%c", OCTX2_MEMPOOL_NAME, bp->index, 0);
#ifdef NO_CACHE
  ...
  rte_mempool_set_ops_byname (mp, OCTX2_MEMPOOL_NAME, NULL);
  ...
  if (rte_eth_dev_count_avail ())
      ...
      if (mp->populated_size > bp->n_buffers)
	{
      ...
      /* Should use number of buffers populated by DPDK
       * instead of original buffers created by VPP
       * */
      ...
      for (i = 0; i < bp->n_buffers; i++)
	{
      ...
      nmbufs = dpdk_config_main.num_mbufs;
      if (vec_len (bp->buffers) < dpdk_config_main.num_mbufs)
	{
