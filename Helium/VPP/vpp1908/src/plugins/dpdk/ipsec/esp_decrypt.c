...
//   struct rte_mbuf *m,*mb0t =NULL;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (sa_index0 != last_sa_index)
		...
		{
		  clib_warning ("failed to get crypto session %s", error->what );
		  if (is_ip6)
		    vlib_node_increment_counter (vm,
						 dpdk_esp6_decrypt_node.index,
	  ...
	  /*
	    not_including_first_buffer = 0;
		m = mb0;
		mb0t = m;
		cd =((u8 *) m->buf_addr+m->data_off);
		clib_warning ("packet_len: %d seglen: %d\n", m->pkt_len,m->data_len);
		for(int i=0;i<(m->data_len/16+2);i++)
			clib_warning("de be[%3d - %3d]  %02x %02x %02x %02x %02x %02x %02x %02x -- %02x %02x %02x %02x %02x %02x %02x %02x",i*16+1,i*16+16,
		...
		while (m->next != NULL)
		{
			m = m->next;
			cd =((u8 *) m->buf_addr+m->data_off);
			not_including_first_buffer += m->data_len;

			clib_warning ("packet_len: %d seglen: %d\n addr: %d off:%d", m->pkt_len,m->data_len,m->buf_addr,m->data_off);
			for(int i=0;i<(m->data_len/16+2);i++)
					clib_warning("de be[%3d - %3d]  %02x %02x %02x %02x %02x %02x %02x %02x -- %02x %02x %02x %02x %02x %02x %02x %02x",i*16+1,i*16+16,
					cd[i*16+0],cd[i*16+1],cd[i*16+2],cd[i*16+3],cd[i*16+4],cd[i*16+5],cd[i*16+6],cd[i*16+7],cd[i*16+8],cd[i*16+9],cd[i*16+10],cd[i*16+11],cd[i*16+12],cd[i*16+13],cd[i*16+14],cd[i*16+15]);
		}
	  ...
	  if(mb0->nb_segs >1 )
	  {
		mb0->pkt_len = b0->current_length + b0->total_length_not_including_first_buffer;
	  }
	  else
	  {
	  ...
	  u16 payload_len =
	    b0->current_length - sizeof (esp_header_t) - iv_size - trunc_size + b0->total_length_not_including_first_buffer;
      ...
      struct rte_mbuf *last = rte_pktmbuf_lastseg(mb0);
	  u16 last_len = 0;
      if(PREDICT_FALSE(last->data_len < trunc_size))
      {
	  	last_len = last->data_len;
      	/*digest is in the last but one*/
		u8 nb_seg = 1;
  		struct rte_mbuf *mb_seg = 0;
		last = mb_seg = mb0;
		while (nb_seg < mb0->nb_segs)
		{
			mb_seg = mb_seg->next;
			if(mb_seg->next){
				last = mb_seg->next;
	  ...
	  u8 *digest = last->buf_addr + last->data_off + last->data_len - trunc_size + last_len;
  ...
  if (is_ip6)
	...
	if(m){
		...
		clib_warning ("packet_len: %d seglen: %d\n", m->pkt_len,m->data_len);
		for(int i=0;i<(m->data_len/16+2);i++)
				clib_warning("de af[%4d - %4d]  %02x %02x %02x %02x %02x %02x %02x %02x -- %02x %02x %02x %02x %02x %02x %02x %02x",i*16+1,i*16+16,
				cd[i*16+0],cd[i*16+1],cd[i*16+2],cd[i*16+3],cd[i*16+4],cd[i*16+5],cd[i*16+6],cd[i*16+7],cd[i*16+8],cd[i*16+9],cd[i*16+10],cd[i*16+11],cd[i*16+12],cd[i*16+13],cd[i*16+14],cd[i*16+15]);
		while (m->next != NULL){
			m = m->next;
			cd =((u8 *) m->buf_addr+m->data_off);
			      ...
			      vlib_frame_t * from_frame, int is_ip6)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  struct rte_mbuf *last = rte_pktmbuf_lastseg(mb0);
	  u16 last_len = 0;
	  if(PREDICT_FALSE(last->data_len < (trunc_size +2)))
	  {
      	/*digest is in the last but one*/
		last_len = last->data_len;
		u8 nb_seg = 1;
	  ...
	  f0 = last->buf_addr+last->data_off+last->data_len - trunc_size - 2 + last_len;// (esp_footer_t *) (vlib_buffer_get_tail (b0) - trunc_size - 2);
	  if((b0->flags & VLIB_BUFFER_DPDK_RTE_CHAIN_VALID) == 0)
	  	b0->current_length -= (f0->pad_length + trunc_size + 2);
	  else
	  {
		b0->total_length_not_including_first_buffer -= (f0->pad_length + trunc_size + 2);
		last->data_len -= (f0->pad_length + trunc_size + 2);
	  }
#if 0
	  /* check padding */
