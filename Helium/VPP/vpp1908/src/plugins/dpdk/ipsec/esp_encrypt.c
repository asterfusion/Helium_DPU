...
//   struct rte_mbuf *m,*mb0t =NULL;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  b0->flags |= VLIB_BUFFER_IPSEC_HADNOFF_DISABLE;
	  /* ih0/ih6_0 */
	  ...
	  if(mb0->nb_segs > 1)
	  {
		orig_sz = b0->current_length + b0->total_length_not_including_first_buffer;
	  }
	  else
	  {
	  ...
	  u8 *padding = NULL;
	  padding = (uint8_t *)rte_pktmbuf_append(mb0, pad_bytes + 2 + trunc_size);
	  if(padding)
	  {
		if(mb0->nb_segs > 1)
		{
			b0->total_length_not_including_first_buffer += pad_bytes + 2 + trunc_size;
		}
		else{
	  ...
	  {
        u32 tmp_bi[1];
		if (vlib_buffer_alloc_from_pool (vm, tmp_bi,1, b0->buffer_pool_index)== 1)
		{
			struct rte_mbuf *m_last = rte_pktmbuf_lastseg(mb0);
      		vlib_buffer_t *append = vlib_get_buffer (vm, tmp_bi[0]);
	  		struct rte_mbuf *append_mb = rte_mbuf_from_vlib_buffer (append);
			dpdk_per_thread_data_t *ptd = vec_elt_at_index (dpdk_main.per_thread_data,
									vm->thread_index);

			append_mb->buf_addr = (void *)((u8 *)append_mb+sizeof(struct rte_mbuf))+VLIB_BUFFER_HDR_SIZE;
			append_mb->buf_physaddr = (u64)append_mb->buf_addr;
			append_mb->pkt_len = 0;
			append_mb->data_off = 128;
			append_mb->buf_len = 2176;
			vlib_buffer_copy_template (append, &ptd->buffer_template);

			append_mb->data_len = pad_bytes + 2 + trunc_size;
			b0->total_length_not_including_first_buffer += append_mb->data_len;
			padding = append_mb->buf_addr + append_mb->data_off;

			mb0->nb_segs += 1;
			m_last->next = append_mb;
			append_mb->next = NULL;
		}
		else
		{
	  ...
	  if (oh6_0)
	    ...
	    {
	      oh0->ip4.length =
		clib_host_to_net_u16 (b0->current_length+ b0->total_length_not_including_first_buffer - rewrite_len);
	  ...
	  mb0->pkt_len = mb0->nb_segs==1 ? mb0->data_len: mb0->data_len+ b0->total_length_not_including_first_buffer;
	  mb0->data_off = ((void *) esp0) - mb0->buf_addr;

	  u32 cipher_off, cipher_len, auth_len = 0;
	  u32 *aad = NULL;

	  u8 *digest = padding + pad_bytes+2;// vlib_buffer_get_tail (b0) - trunc_size;
	  u64 digest_paddr = (u64)(digest);
	  ...
	  if (is_aead)
	    ...
	    {
	      auth_len = mb0->pkt_len - trunc_size;
		...
		mb0->packet_type &= ~RTE_PTYPE_L4_UDP;
		mb0->packet_type &= ~RTE_PTYPE_L4_TCP;
		b0->flags &= ~VNET_BUFFER_F_OFFLOAD_UDP_CKSUM;
		b0->flags &= ~VNET_BUFFER_F_OFFLOAD_TCP_CKSUM;
		/*
		mb0t = m = mb0;
		cd =((u8 *) m);
		clib_warning ("packet_len: %d seglen: %d\n", m->pkt_len,m->data_len);
			for(int i=0;i<((2048+384+128)/16);i++)
				clib_warning("en set[%4d - %4d]  %02x %02x %02x %02x %02x %02x %02x %02x -- %02x %02x %02x %02x %02x %02x %02x %02x",i*16+1,i*16+16,
				cd[i*16+0],cd[i*16+1],cd[i*16+2],cd[i*16+3],cd[i*16+4],cd[i*16+5],cd[i*16+6],cd[i*16+7],cd[i*16+8],cd[i*16+9],cd[i*16+10],cd[i*16+11],cd[i*16+12],cd[i*16+13],cd[i*16+14],cd[i*16+15]);
		while (m->next != NULL){
			m = m->next;
			cd =((u8 *) m);
			clib_warning ("packet_len: %d seglen: %d\n", m->pkt_len,m->data_len);
	  ...
	  crypto_op_setup (is_aead, mb0, op, session, cipher_off, cipher_len,
			   0, auth_len, (u8 *) aad, digest+1, digest_paddr);
  ...
  if (is_ip6)
	...
	if(m){
		...
		clib_warning ("packet_len: %d seglen: %d\n", m->pkt_len,m->data_len);
		for(int i=0;i<((2048+384+128)/16);i++)
			clib_warning("en af[%4d - %4d]  %02x %02x %02x %02x %02x %02x %02x %02x -- %02x %02x %02x %02x %02x %02x %02x %02x",i*16+1,i*16+16,
			cd[i*16+0],cd[i*16+1],cd[i*16+2],cd[i*16+3],cd[i*16+4],cd[i*16+5],cd[i*16+6],cd[i*16+7],cd[i*16+8],cd[i*16+9],cd[i*16+10],cd[i*16+11],cd[i*16+12],cd[i*16+13],cd[i*16+14],cd[i*16+15]);
		while (m->next != NULL)
		{
			m = m->next;
