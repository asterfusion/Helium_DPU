...
#define AES_GMAC_TYPE RTE_CRYPTO_SYM_XFORM_AUTH
#define AES_GMAC_ALG RTE_CRYPTO_AUTH_AES_GMAC

  a = &dcm->auth_algs[IPSEC_INTEG_ALG_AES_128_GMAC];
  a->type = AES_GMAC_TYPE;
  a->alg = AES_GMAC_ALG;
  a->key_len = 16;
  a->iv_len = 12;
  a->trunc_size = 16;

  a = &dcm->auth_algs[IPSEC_INTEG_ALG_AES_192_GMAC];
  a->type = AES_GMAC_TYPE;
  a->alg = AES_GMAC_ALG;
  a->key_len = 24;
  a->iv_len = 12;
  ...
  a = &dcm->auth_algs[IPSEC_INTEG_ALG_AES_256_GMAC];
  a->type = AES_GMAC_TYPE;
  a->alg = AES_GMAC_ALG;
  a->key_len = 32;
  a->iv_len = 12;
  ...
  a = &dcm->auth_algs[IPSEC_INTEG_ALG_AES_XCBC_MAC];
  a->type = RTE_CRYPTO_SYM_XFORM_AUTH;
		       ...
		       ipsec_sa_t * sa, u8 is_outbound)
{
  ...
  if(xform->auth.algo == RTE_CRYPTO_AUTH_AES_GMAC)
  {
    ...
    (sizeof(struct rte_crypto_op) + sizeof(struct rte_crypto_sym_op) + 2 * sizeof(struct rte_crypto_sym_xform));
			   ...
			   u32 n_mains)
{
  ...
  for (; cap->op != RTE_CRYPTO_OP_TYPE_UNDEFINED; cap++)
	...
	case RTE_CRYPTO_SYM_XFORM_AUTH:
      ...
      if(cap->sym.auth.algo == RTE_CRYPTO_AUTH_AES_GMAC)
      {
        alg = &dcm->auth_algs[IPSEC_INTEG_ALG_AES_192_GMAC];
        dev->auth_support[auth_alg_index (alg)] = 1;
	      alg->resources += vec_len (dev->free_resources);
	      /* At least enough resources to support one algo */
	      dcm->enabled |= (alg->resources >= n_mains);
        
        alg = &dcm->auth_algs[IPSEC_INTEG_ALG_AES_256_GMAC];
        dev->auth_support[auth_alg_index (alg)] = 1;
	      alg->resources += vec_len (dev->free_resources);
	      /* At least enough resources to support one algo */
	      dcm->enabled |= (alg->resources >= n_mains);
