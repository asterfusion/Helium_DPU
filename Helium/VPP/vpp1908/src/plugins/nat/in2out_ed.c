...
nat44_i2o_ed_is_idle_session_cb (clib_bihash_kv_16_8_t * kv, void *arg)
{
  ...
  if (ctx->now >= sess_timeout_time)
	...
	{
	  for (i = 0; i < vec_len (s->twice_nat_pool_address); i++)
	    {
	      ...
	      tcp_header_t * tcp)
{
  ...
  if (proto == SNAT_PROTOCOL_TCP)
      ...
      if (!tcp_is_init (tcp))
	{
      ...
      if (!is_sm)
	snat_free_outside_address_and_port (pool_address,
					    thread_index, &key1);
	  return NAT_NEXT_DROP;
	}
    }
		      ...
		      u8 * p_dont_translate, void *d, void *e)
{
  ...
  if (clib_bihash_search_16_8 (&tsm->in2out_ed, &kv, &value))
      ...
      if (PREDICT_FALSE (icmp_is_error_message (icmp)))
	{
      ...
      next = slow_path_ed (sm, b, sw_if_index, rx_fib_index, &kv, &s, node, next,
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (clib_bihash_search_16_8 (&tsm->in2out_ed, &kv0, &value0))
	      ...
	      if (is_slow_path)
		{
		  ...
		  next0 =
		    slow_path_ed (sm, b0, sw_if_index0, rx_fib_index0, &kv0, &s0, node,
	...
	trace00:
	  ...
	  if (clib_bihash_search_16_8 (&tsm->in2out_ed, &kv1, &value1))
	      ...
	      if (is_slow_path)
		{
		  ...
		  next1 =
		    slow_path_ed (sm, b1, sw_if_index1, rx_fib_index1, &kv1, &s1, node,
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (ip4_is_first_fragment (ip0)))
		  ...
		  (&per_thread_data->in2out_ed, &kv0, &value0))
		{
		  ...
		  next0 = slow_path_ed (sm, b0, sw_if_index0, rx_fib_index0, &kv0,
