...
is_hairpinning (snat_main_t * sm, u32 sw_if_index, ip4_address_t * dst_addr)
{
  snat_address_t *ap;
  clib_bihash_kv_8_8_t kv, value;
  snat_session_key_t m_key;

  snat_address_t *addresses = nat44_find_addresses_pool_by_sw_if_index(sm, sw_if_index, 0);
  /* *INDENT-OFF* */
			    ...
			    vlib_frame_t * frame, int is_ed)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (is_hairpinning (sm, sw_if_index0, &ip0->dst_address)))
	    {
