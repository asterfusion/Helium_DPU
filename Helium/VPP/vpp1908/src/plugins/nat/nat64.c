...
static u32
nat64_port_range_get_worker_index( ip4_address_t *addr, u32 proto, u16 port)
{
    nat64_main_t *nm = &nat64_main;
    snat_address_t *a;
    u32 address_index;
    ...
    {
        if (nm->addr_pool[address_index].addr.as_u32 == addr->as_u32)
            break;
    }
...
u32
nat64_get_worker_out2in (ip4_header_t * ip, u32 fib_index)
{
  nat64_main_t *nm = &nat64_main;
  snat_main_t *sm = nm->sm;
  udp_header_t *udp;
  ...
  if (PREDICT_FALSE (ip4_is_fragment (ip)))
      ...
      if (ip4_is_first_fragment (ip))
	{
      ...
      if (nm->dnop.dynamic_no_pat_mappings_cnt)
      {
          ...
          nat64_db_st_entry_t *d_nopat;
          key.addr.ip4 = ip->dst_address;
          key.port = port;
		  ...
		  (&nm->dnop.dynamic_mapping_by_no_pat, &kv, &value))
		{
          d_nopat = (nat64_db_st_entry_t *)value.value;
      ...
      case NAT_ADDR_AND_PORT_ALLOC_ALG_RANGE:
          reass->thread_index = nat64_port_range_get_worker_index(&ip->dst_address, proto, clib_net_to_host_u16 (port));
          break;
...
no_reass:
  ...
  {
  case NAT_ADDR_AND_PORT_ALLOC_ALG_RANGE:
      return nat64_port_range_get_worker_index(&ip->dst_address, proto, clib_net_to_host_u16 (port));
  case NAT_ADDR_AND_PORT_ALLOC_ALG_DEFAULT:
  case NAT_ADDR_AND_PORT_ALLOC_ALG_MAPE:
  default:
      port = clib_net_to_host_u16 (port);
...
nat64_init (vlib_main_t * vm)
{
  ...
  nm->nat64_expire_walk_interval = NAT64_EXPIRE_WALK_INTERVAL;
...
void
nat64_set_hash (u32 max_st_per_worker, u32 bib_buckets, u32 bib_memory_size, u32 st_buckets,
		u32 st_memory_size, u32 no_pat_buckets, u32 no_pat_memory_size)
{
  ...
  clib_bihash_init_24_8 (&nm->dnop.dynamic_mapping_by_no_pat, "nat64-dynamic-no-pat", 
			 nm->no_pat_buckets, nm->no_pat_memory_size);

  /* *INDENT-OFF* */
  vec_foreach (db, nm->db)
    {
      if (nat64_db_init (max_st_per_worker, db, bib_buckets, bib_memory_size, st_buckets,
...
nat64_add_del_pool_addr (u32 thread_index,
			 ip4_address_t * addr, u32 vrf_id, u8 is_add, u8 no_pat, u32 limit_ip_cnt)
{
  nat64_main_t *nm = &nat64_main;
  snat_address_t *a = 0;
...
int
nat64_add_interface_address (u32 sw_if_index, int is_add, u8 no_pat, u32 limit_ip_cnt)
{
  nat64_main_t *nm = &nat64_main;
      ...
      if (nm->auto_add_sw_if_indices[i] == sw_if_index)
	{
	  if (is_add)
	  ...
	  else
	    {
	      ...
	      if (first_int_addr)
          {
              (void) nat64_add_del_pool_addr (vlib_get_thread_index (), first_int_addr, ~0, 0, no_pat, limit_ip_cnt);
              ...
              while(ia_index != ~0)
              {
                  ...
                  (void) nat64_add_del_pool_addr (vlib_get_thread_index (), follow_int_addr, ~0, 0, no_pat, limit_ip_cnt);
                  ia_index = ia->next_this_sw_interface;                            
              }
          }
  ...
  if (first_int_addr)
  {
    (void) nat64_add_del_pool_addr (vlib_get_thread_index (), first_int_addr, ~0, 1, no_pat, limit_ip_cnt);
    ia_index = ia->next_this_sw_interface;                            
    while(ia_index != ~0)
    {
        ia = pool_elt_at_index (ip4_main->lookup_main.if_address_pool, ia_index);
        follow_int_addr = ip_interface_address_get_address (&ip4_main->lookup_main, ia);
        (void) nat64_add_del_pool_addr (vlib_get_thread_index (), follow_int_addr, ~0, 1, no_pat, limit_ip_cnt);
        ia_index = ia->next_this_sw_interface;                            
    }
  }
		      ...
		      vlib_frame_t * f)
{
  ...
  while (1)
	...
	{
	  vlib_process_wait_for_event_or_clock (vm, nm->nat64_expire_walk_interval);
