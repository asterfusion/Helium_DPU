...
void nat44_submit_or_update_session_timer(snat_main_t * sm, u32 thread_index, snat_session_t *s)
{
    snat_main_per_thread_data_t *tsm = &sm->per_thread_data[thread_index];
    u32 session_index = s - tsm->sessions;
    //update
    if (s->session_timer_handle != 0 &&
        !tw_timer_handle_is_free_1t_3w_64sl(tsm->timers_per_worker, s->session_timer_handle))
    {
        tw_timer_update_1t_3w_64sl(
                tsm->timers_per_worker,
                s->session_timer_handle,
                nat44_session_get_timeout(sm, s));
    }
    else
    {
        s->session_timer_handle = tw_timer_start_1t_3w_64sl(
                tsm->timers_per_worker,
                session_index,
                0, nat44_session_get_timeout(sm, s));
    }
}

void nat44_delete_session_timer(snat_main_t * sm, u32 thread_index, snat_session_t *s)
{
    snat_main_per_thread_data_t *tsm = &sm->per_thread_data[thread_index];
    if (s->session_timer_handle != 0 &&
        !tw_timer_handle_is_free_1t_3w_64sl(tsm->timers_per_worker, s->session_timer_handle))
    {
        tw_timer_stop_1t_3w_64sl(tsm->timers_per_worker, s->session_timer_handle);
    }
}
		       ...
		       u8 is_ha)
{
  ...
  if (is_ed_session (s))
	...
	nat_elog_warn ("out2in key del failed");
      if( s->flags & SNAT_SESSION_FLAG_NO_PAT)
      {
	clib_bihash_add_del_8_8 (&sm->dynamic_mapping_by_no_pat, &kv, 0);
  ...
  if (is_twice_nat_session (s))
    {
      ...
      snat_free_outside_address_and_port (s->twice_nat_pool_address,
					  thread_index, &key);
    }
  ...
  nat44_delete_session_timer(sm, thread_index, s);
#endif
  snat_free_outside_address_and_port (s->pool_address, thread_index,
				      &s->out2in);
}
  ...
  if (clib_bihash_search_8_8 (&tsm->user_hash, &kv, &value))
    {
      ...
      u->sessions_per_user_list_head_index = per_user_list_head_elt - tsm->list_pool;
...
snat_add_address (snat_main_t * sm, ip4_address_t * addr, u32 vrf_id,
		  u8 twice_nat, u8 no_pat, u32 limit_ip_cnt, u32 pool_index)
{
  ...
  nat44_pool_addresses_t *pool_addresses = NULL;

  if (twice_nat && !sm->endpoint_dependent)
    return VNET_API_ERROR_FEATURE_DISABLED;

  /* Check if address already exists */
  /* All IP addresses in the address pool cannot be duplicated */
  /* *INDENT-OFF* */
  if ( !twice_nat )
  {
    ...
    pool_foreach (pool_addresses, sm->addresses_pool,
    ({
      vec_foreach (ap, pool_addresses->addresses)
      {
        if (ap->addr.as_u32 == addr->as_u32)
          return VNET_API_ERROR_VALUE_EXIST;
      }
  ...
  else
  {
    ...
    pool_foreach (pool_addresses, sm->addresses_pool,
    ({
      vec_foreach (ap, pool_addresses->twice_nat_addresses)
      {
        if (ap->addr.as_u32 == addr->as_u32)
          return VNET_API_ERROR_VALUE_EXIST;
      }
  ...
  if(pool_index != ~0)
  {
    pool_addresses = pool_elt_at_index(sm->addresses_pool, pool_index);
    addresses = twice_nat ? &pool_addresses->twice_nat_addresses : &pool_addresses->addresses;
  }
  else
      addresses = twice_nat ? &sm->twice_nat_addresses : &sm->addresses;

  vec_add2 (*addresses, ap, 1);

  clib_memset(ap, 0 , sizeof(snat_address_t));

  ap->addr = *addr;
  ap->no_pat = no_pat;

  ap->limit_user_max = limit_ip_cnt;
  ap->limit_user_cnt = 0;

  ap->pool_index = pool_index;
  ...
  if (vrf_id != ~0)
    ...
    ap->fib_index = ~0;
#define _(N, i, n, s) \
  vec_validate_init_empty (ap->busy_##n##_port_used_flag, 65536, 0); \
  vec_validate_init_empty (ap->port_range_workers_##n##_ports, 65536, ~0); \
  ap->busy_##n##_ports = 0; \
...
snat_del_address (snat_main_t * sm, ip4_address_t addr, u8 delete_sm,
		  u8 twice_nat, u32 pool_index)
{
  snat_address_t *a = NULL;
  snat_session_t *ses;
...
#undef _
  ...
  clib_memset(a, 0 , sizeof(snat_address_t));

  vec_del1 (*addresses, i);
  if (twice_nat)
    return 0;
			    ...
			    u8 twice_nat, u8 no_pat, u32 limit_ip_cnt, u32 pool_index)
{
  ...
  snat_sw_if_indices_t sw_if_indices = {
      .sw_if_index = sw_if_index,
      .pool_address_index = pool_index,
  };

  snat_sw_if_indices_t *auto_add_sw_if_indices =
    twice_nat ? sm->auto_add_sw_if_indices_twice_nat : sm->auto_add_sw_if_indices;

  first_int_addr = ip4_interface_first_address (ip4_main, sw_if_index, &ia);

  for (i = 0; i < vec_len (auto_add_sw_if_indices); i++)
    {
      if (auto_add_sw_if_indices[i].sw_if_index == sw_if_index)
	{
	  if (is_del)
	    {
	      ...
	      if (first_int_addr)
		{
            (void) snat_del_address (sm, first_int_addr[0], 1, twice_nat, pool_index);
            ia_index = ia->next_this_sw_interface;                            
            while(ia_index != ~0)
            {
                ia = pool_elt_at_index (ip4_main->lookup_main.if_address_pool, ia_index);
                follow_int_addr = ip_interface_address_get_address (&ip4_main->lookup_main, ia);
                (void) snat_del_address (sm, follow_int_addr[0], 1, twice_nat, pool_index);
                ia_index = ia->next_this_sw_interface;                            
		    }
		}
	      else
  ...
  if (twice_nat)
    vec_add1 (sm->auto_add_sw_if_indices_twice_nat, sw_if_indices);
  else
    vec_add1 (sm->auto_add_sw_if_indices, sw_if_indices);

  /* If the address is already bound - or static - add it now */
  if (first_int_addr)
  {
    (void) snat_add_address (sm, first_int_addr, ~0, twice_nat, no_pat, limit_ip_cnt, pool_index);
    ia_index = ia->next_this_sw_interface;                            
    while(ia_index != ~0)
    {
        ia = pool_elt_at_index (ip4_main->lookup_main.if_address_pool, ia_index);
        follow_int_addr = ip_interface_address_get_address (&ip4_main->lookup_main, ia);
        (void) snat_add_address (sm, follow_int_addr, ~0, twice_nat, no_pat, limit_ip_cnt, pool_index);
        ia_index = ia->next_this_sw_interface;                            
    }
  }
...
u32 nat44_get_addresses_pool_index_by_sw_if_index(snat_main_t * sm,  u32 sw_if_index)
{
  snat_interface_t *i;
  pool_foreach (i, sm->interfaces,
  ({
    if (i->sw_if_index == sw_if_index)
      return i->pool_address_index;
  }));
  /* *INDENT-ON* */
  pool_foreach (i, sm->output_feature_interfaces,
  ({
...
snat_address_t* nat44_find_addresses_pool_by_sw_if_index(snat_main_t * sm,  u32 sw_if_index, int twice_nat)
{
  snat_interface_t *i;
  nat44_pool_addresses_t *pool_addresses = NULL;
  /* *INDENT-OFF* */
  ...
  ({
    if (i->sw_if_index == sw_if_index)
    {
      if (i->pool_address_index == ~0)
        return twice_nat ? sm->twice_nat_addresses : sm->addresses;
      pool_addresses = pool_elt_at_index(sm->addresses_pool, i->pool_address_index);
      return twice_nat ? pool_addresses->twice_nat_addresses : pool_addresses->addresses;
    }
  }));
  /* *INDENT-ON* */
  pool_foreach (i, sm->output_feature_interfaces,
...
u32
nat44_get_addresses_pool_index_by_name(snat_main_t * sm, u8 *pool_name)
{
  nat44_pool_addresses_t *pool_addresses = NULL;
  nat44_pool_addresses_t *check_pool_addresses = NULL;

  if (!pool_name)
      return ~0;

  pool_foreach (check_pool_addresses, sm->addresses_pool,
  ({
    if (clib_memcmp(check_pool_addresses->name,  pool_name, vec_len (pool_name)) == 0)
    {
      pool_addresses = check_pool_addresses;
      break;
    }
  }));

  if (pool_addresses)
      return pool_addresses->pool_index;
  else
      return ~0;
}

int
nat44_add_del_addresses_pool (snat_main_t * sm, u8 *pool_name, int is_add)
{
  snat_interface_t *i;
  nat44_pool_addresses_t *pool_addresses = NULL;
  nat44_pool_addresses_t *check_pool_addresses = NULL;
  ...
  if (is_add)
  {
      ...
      if(pool_elts(sm->addresses_pool) > MAX_ADDRESSES_POOL_CNT)
          return VNET_API_ERROR_INVALID_VALUE;

      pool_get (sm->addresses_pool, pool_addresses);
      clib_memset(pool_addresses, 0 , sizeof(pool_addresses));
      clib_memcpy (pool_addresses->name, pool_name, vec_len (pool_name));
      pool_addresses->pool_index = pool_addresses - sm->addresses_pool;
  }
  else
  {
      //check
      ...
      if (pool_addresses)
      {
        if (pool_elts(pool_addresses->addresses) != 0 ||
            pool_elts(pool_addresses->twice_nat_addresses) != 0)
            return VNET_API_ERROR_ADDRESS_IN_USE;
        pool_foreach (i, sm->output_feature_interfaces,
        ({
          if (i->pool_address_index == pool_addresses->pool_index)
              return VNET_API_ERROR_UNSPECIFIED;
        }));
        pool_foreach (i, sm->interfaces,
        ({
        ...
        pool_put (sm->addresses_pool, pool_addresses);
      }
      else
        return VNET_API_ERROR_NO_SUCH_ENTRY;
  }
...
#define _(N, j, n, s) \
                    case SNAT_PROTOCOL_##N: \
                      if (a->busy_##n##_port_used_flag[e_port] > 0) \
                        return VNET_API_ERROR_INVALID_VALUE; \
                      a->busy_##n##_port_used_flag[e_port] = 1; \
                      if (e_port > 1024) \
                        { \
...
#define _(N, j, n, s) \
	  ...
	  pool_foreach (pool_addresses, sm->addresses_pool,
	  ({
        for (i = 0; i < vec_len (pool_addresses->addresses); i++)
	    {
	      if (pool_addresses->addresses[i].addr.as_u32 == e_addr.as_u32)
		{
		  a = pool_addresses->addresses + i;
		  /* External port must be unused */
		  switch (proto)
		    {
		      foreach_snat_protocol
...
#define _(N, j, n, s) \
                    case SNAT_PROTOCOL_##N: \
                      a->busy_##n##_port_used_flag[e_port] = 0; \
                      if (e_port > 1024) \
                        { \
...
int
snat_interface_add_del (u32 sw_if_index, u8 is_inside, int is_del, u32 pool_index)
{
  snat_main_t *sm = &snat_main;
  snat_interface_t *i;
  ...
  nat44_pool_addresses_t *pool_address;
...
feature_set:
  ...
  i->flags = 0;
  i->pool_address_index = pool_index;
...
set_flags:
  ...
  vlib_process_signal_event (sm->vlib_main,
          sm->nat44_expire_walk_node_index,
          NAT44_CLEANER_RESCHEDULE, 0);
...
fib:
  ...
  if (pool_index != ~0)
  {
      pool_address = pool_elt_at_index(sm->addresses_pool, pool_index);
      addresses = pool_address->addresses;
  }
  else
...
int
snat_interface_add_del_output_feature (u32 sw_if_index, u8 is_inside, int is_del, u32 pool_index)
{
  snat_main_t *sm = &snat_main;
  snat_interface_t *i;
  snat_address_t *ap, *addresses;
...
snat_init (vlib_main_t * vm)
{
  ...
  sm->workers = 0;
  sm->port_per_thread = NAT_PORT_PER_THREAD;
  ...
  /* Init NAT44 Sessoin Timer */
#ifndef USER_WALK_EXPIRE_FLAG
  vec_foreach (tsm, sm->per_thread_data)
  {
      tsm->timers_per_worker = clib_mem_alloc (sizeof (TWT (tw_timer_wheel)));
      tw_timer_wheel_init_1t_3w_64sl (tsm->timers_per_worker, NULL, 1.0, NAT44_PER_WORKER_MAX_SESSION);
  }
#endif

  /* Init Session expire */
  sm->total_enabled_count = 0;
  sm->nat44_expire_walk_interval = NAT44_EXPIRE_WALK_INTERVAL;
...
#define _(N, j, n, s) \
        case SNAT_PROTOCOL_##N: \
          if (a->busy_##n##_port_used_flag[port_host_byte_order] > 0) \
            return VNET_API_ERROR_INSTANCE_IN_USE; \
          a->busy_##n##_port_used_flag[port_host_byte_order] = 1; \
          a->busy_##n##_ports_per_thread[thread_index]++; \
          clib_atomic_fetch_add(&a->busy_##n##_ports, 1); \
          return 0;
	  foreach_snat_protocol
...
int
snat_dynamic_limit_ip_translation(snat_address_t * addresses,
        snat_user_t *u,
        snat_session_key_t *out2in_k)
{
  snat_address_t *a;
  u32 address_index;
  u32 index;
  ...
  {
      if (addresses[address_index].addr.as_u32 == out2in_k->addr.as_u32)
          break;
  }
  ASSERT (address_index < vec_len (addresses));
  a = addresses + address_index;
  if(a->limit_user_max == ~0)
      return 0;

  vec_validate(a->limit_user, a->limit_user_max);
  index = vec_search(a->limit_user, u);
  if(~0 != index) 
      return 0;
  if(a->limit_user_cnt >= a->limit_user_max)
      return 1;
  clib_atomic_fetch_add(&a->limit_user_cnt, 1);
  vec_add1(a->limit_user, u);
  return 0;
}
...
u8 *
format_dynamic_mapping_by_no_pat_kvp (u8 * s, va_list * args)
{
  clib_bihash_kv_8_8_t *v = va_arg (*args, clib_bihash_kv_8_8_t *);
  snat_session_key_t k;
  ...
  s = format (s, "%U dynamic-mapping-by-nopat-index %llu",
	      format_dynamic_mapping_by_no_pat_key, &k, v->value);
...
static u32
nat_port_range_get_worker_index( ip4_address_t *addr, u32 proto, u16 port)
{
    snat_main_t *sm = &snat_main;
    snat_address_t *a = NULL;
    nat44_pool_addresses_t *pool_addresses = NULL;
    u32 address_index;
    for (address_index = 0; address_index < vec_len (sm->addresses); address_index++)
    {
        if (sm->addresses[address_index].addr.as_u32 == addr->as_u32)
        {
    ...
    pool_foreach (pool_addresses, sm->addresses_pool,
    ({
      for (address_index = 0; address_index < vec_len (pool_addresses->addresses); address_index++)
      {
        if (pool_addresses->addresses[address_index].addr.as_u32 == addr->as_u32)
        {
            a = &pool_addresses->addresses[address_index];
            goto match;
        }
      }
    }));
			   ...
			   u8 is_output)
{
  ...
  if (PREDICT_FALSE (ip4_is_fragment (ip0)))
      ...
      if (ip4_is_first_fragment (ip0))
	{
      ...
      if (sm->dynamic_no_pat_mappings_cnt)
      {
          m_key.addr = ip0->dst_address;
          m_key.port = port;
          m_key.protocol = proto;
          ...
          kv.key = m_key.as_u64;
          if (!clib_bihash_search_8_8
		  (&sm->dynamic_mapping_by_no_pat, &kv, &value))
		{
		  d_nopat = (snat_session_t *)value.value;
      ...
      case NAT_ADDR_AND_PORT_ALLOC_ALG_RANGE:
          reass->thread_index = nat_port_range_get_worker_index(&ip0->dst_address, proto, clib_net_to_host_u16 (port));
          break;
      case NAT_ADDR_AND_PORT_ALLOC_ALG_DEFAULT:
      case NAT_ADDR_AND_PORT_ALLOC_ALG_MAPE:
      default:
          reass->thread_index = sm->first_worker_index;
          reass->thread_index += sm->workers[(clib_net_to_host_u16 (port) - 1024) / sm->port_per_thread];
          break;
      }
	  return reass->thread_index;
	}
...
no_reass:
  ...
  {
  case NAT_ADDR_AND_PORT_ALLOC_ALG_RANGE:
      next_worker_index = nat_port_range_get_worker_index(&ip0->dst_address, proto, clib_net_to_host_u16 (port));
      break;
  case NAT_ADDR_AND_PORT_ALLOC_ALG_DEFAULT:
  case NAT_ADDR_AND_PORT_ALLOC_ALG_MAPE:
  default:
      next_worker_index = sm->first_worker_index;
      next_worker_index += sm->workers[(clib_net_to_host_u16 (port) - 1024) / sm->port_per_thread];
      break;
  }
  return next_worker_index;
}
...
nat_ha_sadd_cb (ip4_address_t * in_addr, u16 in_port,
		...
		u32 fib_index, u32 pool_index, u16 flags, u32 thread_index)
{
  snat_main_t *sm = &snat_main;
  snat_session_key_t key;
  snat_user_t *u;
  ...
  pool_address = pool_index != ~0 ? 
           pool_elt_at_index(sm->addresses_pool, pool_index) : NULL;
  ...
  if (!(flags & SNAT_SESSION_FLAG_STATIC_MAPPING))
    {
	  ...
	  (pool_address ? pool_address->addresses : sm->addresses, thread_index, &key))
	return;
    }
  ...
  s->pool_address = pool_address ? pool_address->addresses : sm->addresses;
		   ...
		   u32 fib_index, u32 pool_index, u16 flags, u32 thread_index)
{
  ...
  if (flags & SNAT_SESSION_FLAG_TWICE_NAT)
    {
      if (nat_set_outside_address_and_port
	  (pool_address ? pool_address->twice_nat_addresses : sm->twice_nat_addresses, thread_index, &key))
	return;
    }
...
snat_config (vlib_main_t * vm, unformat_input_t * input)
{
  ...
  u32 pre_worker_max_translations = NAT44_PER_WORKER_MAX_SESSION;
  u32 translation_buckets = NAT44_TRANSLATION_BUCKETS;
  u32 translation_memory_size = NAT44_TRANSLATION_MEMORY_SIZE;
  u32 user_buckets = NAT44_USER_BUCKETS;
  u32 user_memory_size = NAT44_USER_MEMORY_SIZE;
  u32 max_translations_per_user = NAT44_MAX_TRANSLATIONS_PER_USER;
  u32 nat64_max_st_per_worker = NAT64_PER_WORKER_MAX_ST;
  u32 nat46_max_st_per_worker = NAT46_PER_WORKER_MAX_ST;
  u32 outside_vrf_id = 0;
  u32 outside_ip6_vrf_id = 0;
  u32 inside_vrf_id = 0;
  u32 static_mapping_buckets = NAT44_STATIC_MAPPING_BUCKETS;
  u32 static_mapping_memory_size = NAT44_STATIC_MAPPING_MEMORY_SIZE;
  u32 dynamic_mapping_by_no_pat_buckets = NAT44_DYNAMIC_MAPPING_BY_NO_PAT_BUCKETS;
  u32 dynamic_mapping_by_no_pat_memory_size = NAT44_DYNAMIC_MAPPING_BY_NO_PAT_MEMORY_SIZE;
  u32 nat64_bib_buckets = NAT64_BIB_BUCKETS;
  u32 nat64_bib_memory_size = NAT64_BIB_MEMORY_SIZE;
  u32 nat64_st_buckets = NAT64_ST_BUCKETS;
  u32 nat64_st_memory_size = NAT64_ST_MEMORY_SIZE;
  u32 nat64_dynamic_mapping_by_no_pat_buckets = NAT64_DYNAMIC_MAPPING_BY_NO_PAT_BUCKETS;
  u32 nat64_dynamic_mapping_by_no_pat_memory_size = NAT64_DYNAMIC_MAPPING_BY_NO_PAT_MEMORY_SIZE;
  u32 nat46_bib_buckets = NAT46_BIB_BUCKETS;
  u32 nat46_bib_memory_size = NAT46_BIB_MEMORY_SIZE;
  u32 nat46_st_buckets = NAT46_ST_BUCKETS;
  u32 nat46_st_memory_size = NAT46_ST_MEMORY_SIZE;
  u32 nat46_dynamic_mapping_by_no_pat_buckets = NAT46_DYNAMIC_MAPPING_BY_NO_PAT_BUCKETS;
  u32 nat46_dynamic_mapping_by_no_pat_memory_size = NAT46_DYNAMIC_MAPPING_BY_NO_PAT_MEMORY_SIZE;
  u32 nat46_remote_map_buckets = NAT46_REMOTE_MAPPING_BUCKETS;
  u32 nat46_remote_map_memory_size = NAT46_REMOTE_MAPPING_MEMORY_SIZE;
  ...
  while (unformat_check_input (input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (input, "max translations per user %d", &max_translations_per_user))
	;
      else if (unformat (input, "max translations per worker %d", &pre_worker_max_translations))
	;
      else if (unformat (input, "no pat mapping hash buckets %d", &dynamic_mapping_by_no_pat_buckets))
	;
      else if (unformat (input, "no pat mapping hash memory %d", &dynamic_mapping_by_no_pat_memory_size))
	;
	...
	sm->deterministic = 1;
      else if (unformat (input, "nat64 bib hash buckets %d", &nat64_bib_buckets))
	;
      else if (unformat (input, "nat64 bib hash memory %d", &nat64_bib_memory_size))
	;
      else if (unformat (input, "nat64 st hash buckets %d", &nat64_st_buckets))
	;
      else if (unformat (input, "nat64 st hash memory %d", &nat64_st_memory_size))
	;
      else if (unformat (input, "nat64 no pat mapping hash buckets %d", &nat64_dynamic_mapping_by_no_pat_buckets))
	;
      else if (unformat (input, "nat64 no pat mapping hash memory %d", &nat64_dynamic_mapping_by_no_pat_memory_size))
	;
      else if (unformat (input, "nat46 bib hash buckets %d", &nat46_bib_buckets))
	;
      else if (unformat (input, "nat46 bib hash memory %d", &nat46_bib_memory_size))
	;
      else if (unformat (input, "nat46 st hash buckets %d", &nat46_st_buckets))
	;
      else if (unformat (input, "nat46 st hash memory %d", &nat46_st_memory_size))
	;
      else if (unformat (input, "nat46 no pat mapping hash buckets %d", &nat46_dynamic_mapping_by_no_pat_buckets))
	;
      else if (unformat (input, "nat46 no pat mapping hash memory %d", &nat46_dynamic_mapping_by_no_pat_memory_size))
	;
      else if (unformat (input, "nat46 remote mapping hash buckets %d", &nat46_remote_map_buckets))
	;
      else if (unformat (input, "nat46 remote mapping hash memory %d", &nat46_remote_map_memory_size))
	;
  ...
  sm->max_translations = pre_worker_max_translations;
  ...
  nat64_set_hash (nat64_max_st_per_worker, nat64_bib_buckets, nat64_bib_memory_size, 
          nat64_st_buckets, nat64_st_memory_size, 
          nat64_dynamic_mapping_by_no_pat_buckets, nat64_dynamic_mapping_by_no_pat_memory_size);
  nat46_set_hash (nat46_max_st_per_worker, nat46_bib_buckets, nat46_bib_memory_size, 
          nat46_st_buckets, nat46_st_memory_size,
          nat46_dynamic_mapping_by_no_pat_buckets, nat46_dynamic_mapping_by_no_pat_memory_size, 
          nat46_remote_map_buckets, nat46_remote_map_memory_size);
  ...
  if (sm->deterministic)
      ...
      else
	{
      ...
      //no pat
      clib_bihash_init_8_8 (&sm->dynamic_mapping_by_no_pat, 
			    "dynamic_mapping_by_no_pat",
			    dynamic_mapping_by_no_pat_buckets,
			    dynamic_mapping_by_no_pat_memory_size);
      clib_bihash_set_kvp_format_fn_8_8 (&sm->dynamic_mapping_by_no_pat, 
					 format_dynamic_mapping_by_no_pat_kvp);
    }
				       ...
				       u32 if_address_index, u32 is_delete)
{
  ...
  for (i = 0; i < vec_len (sm->auto_add_sw_if_indices); i++)
    {
      if (sw_if_index == sm->auto_add_sw_if_indices[i].sw_if_index)
      {
        if (sm->auto_add_sw_if_indices[i].pool_address_index != ~0)
        {
            pool_addresses = pool_elt_at_index(sm->addresses_pool, sm->auto_add_sw_if_indices[i].pool_address_index);
            addresses = &pool_addresses->addresses;
        }
        else
            addresses = &sm->addresses;
        goto match;
  ...
  for (i = 0; i < vec_len (sm->auto_add_sw_if_indices_twice_nat); i++)
    {
      twice_nat = 1;
      if (sw_if_index == sm->auto_add_sw_if_indices_twice_nat[i].sw_if_index)
      {
        if (sm->auto_add_sw_if_indices_twice_nat[i].pool_address_index != ~0)
        {
            pool_addresses = pool_elt_at_index(sm->addresses_pool, sm->auto_add_sw_if_indices[i].pool_address_index);
            addresses = &pool_addresses->twice_nat_addresses;
        }
        else
...
match:
  if (!is_delete)
	...
	if ((*addresses)[j].addr.as_u32 == address->as_u32)
      ...
      (void) snat_add_address (sm, address, ~0, twice_nat, 0, ~0, pool_addresses ? pool_addresses->pool_index : ~0);
    ...
    {
      (void) snat_del_address (sm, address[0], 1, twice_nat, pool_addresses ? pool_addresses->pool_index : ~0);
      return;
    }
}
...
nat_set_alloc_addr_and_port_mape (u16 psid, u16 psid_offset, u16 psid_length)
{
  ...
  nm->alloc_addr_and_port = nat46_alloc_addr_and_port_mape;
}
...
nat_set_alloc_addr_and_port_range (u16 start_port, u16 end_port)
{
  ...
  nm->alloc_addr_and_port = nat46_alloc_addr_and_port_range;
}
...
static vlib_node_registration_t nat44_expire_walk_node;
/*
 * @brief Centralized process to drive per worker expire walk.
 */
static uword
nat44_expire_walk_fn (vlib_main_t * vm, vlib_node_runtime_t * rt, vlib_frame_t * f)
{
    snat_main_t *sm = &snat_main;
    ...
    sm->nat44_expire_walk_node_index = nat44_expire_walk_node.index;
    ...
    {
        /* Wait for Godot... */
        if (sm->total_enabled_count)
        {
            vlib_process_wait_for_event_or_clock (vm, sm->nat44_expire_walk_interval);
        ...
        for (i = 0; i < vec_len (worker_vms); i++) {
            ...
            if (worker_vm) {
                vlib_node_set_interrupt_pending (worker_vm, nat44_expire_worker_walk_node.index);
            }
        }
    }
    return 0;
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (nat44_expire_walk_node,static) = {
  .function = nat44_expire_walk_fn,
  .type = VLIB_NODE_TYPE_PROCESS,
  .name = "nat44-expire-walk",
};
/* *INDENT-ON* */
