  ...
  (vl_api_nat44_add_del_address_range_t * mp)
{
  ...
  no_pat = mp->no_pat;
  limit_ip_cnt = mp->limit_ip_cnt ? mp->limit_ip_cnt : ~0 ;
  ...
  for (i = 0; i < count; i++)
    {
      if (is_add)
	rv = snat_add_address (sm, &this_addr, vrf_id, twice_nat, no_pat, limit_ip_cnt, ~0);
      else
	rv = snat_del_address (sm, this_addr, 0, twice_nat, ~0);
  ...
  (vl_api_nat44_add_del_interface_addr_t * mp)
{
  ...
  rv = snat_add_interface_address (sm, sw_if_index, is_del,
				   mp->flags & NAT_API_IS_TWICE_NAT, no_pat, limit_ip_cnt, ~0);
					    ...
					    * mp)
{
  ...
  vec_foreach (i, sm->auto_add_sw_if_indices)
    send_nat44_interface_addr_details(i->sw_if_index, reg, mp->context, 0);
  vec_foreach (i, sm->auto_add_sw_if_indices_twice_nat)
    send_nat44_interface_addr_details(i->sw_if_index, reg, mp->context, 1);
  /* *INDENT-ON* */
}
  ...
  (vl_api_nat64_add_del_pool_addr_range_t * mp)
{
  ...
  for (i = 0; i < count; i++)
    {
      if ((rv = nat64_add_del_pool_addr (0, &this_addr, vrf_id, mp->is_add, no_pat, limit_ip_cnt)))
	goto send_reply;
  ...
  (vl_api_nat64_add_del_interface_addr_t * mp)
{
  ...
  rv = nat64_add_interface_address (sw_if_index, mp->is_add, no_pat, limit_ip_cnt);
