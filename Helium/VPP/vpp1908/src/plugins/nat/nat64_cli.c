				    ...
				    vlib_cli_command_t * cmd)
{
  ...
  for (i = 0; i < count; i++)
    {
      rv = nat64_add_del_pool_addr (0, &this_addr, vrf_id, is_add, no_pat, limit_ip_cnt);
					...
					vlib_cli_command_t * cmd)
{
  ...
  rv = nat64_add_interface_address (sw_if_index, is_add, no_pat, limit_ip_cnt);
...
static clib_error_t *
show_nat64_expire_walk_interval_command_fn (vlib_main_t * vm,
					unformat_input_t * input,
					vlib_cli_command_t * cmd)
{
    nat64_main_t *nm = &nat64_main;
    vlib_cli_output (vm, "NAT64 expire walk interval %lfs", nm->nat64_expire_walk_interval);
    return 0;
}

static clib_error_t *
nat64_set_expire_walk_interval_command_fn (vlib_main_t * vm,
					unformat_input_t * input,
					vlib_cli_command_t * cmd)
{
    nat64_main_t *nm = &nat64_main;
    ...
    nm->nat64_expire_walk_interval = second;
done:
    unformat_free (line_input);
    return error;
}
...
VLIB_CLI_COMMAND (nat64_add_interface_address_command, static) = {
    ...
    .short_help = "nat64 add interface address <interface> [no-pat] [limit-ip-cnt <cnt>] [del]",
    .function = nat64_add_interface_address_command_fn,
};

VLIB_CLI_COMMAND (nat64_set_expire_walk_interval_command, static) = {
    .path = "nat64 set expire walk interval",
    .short_help = "nat64 set expire walk interval [second]",
    .function = nat64_set_expire_walk_interval_command_fn,
};

VLIB_CLI_COMMAND (show_nat64_expire_walk_interval_command, static) = {
    .path = "show nat64 expire walk interval",
    .short_help = "show nat64 expire walk interval",
    .function = show_nat64_expire_walk_interval_command_fn,
};
/* *INDENT-ON* */
