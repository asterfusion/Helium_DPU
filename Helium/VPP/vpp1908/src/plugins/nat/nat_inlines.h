...
static_always_inline u8
icmp6_is_error_message (icmp46_header_t * icmp)
{
  switch (icmp->type)
    {
...
always_inline void
nat44_delete_dynamic_limit_ip_translation(snat_address_t * addresses,
        snat_user_t *u,
  ...
  if (u->nstaticsessions == 0 && u->nsessions == 0)
    {
        ...
        if (address_index >= vec_len (addresses))
            return ;
        ...
        if(index) 
        {
            vec_del1(a->limit_user, index);
            clib_atomic_fetch_sub(&a->limit_user_cnt, 1);
        }
    }
}
		      ...
		      u32 thread_index)
{
  ...
  snat_session_key_t out2in_k = ses->out2in;
  ...
  if (!clib_bihash_search_8_8 (&tsm->user_hash, &kv, &value))
	...
	u->nsessions--;
      ...
      nat44_delete_dynamic_limit_ip_translation(ses->pool_address, u , &out2in_k);
				 ...
				 tcp_header_t * tcp, u32 thread_index)
{
  ...
  if (nat44_is_ses_closed (ses)
      && !(ses->flags & SNAT_SESSION_FLAG_OUTPUT_FEATURE))
    {
      ...
      nat44_delete_session_timer(sm, thread_index, ses);
#endif
...
#ifndef USER_WALK_EXPIRE_FLAG
  if (!old_state && old_state != ses->state)
      nat44_submit_or_update_session_timer(sm, thread_index, ses);
#endif
  return 0;
}
