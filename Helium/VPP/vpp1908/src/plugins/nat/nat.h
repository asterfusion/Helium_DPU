...
typedef enum
{
  NAT44_CLEANER_RESCHEDULE = 1,
} nat44_cleaner_process_event_e;
...
#define SNAT_SESSION_FLAG_NO_PAT               256
...
typedef CLIB_PACKED(struct
{
  ...
  /* addresses pool */
  snat_address_t *pool_address;
  snat_address_t *twice_nat_pool_address;
...
typedef struct
{
    u8 name[MAX_ADDRESSES_POOL_NAME_LEN];
    u32  pool_index;
    snat_address_t *addresses;
    snat_address_t *twice_nat_addresses;
} nat44_pool_addresses_t;
...
typedef struct snat_main_s
{
  ...
  clib_bihash_8_8_t dynamic_mapping_by_no_pat;
  /* dynamic no pat mapping cnt*/
  ...
  snat_address_t *addresses; //default pool
  /* Vector of twice NAT addresses for extenal hosts */
  snat_address_t *twice_nat_addresses; //defaut pool
  /* Vector of outside addresses pool*/
  nat44_pool_addresses_t *addresses_pool;
  ...
  snat_sw_if_indices_t *auto_add_sw_if_indices_twice_nat;
  ...
  u32 nat44_expire_walk_node_index;
  f64 nat44_expire_walk_interval;
...
extern vlib_node_registration_t nat44_expire_worker_walk_node;
...
format_function_t format_dynamic_mapping_by_no_pat_key;
 ...
 * @param no_pat    1 if usr no-pat
 * @param limit_ip_cnt   max limit local ip cnt 
...
/**
 * @brief used sw_if_index find mapping addresses pool index from NAT44
 *
 * @param sw_if_index      
 *
 * @return find snat_address , not find use default 
 */

u32 nat44_get_addresses_pool_index_by_sw_if_index(snat_main_t * sm,  u32 sw_if_index);
/**
 * @brief used sw_if_index find mapping addresses pool from NAT44
 *
 * @param sw_if_index      
 * @param twice        if twice NAT address
 *
 * @return find snat_address , not find use default 
 */
...
/**
 * @brief used pool name find external addresses pool from NAT44
 *
 * @param pool_name      Addresses pool name 
 *
 * @return ~0 not find, != ~0 find
 */

u32 nat44_get_addresses_pool_index_by_name(snat_main_t * sm, u8 *pool_name);

/**
 * @brief Add and Delete external addresses pool from NAT44
 *
 * @param pool_name      Addresses pool name 
...
int nat44_add_del_addresses_pool (snat_main_t * sm, u8 *pool_name, int is_add);
 ...
 * @param pool_index  ~0 = default
 *
 * @return 0 on success, non-zero value otherwise
 */
int snat_interface_add_del (u32 sw_if_index, u8 is_inside, int is_del, u32 pool_index);
 ...
 * @return 0 on success, non-zero value otherwise
 */
int snat_interface_add_del_output_feature (u32 sw_if_index, u8 is_inside, int is_del, u32 pool_index);
 ...
 * @param twice_nat   1 = no pat
 * @param limit_ip_cnt   max limit local ip cnt 
 * @param pool_index  addresses pool index
 *
 * @return 0 on success, non-zero value otherwise
 */
...
/**
 * @brief check dymamic ip limit local ip cnt 
 *
 * @param addresses         vector of outside addresses
 * @param u                 NAT user 
 * @param k                 allocated address and port pair
...
/**
 * @brief sumbit or update NAT session timer
 *
 * @param thread_index thread index
 * @param s            NAT session
 *
 */
void nat44_submit_or_update_session_timer( snat_main_t * sm, 
                   u32 thread_index, snat_session_t *s);

/**
 * @brief stop NAT session timer
 *
 * @param thread_index thread index
 * @param s            NAT session
 *
 */
void nat44_delete_session_timer( snat_main_t * sm, 
                   u32 thread_index, snat_session_t *s);
/*
