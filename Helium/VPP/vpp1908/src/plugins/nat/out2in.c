				   ...
				   u32 thread_index, f64 now)
{
  ...
  s->pool_address = nat44_find_addresses_pool_by_sw_if_index(sm, sw_if_index, 0);
			...
			u8 * p_dont_translate, void *d, void *e)
{
  ...
  if (clib_bihash_search_8_8 (&sm->per_thread_data[thread_index].out2in, &kv0,
      ...
      if (PREDICT_FALSE (identity_nat))
	{
      ...
      s0 = create_session_for_static_mapping (sm, b0, sw_if_index0, sm0, key0,
					      node, thread_index,
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (clib_bihash_search_8_8
		...
		goto trace0;
	      ...
	      /* Create session initiated by host from external network */
	      s0 = create_session_for_static_mapping (sm, b0, sw_if_index0, sm0, key0, node,
						      thread_index, now);
	      if (!s0)
		{
		  next0 = SNAT_OUT2IN_NEXT_DROP;
	...
	trace0:
	  ...
	  if (clib_bihash_search_8_8
		...
		goto trace1;
	      ...
	      /* Create session initiated by host from external network */
	      s1 = create_session_for_static_mapping (sm, b1, sw_if_index1, sm1, key1, node,
						      thread_index, now);
	      if (!s1)
		{
		  next1 = SNAT_OUT2IN_NEXT_DROP;
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (ip4_is_first_fragment (ip0)))
		  ...
		  (&per_thread_data->out2in, &kv0, &value0))
		{
		  ...
		  /* Create session initiated by host from external network */
		  s0 =
		    create_session_for_static_mapping (sm, b0, sw_if_index0, sm0, key0,
						       node, thread_index,
						       now);
		  if (!s0)
		    {
