...
nat44_i2o_is_idle_session_cb (clib_bihash_kv_8_8_t * kv, void *arg)
{
  ...
  if (ctx->now >= sess_timeout_time)
	...
	snat_free_outside_address_and_port (s->pool_address, ctx->thread_index,
					    &s->out2in);
	   ...
	   vlib_node_runtime_t * node, u32 next0, u32 thread_index, f64 now)
{
  ...
  pool_address = nat44_find_addresses_pool_by_sw_if_index(sm, sw_if_index, 0);
  ...
  if (snat_static_mapping_match
					       ...
					       sm->per_thread_data[thread_index].snat_thread_index))
	{
      ...
      /* check dynamic port change ? */
      if (key1.port == key0->port)
          is_no_pat = 1;
    }
  else
    {
  ...
  if (!u)
    {
      ...
      if (!is_sm)
          snat_free_outside_address_and_port (pool_address, thread_index, &key1);
      return SNAT_IN2OUT_NEXT_DROP;
    }

  if(!is_sm)
  {
      /* check dynamic limit ip cnt*/
      if(snat_dynamic_limit_ip_translation(pool_address, u, &key1))
      {
          nat44_delete_user_with_no_session (sm, u, thread_index);
          nat_elog_warn ("Exceeded the maximum address translation limit");
          if (!is_sm)
              snat_free_outside_address_and_port (pool_address, thread_index, &key1);
          return SNAT_IN2OUT_NEXT_DROP;
      }
  ...
  if (is_no_pat)
    s->flags |= SNAT_SESSION_FLAG_NO_PAT;
  ...
  s->fib_index = sm->outside_fib_index;
  s->worker_index = thread_index;
  *sessionp = s;
  ...
  if (is_no_pat) {
      clib_atomic_fetch_add(&sm->dynamic_no_pat_mappings_cnt, 1);
      kv0.key = s->out2in.as_u64;
      kv0.value = (u64)s;
      if (clib_bihash_add_del_8_8(&sm->dynamic_mapping_by_no_pat, &kv0, 1))
          nat_elog_notice ("dynamic no pat mapping key add failed");
  }

#ifndef USER_WALK_EXPIRE_FLAG
  /* session timer */
  nat44_submit_or_update_session_timer(sm, thread_index, s);
#endif
  ...
  /* about pool_address pool index : because pool_addresses has a same pool index */
  nat_ha_sadd (&s->in2out.addr, s->in2out.port, &s->out2in.addr,
	       s->out2in.port, &s->ext_host_addr, s->ext_host_port,
	       &s->ext_host_nat_addr, s->ext_host_nat_port,
	       s->in2out.protocol, s->in2out.fib_index, s->pool_address->pool_index, s->flags,
	       thread_index, 0);
			...
			u8 * p_dont_translate, void *d, void *e)
{
  ...
  if (clib_bihash_search_8_8 (&sm->per_thread_data[thread_index].in2out, &kv0,
      ...
      if (PREDICT_FALSE (icmp_is_error_message (icmp0)))
	{
      ...
      next0 = slow_path (sm, b0, ip0, sw_if_index0, rx_fib_index0, &key0, &s0, node, next0,
			    ...
			    int is_output_feature)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (PREDICT_FALSE
	      ...
	      if (is_slow_path)
		{
		  ...
		  next0 = slow_path (sm, b0, ip0, sw_if_index0, rx_fib_index0, &key0,
	...
	trace00:
	  ...
	  if (PREDICT_FALSE
	      (clib_bihash_search_8_8
	       (&sm->per_thread_data[thread_index].in2out, &kv1, &value1)))
	    {
	      if (is_slow_path)
		{
		  if (is_output_feature)
		  ...
		  next1 = slow_path (sm, b1, ip1, sw_if_index1, rx_fib_index1, &key1,
