...
nat_reass_ip6_t *
nat_ip6_reass_find (ip6_address_t src, ip6_address_t dst, u32 frag_id,
		    u8 proto)
{
  nat_reass_main_t *srm = &nat_reass_main;
...
nat_reass_ip6_t *
nat_ip6_reass_create (ip6_address_t src, ip6_address_t dst,
			      u32 frag_id, u8 proto)
{
  ...
  if (clib_bihash_add_del_48_8 (&srm->ip6_reass_hash, &kv, 1))
      nat_elog_warn ("ip6_reass_hash add key failed");
