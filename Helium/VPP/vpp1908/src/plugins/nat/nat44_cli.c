...
static clib_error_t *
nat_show_main_commnad_fn (vlib_main_t * vm, unformat_input_t * input,
			     vlib_cli_command_t * cmd)
{
  snat_main_t *sm = &snat_main;

  /* Thread settings */
  vlib_cli_output (vm, "num_workers: %u", sm->num_workers);
  vlib_cli_output (vm, "first_worker_index: %u", sm->first_worker_index);
  vlib_cli_output (vm, "port_per_thread: %u", sm->port_per_thread);
  vlib_cli_output (vm, "num_snat_thread: %u", sm->num_snat_thread);

  /* Worker handoff frame-queue index */
  vlib_cli_output (vm, "fq_in2out_index: %u", sm->fq_in2out_index);
  vlib_cli_output (vm, "fq_in2out_output_index: %u", sm->fq_in2out_output_index);
  vlib_cli_output (vm, "fq_out2in_index: %u", sm->fq_out2in_index);

  /* handoff fq nodes  */
  vlib_cli_output (vm, "handoff_in2out_index: %u", sm->handoff_in2out_index);
  vlib_cli_output (vm, "handoff_in2out_output_index: %u", sm->handoff_in2out_output_index);
  vlib_cli_output (vm, "handoff_out2in_index: %u", sm->handoff_out2in_index);

  /* Config parameters */
  vlib_cli_output (vm, "static_mapping_only: %u", sm->static_mapping_only);
  vlib_cli_output (vm, "static_mapping_connection_tracking: %u", sm->static_mapping_connection_tracking);
  vlib_cli_output (vm, "deterministic: %u", sm->deterministic);
  vlib_cli_output (vm, "out2in_dpo: %u", sm->out2in_dpo);
  vlib_cli_output (vm, "endpoint_dependent: %u", sm->endpoint_dependent);
  vlib_cli_output (vm, "translation_buckets: %u", sm->translation_buckets);
  vlib_cli_output (vm, "translation_memory_size: %u", sm->translation_memory_size);
  vlib_cli_output (vm, "max_translations: %u", sm->max_translations);
  vlib_cli_output (vm, "user_buckets: %u", sm->user_buckets);
  vlib_cli_output (vm, "user_memory_size: %u", sm->user_memory_size);
  vlib_cli_output (vm, "max_translations_per_user: %u", sm->max_translations_per_user);
  vlib_cli_output (vm, "outside_vrf_id: %u", sm->outside_vrf_id);
  vlib_cli_output (vm, "outside_fib_index: %u", sm->outside_fib_index);
  vlib_cli_output (vm, "inside_vrf_id: %u", sm->inside_vrf_id);
  vlib_cli_output (vm, "inside_fib_index: %u", sm->inside_fib_index);

  /* addresses pool config */
  vlib_cli_output (vm, "max addresses pool cnt: %u", MAX_ADDRESSES_POOL_CNT);

  return 0;
}

static clib_error_t *
nat_set_timeout_command_fn (vlib_main_t * vm,
			unformat_input_t * input, vlib_cli_command_t * cmd)
{
  snat_main_t *sm = &snat_main;
  unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
nat_set_alloc_addr_and_port_alg_command_fn (vlib_main_t * vm,
					      unformat_input_t * input,
					      vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
nat_show_alloc_addr_and_port_alg_command_fn (vlib_main_t * vm,
					       unformat_input_t * input,
					       vlib_cli_command_t * cmd)
{
  snat_main_t *sm = &snat_main;
...
static clib_error_t *
nat44_create_addresses_pool_command_fn (vlib_main_t * vm,
			unformat_input_t * input, vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  snat_main_t *sm = &snat_main;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
  {
    if (unformat (line_input, "name %s", &pool_name))
      ;
  ...
  if (vec_len(pool_name) > MAX_ADDRESSES_POOL_NAME_LEN - 1)
  {
    error = clib_error_return (0, 
            "NAT44 addresses pool name exceeds maximum length(%d)", MAX_ADDRESSES_POOL_NAME_LEN - 1);
    goto done;
  }

  rv = nat44_add_del_addresses_pool(sm, pool_name, is_add);

  switch (rv)
  {
  case VNET_API_ERROR_VALUE_EXIST:
    error = clib_error_return (0, "NAT44 addresses pool already exists.");
    goto done;
  case VNET_API_ERROR_NO_SUCH_ENTRY:
    error = clib_error_return (0, "NAT address not exist.");
    goto done;
  case VNET_API_ERROR_UNSPECIFIED:
    error = clib_error_return (0, "NAT44 addresses pool already in use.");
    goto done;
  case VNET_API_ERROR_INVALID_VALUE:
    error = clib_error_return (0, "NAT44 addresses pool cnt ecceeds maximum.");
    goto done;
  case VNET_API_ERROR_ADDRESS_IN_USE:
    error = clib_error_return (0, "NAT44 addresses pool %s existing address. Please delete the address first.", pool_name);
    goto done;
  default:
    break;
  }
...
static clib_error_t *
nat44_show_addresses_pool_command_fn (vlib_main_t * vm, unformat_input_t * input,
				 vlib_cli_command_t * cmd)
{
  snat_main_t *sm = &snat_main;
  ...
  vlib_cli_output (vm, "NAT44 addresses pool:");
  if (!verbose)
  {
    vlib_cli_output (vm, "  default addresses pool : has %u ip", pool_elts(sm->addresses));
    /* *INDENT-OFF* */
    pool_foreach (pool_addresses, sm->addresses_pool,
    ({
      vlib_cli_output (vm, "  %s addresses pool : has %u ip", (char *)pool_addresses->name, pool_elts(pool_addresses->addresses));
    }));
    /* *INDENT-ON* */
  }
  else
...
#undef _
  ...
  if (!verbose)
  {
    vlib_cli_output (vm, "  default addresses pool : has %u ip", pool_elts(sm->twice_nat_addresses));
    /* *INDENT-OFF* */
    pool_foreach (pool_addresses, sm->addresses_pool,
    ({
      vlib_cli_output (vm, "  %s addresses pool : has %u ip", (char *)pool_addresses->name, pool_elts(pool_addresses->twice_nat_addresses));
    }));
    /* *INDENT-ON* */
  }
  else
...
static clib_error_t *
nat44_add_address_command_fn (vlib_main_t * vm,
			unformat_input_t * input, vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  snat_main_t *sm = &snat_main;
  ...
  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	...
	;
      else if (unformat (line_input, "pool-name %s", &pool_name))
      ;
      else if (unformat (line_input, "del"))
	is_add = 0;
      else
  ...
  pool_index = nat44_get_addresses_pool_index_by_name(sm, pool_name);
  
  if(pool_name  && pool_index == ~0)
  {
      error = clib_error_return (0, "Not find pool addresses does not exist.");
	  goto done;
  }
  ...
  for (i = 0; i < count; i++)
    {
      if (is_add)
	rv = snat_add_address (sm, &this_addr, vrf_id, twice_nat, no_pat, limit_ip_cnt, pool_index);
      else
	rv = snat_del_address (sm, this_addr, 0, twice_nat, pool_index);
	...
	case VNET_API_ERROR_UNSPECIFIED:
	  ...
	  goto done;
	case VNET_API_ERROR_FEATURE_DISABLED:
	  error = clib_error_return (0, "twice NAT available only for endpoint-dependent mode.");
	  goto done;
	default:
	  break;
	}
...
{
  unformat_input_t _line_input, *line_input = &_line_input;
  u8 unformat_user_flag = 1;
  clib_error_t *error = 0;
  snat_main_t *sm = &snat_main;
  snat_address_t *addresses, *twice_nat_addresses;
  snat_address_t *ap;
  char *pool_name = NULL;
  u32 pool_index;
  nat44_pool_addresses_t *pool_addresses = NULL;
  ...
  twice_nat_addresses = sm->twice_nat_addresses;
  ...
  if (!unformat_user (input, unformat_line_input, line_input))
  {
      unformat_user_flag = 0;      
      goto ouput;
  }

  while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
      if (unformat (line_input, "pool-name %s", &pool_name))
     ;
      else
	{
  ...
  pool_index = nat44_get_addresses_pool_index_by_name(sm, (u8 *)pool_name);
  
  if(pool_name  && pool_index == ~0)
  {
      error = clib_error_return (0, "Not find %s pool addresses does not exist.", pool_name);
      goto done;
  }
  ...
  if (pool_name && pool_index != ~0)
  {
    ...
    twice_nat_addresses = pool_addresses->twice_nat_addresses;
  }

ouput:
  vlib_cli_output (vm, "NAT44 %s pool addresses:", pool_name ? pool_name : "default");
  /* *INDENT-OFF* */
  ...
  vlib_cli_output (vm, "NAT44 %s twice-nat pool addresses:", pool_name ? pool_name : "default");
  vec_foreach (ap, twice_nat_addresses)
    {
      vlib_cli_output (vm, "%U", format_ip4_address, &ap->addr);
      if (ap->no_pat)
          vlib_cli_output (vm, "  no-pat");
  ...
  if (unformat_user_flag)
      unformat_free (line_input);
  return error;
}

static clib_error_t *
nat44_feature_command_fn (vlib_main_t * vm,
			 unformat_input_t * input, vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  ...
  pool_address_index = nat44_get_addresses_pool_index_by_name(sm, (u8 *)pool_name);
  
  if(pool_name  && pool_address_index == ~0)
  {
      error = clib_error_return (0, "Not find %s pool addresses.", pool_name);
	  goto done;
  }
	  ...
	  if (is_output_feature)
	    {
		  ...
		  (sw_if_index, 1, is_del, pool_address_index))
		{
	    ...
	    {
	      if (snat_interface_add_del (sw_if_index, 1, is_del, pool_address_index))
		{
		  error = clib_error_return (0, "%s %U failed",
					     is_del ? "del" : "add",
					     format_vnet_sw_if_index_name,
  ...
  if (vec_len (outside_sw_if_indices))
      ...
      for (i = 0; i < vec_len (outside_sw_if_indices); i++)
	{
	  ...
	  if (is_output_feature)
	    {
	      if (snat_interface_add_del_output_feature
		  (sw_if_index, 0, is_del, pool_address_index))
		{
		  error = clib_error_return (0, "%s %U failed",
					     is_del ? "del" : "add",
					     format_vnet_sw_if_index_name,
	    ...
	    {
	      if (snat_interface_add_del (sw_if_index, 0, is_del, pool_address_index))
		{
		  error = clib_error_return (0, "%s %U failed",
					     is_del ? "del" : "add",
					     format_vnet_sw_if_index_name,
				  ...
				  vlib_cli_command_t * cmd)
{
  ...
  ({
    ...
    vlib_cli_output (vm, " %U %s used pool %s", format_vnet_sw_if_index_name, vnm,
                     i->sw_if_index,
                     (nat_interface_is_inside(i) &&
                      nat_interface_is_outside(i)) ? "in out" :
                     (nat_interface_is_inside(i) ? "in" : "out"),
                     pool_addresses ? (char *)pool_addresses->name : "default");
    pool_addresses = NULL;
  }));
  ...
  ({
    if (i->pool_address_index != ~0)
      pool_addresses = pool_elt_at_index(sm->addresses_pool, i->pool_address_index);
    vlib_cli_output (vm, " %U output-feature %s used pool %s",
...
static clib_error_t *
nat44_add_static_mapping_command_fn (vlib_main_t * vm,
			       unformat_input_t * input,
			       vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
nat44_add_identity_mapping_command_fn (vlib_main_t * vm,
				 unformat_input_t * input,
				 vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
nat44_add_lb_static_mapping_command_fn (vlib_main_t * vm,
				  unformat_input_t * input,
				  vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
nat44_add_lb_backend_command_fn (vlib_main_t * vm,
			   unformat_input_t * input, vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
  snat_main_t *sm = &snat_main;
...
static clib_error_t *
nat44_add_interface_address_command_fn (vlib_main_t * vm,
				       unformat_input_t * input,
				       vlib_cli_command_t * cmd)
{
  snat_main_t *sm = &snat_main;
  ...
  if(pool_name  && pool_index == ~0)
  {
      error = clib_error_return (0, "snat_add_interface_address not find %s pool addresses", pool_name);
	  goto done;
  }

  rv = snat_add_interface_address (sm, sw_if_index, is_del, twice_nat, no_pat, limit_ip_cnt, pool_index);
					 ...
					 vlib_cli_command_t * cmd)
{
  snat_main_t *sm = &snat_main;
  vnet_main_t *vnm = vnet_get_main ();
  snat_sw_if_indices_t *snat_sw_if_index;
  nat44_pool_addresses_t *pool_addresses = NULL;
  ...
  vec_foreach (snat_sw_if_index, sm->auto_add_sw_if_indices)
    {
      if ( snat_sw_if_index->pool_address_index != ~0)
      {
          pool_addresses = pool_elt_at_index(sm->addresses_pool, snat_sw_if_index->pool_address_index);
      }
      vlib_cli_output (vm, "  pool address %s : %U", 
                       pool_addresses ? (char *)pool_addresses->name : "default",
                       format_vnet_sw_if_index_name, vnm,
                       snat_sw_if_index->sw_if_index);
      pool_addresses = NULL;
    }
  vlib_cli_output (vm, "NAT44 twice-nat pool address interfaces:");
  vec_foreach (snat_sw_if_index, sm->auto_add_sw_if_indices_twice_nat)
    {
      if ( snat_sw_if_index->pool_address_index != ~0)
      {
          pool_addresses = pool_elt_at_index(sm->addresses_pool, snat_sw_if_index->pool_address_index);
...
static clib_error_t *
nat44_forwarding_set_command_fn (vlib_main_t * vm,
				unformat_input_t * input,
				vlib_cli_command_t * cmd)
{
  snat_main_t *sm = &snat_main;
...
static clib_error_t *
nat44_det_map_command_fn (vlib_main_t * vm,
			 unformat_input_t * input, vlib_cli_command_t * cmd)
{
  snat_main_t *sm = &snat_main;
  unformat_input_t _line_input, *line_input = &_line_input;
...
static clib_error_t *
nat44_det_forward_command_fn (vlib_main_t * vm,
			     unformat_input_t * input,
			     vlib_cli_command_t * cmd)
{
  snat_main_t *sm = &snat_main;
...
static clib_error_t *
nat44_det_reverse_command_fn (vlib_main_t * vm,
			     unformat_input_t * input,
			     vlib_cli_command_t * cmd)
{
  snat_main_t *sm = &snat_main;
...
static clib_error_t *
nat44_det_close_session_out_fn (vlib_main_t * vm,
			       unformat_input_t * input,
			       vlib_cli_command_t * cmd)
{
  snat_main_t *sm = &snat_main;
...
static clib_error_t *
nat44_det_close_session_in_fn (vlib_main_t * vm,
			      unformat_input_t * input,
			      vlib_cli_command_t * cmd)
{
  snat_main_t *sm = &snat_main;
...
static clib_error_t *
show_nat44_expire_walk_interval_command_fn (vlib_main_t * vm,
					unformat_input_t * input,
					vlib_cli_command_t * cmd)
{
    snat_main_t *sm = &snat_main;
    vlib_cli_output (vm, "NAT44 expire walk interval %lfs", sm->nat44_expire_walk_interval);
    return 0;
}

static clib_error_t *
nat44_set_expire_walk_interval_command_fn (vlib_main_t * vm,
					unformat_input_t * input,
					vlib_cli_command_t * cmd)
{
    snat_main_t *sm = &snat_main;
    ...
    while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
    {
        if (unformat (line_input, "%lf", &second))
            ;
        else
        {
            error = clib_error_return (0, "unknown input: '%U'",
    ...
    sm->nat44_expire_walk_interval = second;
done:
    unformat_free (line_input);
    return error;
}
...
/*?
 * luyi@asterfusion.com
 * show detail about snat_main
 *  vpp# show nat main
 *
?*/
VLIB_CLI_COMMAND (nat_show_main_command, static) = {
  .path = "show nat main",
  .short_help = "show nat main",
  .function = nat_show_main_commnad_fn,
};
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat_set_timeout_command, static) = {
  .path = "set nat timeout",
  .function = nat_set_timeout_command_fn,
  .short_help =
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat_set_alloc_addr_and_port_alg_command, static) = {
    .path = "nat addr-port-assignment-alg",
    .short_help = "nat addr-port-assignment-alg <alg-name> [<alg-params>]",
    .function = nat_set_alloc_addr_and_port_alg_command_fn,
};
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat_show_alloc_addr_and_port_alg_command, static) = {
    .path = "show nat addr-port-assignment-alg",
    .short_help = "show nat addr-port-assignment-alg",
    .function = nat_show_alloc_addr_and_port_alg_command_fn,
};
...
/*?
 * @cliexpar
 * @cliexstart{nat44 create addresses pool}
 * Create/Delete NAT44 pool addresses pool.
 * To add NAT44 pool addresses pool:
 *  vpp# nat44 create addresses pool addresses-1 
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat44_create_addresses_pool, static) = {
  .path = "nat44 create addresses pool",
  .short_help = "nat44 create addresses pool name <name> [del]",
  .function = nat44_create_addresses_pool_command_fn,
};

/*?
 * @cliexpar
 * @cliexstart{show nat44 addresses pool}
 * Show NAT44 pool addresses pool.
 * To show NAT44 pool addresses pool:
 *  vpp# nat44 show addresses pool
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat44_show_addresses_pool, static) = {
  .path = "show nat44 pool addresses",
  .short_help = "show nat44 pool addresses [detail]",
  .function = nat44_show_addresses_pool_command_fn,
};
...
VLIB_CLI_COMMAND (add_address_command, static) = {
  ...
  .short_help = "nat44 add address [pool-name <name>] <ip4-range-start> [- <ip4-range-end>] "
                "[tenant-vrf <vrf-id>] [twice-nat] [no-pat] [limit-ip-cnt <cnt>] [del]",
  .function = nat44_add_address_command_fn,
};
...
VLIB_CLI_COMMAND (nat44_show_addresses_command, static) = {
  ...
  .short_help = "show nat44 addresses [pool-name <name>]",
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (set_interface_nat44_command, static) = {
  .path = "set interface nat44",
  .function = nat44_feature_command_fn,
  .short_help = "set interface nat44 in <intfc> out <intfc> [output-feature] [pool-name <pool-name>]"
                "[del]",
};
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat44_add_static_mapping_command, static) = {
  .path = "nat44 add static mapping",
  .function = nat44_add_static_mapping_command_fn,
  .short_help =
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat44_add_identity_mapping_command, static) = {
  .path = "nat44 add identity mapping",
  .function = nat44_add_identity_mapping_command_fn,
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat44_add_lb_static_mapping_command, static) = {
  .path = "nat44 add load-balancing static mapping",
  .function = nat44_add_lb_static_mapping_command_fn,
  .short_help =
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat44_add_lb_backend_command, static) = {
  .path = "nat44 add load-balancing back-end",
  .function = nat44_add_lb_backend_command_fn,
  .short_help =
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat44_add_interface_address_command, static) = {
    .path = "nat44 add interface address",
    .short_help = "nat44 add interface address <interface> [twice-nat] [no-pat] [limit-ip-cnt <cnt>] [pool-name <name>][del]",
    .function = nat44_add_interface_address_command_fn,
};
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat44_forwarding_set_command, static) = {
  .path = "nat44 forwarding",
  .short_help = "nat44 forwarding enable|disable",
  .function = nat44_forwarding_set_command_fn,
};
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat44_det_forward_command, static) = {
    .path = "nat44 deterministic forward",
    .short_help = "nat44 deterministic forward <addr>",
    .function = nat44_det_forward_command_fn,
};
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat44_det_reverse_command, static) = {
    .path = "nat44 deterministic reverse",
    .short_help = "nat44 deterministic reverse <addr>:<port>",
    .function = nat44_det_reverse_command_fn,
};
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat44_det_close_sesion_out_command, static) = {
  ...
  .function = nat44_det_close_session_out_fn,
};
 ...
 * @cliexend
?*/
VLIB_CLI_COMMAND (nat44_det_close_session_in_command, static) = {
  ...
  .function = nat44_det_close_session_in_fn,
};

VLIB_CLI_COMMAND (nat44_add_remote_mapping_command, static) = {
    .path = "nat44 set expire walk interval",
    .short_help = "nat44 set expire walk interval [second]",
    .function = nat44_set_expire_walk_interval_command_fn,
};

VLIB_CLI_COMMAND (show_nat44_remote_mapping_command, static) = {
    .path = "show nat44 expire walk interval",
    .short_help = "show nat44 expire walk interval",
    .function = show_nat44_expire_walk_interval_command_fn,
};
/* *INDENT-ON* */
