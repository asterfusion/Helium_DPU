  ...
  if (ste)
      ...
      if (!bibe)
	{
	  ...
	  if (nat64_alloc_out_addr_and_port
	      (fib_index, ip_proto_to_snat_proto (proto), &out_addr, &out_port, 
           sport, &no_pat,
           ctx->thread_index))
	    return -1;
	...
	return -1;
      ...
      if (no_pat) 
	nat64_db_dynamic_no_pat_create(fib_index, ste, &bibe->out_addr, bibe->out_port, proto);
...
nat64_in2out_icmp_set_cb (ip6_header_t * ip6, ip4_header_t * ip4, void *arg)
{
  ...
  if (icmp->type == ICMP4_echo_request || icmp->type == ICMP4_echo_reply)
      ...
      else
	{
      ...
      if (no_pat) 
	    nat64_db_dynamic_no_pat_create(fib_index, ste, &bibe->out_addr, bibe->out_port, IP_PROTOCOL_ICMP);
...
nat64_in2out_frag_set_cb (ip6_header_t * ip6, ip4_header_t * ip4, void *arg)
{
  ...
  if (ctx->first_frag)
      ...
      else
	{
      ...
      {
          csum = ip_csum_sub_even (*checksum, bibe->in_addr.as_u64[0]);
          csum = ip_csum_sub_even (csum, bibe->in_addr.as_u64[1]);
          csum = ip_csum_sub_even (csum, ste->in_r_addr.as_u64[0]);
          csum = ip_csum_sub_even (csum, ste->in_r_addr.as_u64[1]);
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_TRUE (ip6_frag_hdr_offset (frag0)))
	      ...
	      if (!ste0)
		{
          ...
          if (no_pat) 
              nat64_db_dynamic_no_pat_create(fib_index0, ste0, &bibe0->out_addr, bibe0->out_port, proto0);
