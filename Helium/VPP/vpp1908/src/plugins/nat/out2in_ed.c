				      ...
				      lb_nat_type_t lb_nat, f64 now)
{
  ...
  pool_index = nat44_get_addresses_pool_index_by_sw_if_index(sm, sw_if_index);
  ...
  if (twice_nat == TWICE_NAT || (twice_nat == TWICE_NAT_SELF &&
				 ip->src_address.as_u32 == l_key.addr.as_u32))
    {
      ...
      eh_key.port = e_key.protocol == SNAT_PROTOCOL_ICMP ? e_key.port : udp->src_port;
      pool_address =  nat44_find_addresses_pool_by_sw_if_index(sm, sw_if_index, 1);
					       ...
					       tsm->snat_thread_index))
	{
      ...
      s->twice_nat_pool_address = pool_address;
  ...
  nat_ha_sadd (&s->in2out.addr, s->in2out.port, &s->out2in.addr,
	       ...
	       &s->ext_host_nat_addr, s->ext_host_nat_port,
	       s->in2out.protocol, s->in2out.fib_index, pool_index, s->flags,
	       thread_index, 0);
		      ...
		      u8 * p_dont_translate, void *d, void *e)
{
  ...
  if (clib_bihash_search_16_8 (&tsm->out2in_ed, &kv, &value))
      ...
      if (PREDICT_FALSE (identity_nat))
	{
      ...
      s = create_session_for_static_mapping_ed (sm, b, sw_if_index, l_key, e_key, node,
						thread_index, 0, 0,
						vlib_time_now
						(sm->vlib_main));
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  if (clib_bihash_search_16_8 (&tsm->out2in_ed, &kv0, &value0))
	      ...
	      if (is_slow_path)
		{
		  ...
		  /* Create session initiated by host from external network */
		  s0 = create_session_for_static_mapping_ed (sm, b0, sw_if_index0, l_key0,
							     e_key0, node,
							     thread_index,
							     twice_nat0,
							     lb_nat0, now);
	...
	trace00:
	  ...
	  if (clib_bihash_search_16_8 (&tsm->out2in_ed, &kv1, &value1))
	      ...
	      if (is_slow_path)
		{
		  ...
		  /* Create session initiated by host from external network */
		  s1 = create_session_for_static_mapping_ed (sm, b1, sw_if_index1, l_key1,
							     e_key1, node,
							     thread_index,
							     twice_nat1,
							     lb_nat1, now);
