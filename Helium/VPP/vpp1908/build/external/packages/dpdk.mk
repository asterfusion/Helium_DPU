...
DPDK_VERSION                 ?= marvell-v10
DPDK_BASE_URL                ?= http://fast.dpdk.org/rel
DPDK_TARBALL                 := dpdk-$(DPDK_VERSION).tar.bz2
...
ifneq (,$(findstring $(MIDR_PARTNUM),$(CPU_PART_CAVIUM_OCTEONTX2_96XX)))
...
$(B)/custom-config: $(B)/.dpdk-patch.ok Makefile
	...
	@# modify options
	$(call set,RTE_MAX_LCORE,$(DPDK_MAX_CORES))
	$(call set,RTE_LIBRTE_OCTEONTX_PMD,$(DPDK_OCTEONTX_PMD))
	$(call set,RTE_LIBRTE_PMD_OCTEONTX_SSOVF,$(DPDK_OCTEONTX_PMD))
	$(call set,RTE_LIBRTE_PMD_OCTEONTX_EVENTDEV,$(DPDK_OCTEONTX_PMD))
	$(call set,RTE_LIBRTE_OCTEONTX_MEMPOOL,$(DPDK_OCTEONTX_PMD))
	$(call set,RTE_LIBRTE_OCTEONTX2_PMD,$(DPDK_OCTEONTX2_PMD))
	$(call set,RTE_LIBRTE_PMD_OCTEONTX2_EVENTDEV,$(DPDK_OCTEONTX2_PMD))
	$(call set,RTE_LIBRTE_PMD_OCTEONTX2_DMA_RAWDEV,$(DPDK_OCTEONTX2_PMD))
	$(call set,RTE_LIBRTE_OCTEONTX2_MEMPOOL,$(DPDK_OCTEONTX2_PMD))
	$(call set,RTE_LIBRTE_OCTEONTX2_BAREMETAL_AF_IF_CGX,$(DPDK_OCTEONTX2_PMD))
...
$(B)/.dpdk-copy.ok:
	@mkdir -p $(B)
	@cp -r $(CURDIR)/downloads/dpdk-$(DPDK_VERSION) $(B)/
	@touch $@

$(B)/.dpdk-patch.ok: $(B)/.dpdk-copy.ok
