...
ifeq ($$(call $1_update_cmds),)
define $1_update_cmds
	@echo "followup download empty"
endef
endif

.PHONY: downloads/$($1_tarball)
downloads/$($1_tarball):
	mkdir -p downloads ;
	$$(call h1,"downloading $1 $($1_version) at $$@") ;
	@if [ ! -e $$@ ]; then \
...
$(B)/.$1.download.ok: downloads/$($1_tarball)
	@mkdir -p $(B)
	@echo "$1_skip_tarball_checks: $($1_skip_tarball_checks)" ;
	@if [ -z $($1_skip_tarball_checks) ]; then \
	$$(call h2,"validating $1 $($1_version) checksum") ; \
...
$(B)/.$1.extract.ok: $(B)/.$1.download.ok
	@if [ -z $($1_skip_tarball_extract) ] ; \
		then \
		$$(call h2,"extracting $1 $($1_version)") ;\
		mkdir -p $$($1_src_dir) ; \
		tar \
	             --directory $$($1_src_dir) \
	             --extract \
