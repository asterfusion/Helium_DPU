...
#dpdk_tarball_md5sum_21.11    := 3dc2ad90093b7dbbf3da57d684c8e6b6
dpdk_tarball_md5sum_21.11    := 7851e804f2c019d92ec3941398dd964b
...
dpdk_depends		     := rdma-core armv8-crypto $(if $(ARCH_X86_64), ipsec-mb)
# Debug or release
...
CPU_PART_OCTEONTX2_96XX  = 0x0b2
CPU_PART_CN106XX        = 0xd49

MIDR_PARTNUM=$(shell awk '/part/ {print $$4;exit}' /proc/cpuinfo)

ifneq (,$(findstring $(MIDR_PARTNUM),$(CPU_PART_OCTEONTX2_96XX)))
DPDK_PLATFORM := octeontx2
else ifneq (,$(findstring $(MIDR_PARTNUM),$(CPU_PART_CN106XX)))
DPDK_PLATFORM := cn10k
endif
...
DPDK_MESON_ARGS += --cross-file=$(dpdk_src_dir)/config/arm/arm64_cn10k_linux_gcc
else ifeq ($(OCTEON_VERSION), cn9k)
DPDK_MESON_ARGS += --cross-file=$(dpdk_src_dir)/config/arm/arm64_octeontx2_linux_gcc
endif
...
define dpdk_install_cmds
  ...
  if [ ! -z $(CROSS_TARGET) ];then cp -r $(dpdk_install_dir)/include/* $(CNXK_SDK_SYSROOT)/usr/include/; fi && \
	deactivate
endef
