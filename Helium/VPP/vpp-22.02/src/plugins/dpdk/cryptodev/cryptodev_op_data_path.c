				     ...
				     cryptodev_op_type_t op_type)
{
  ...
  while (n_elts)
      ...
      if (last_key_index != fe->key_index)
	{
       ...
       * rewritten by tx. */
      if (PREDICT_TRUE (fe->integ_start_offset < 0 || b->current_data < 0))
	{
	  sop->m_src->data_off += b->current_data;
	  sop->m_src->pkt_len -= b->current_data;
	  sop->m_src->data_len -= b->current_data;
	  integ_offset = fe->integ_start_offset - b->current_data;
	  crypto_offset = crypto_offset - b->current_data;
	}
      sop->session = sess;
			      ...
			      cryptodev_op_type_t op_type, u8 aad_len)
{
  ...
  while (n_elts)
      ...
      if (last_key_index != fe->key_index)
	{
       ...
       * correctly compute the data pointer. The prepend here will be later
       * rewritten by tx. */
      if (PREDICT_FALSE (b->current_data < 0))
	{
	  sop->m_src->data_off += b->current_data;
	  sop->m_src->data_len = b->current_length;
	  sop->m_src->pkt_len = b->current_length;
	  crypto_offset -= b->current_data;
	}
