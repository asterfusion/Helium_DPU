...
dpdk_buffer_pool_init (vlib_main_t *vm, vlib_buffer_pool_t *bp,
		       ...
		       const char *non_cache_ops_name, int use_dpdk_ops,
		       u32 nmbufs)
{
  ...
  if (use_dpdk_ops)
      ...
      if (!nmbufs)
	{
	  ...
	  return clib_error_return (
	    0, "Nmbufs passed as 0 for DPDK mempool configuraton");
	}
    }
  else
    {
      ...
      nmp = rte_mempool_create_empty ((char *) name, bp->n_buffers, elt_size,
				      0, sizeof (priv), bp->numa_node, 0);
      if (!nmp)
	{
	  rte_mempool_free (mp);
	  vec_free (name);
	  return clib_error_return (
	    0, "failed to create %s mempool for numa nude %u",
  ...
  if (use_dpdk_ops)
      ...
      if (rte_eth_dev_count_avail ())
	{
	  ...
	  if (bp->n_buffers != bp->n_avail)
	    {
	      ...
	      rte_mempool_free (mp);
	      clib_error_return (
		0, "Buffers of pool: %d are not intact(total: %lu, free: %lu)",
	  ...
	  for (i = 0; i < pm->n_pages; i++)
	    {
	      ...
	      pa = (iova_mode == RTE_IOVA_VA) ? pointer_to_uword (va) :
						pm->page_table[i];
...
#else
	      if (do_vfio_map && rte_vfio_container_dma_map (
	  ...
	  if (mp->populated_size > bp->n_buffers)
	    {
	      clib_warning ("Buffers populated by DPDK: %u > "
			    "vib_buffer_pool->n_buffers %u",
  ...
  if (!use_dpdk_ops)
    {
      clib_memcpy (
	vec_elt_at_index (dpdk_mbuf_template_by_pool_index, bp->index),
	rte_mbuf_from_vlib_buffer (
	  vlib_buffer_ptr_from_index (buffer_mem_start, *bp->buffers, 0)),
  ...
  else
    {
      ...
      /*Now we have populated mempools, Drain mempool to fill up VLIB buffer
       * again
       */
      for (i = 0; i < mp->populated_size; i++)
	{
	  ...
	  vlib_buffer_copy_template (b, &bp->buffer_template);
	  vlib_get_buffer_indices_with_offset (vm, (void **) &b, &buffer_index,
					       1, 0);
	  if (CLIB_DEBUG > 0)
	    {
      ...
      for (i = 0; i < nmbufs; i++)
	{
	  ...
	  if (n)
	    {
	      b =
		vlib_buffer_ptr_from_index (buffer_mem_start, buffer_index, 0);
	      mb = rte_mbuf_from_vlib_buffer (b);
...
(struct rte_mempool *mp, void *const *obj_table, unsigned int n)
{
...
(struct rte_mempool *mp, void **obj_table, unsigned int n)
{
  const int batch_size = 32;
  vlib_main_t *vm = vlib_get_main ();
...
clib_error_t *CLIB_MULTIARCH_FN (_dpdk_buffer_pools_create)
(vlib_main_t *vm, char *ops_name, u32 nmbufs)
{
#define DEFAULT_CACHE_MEMPOOL_OPS_NAME "vpp"
  ...
  ops.enqueue = (rte_mempool_enqueue_t) CLIB_MARCH_FN_POINTER (
   ...
   * mempool. If not, cache_ops_name is returned with preferred mempool_ops
   * name
   */
  if (dpdk_check_mempool_ops_support (DEFAULT_CACHE_MEMPOOL_OPS_NAME,
				      cache_ops_name) < 0)
    {
  ...
  vec_foreach (bp, vm->buffer_main->buffer_pools)
    if (bp->start && (err = dpdk_buffer_pool_init (
			vm, bp, (const char *) cache_ops_name,
			no_cache_ops_name, dpdk_is_mempool_ops_used, nmbufs)))
      return err;
  /* *INDENT-ON* */
