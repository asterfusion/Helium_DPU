...
static_always_inline void
dpdk_mempool_update_current_refill_count (
  vlib_main_t *vm, vlib_node_runtime_t *node, int thread_index,
  dpdk_per_thread_data_t *ptd, uword n_rx_packets, u8 buffer_pool_index)
{
		   ...
		   int mempool_refill_enable)
{
  ...
  if (mempool_refill_enable)
    {
      dpdk_mempool_update_current_refill_count (
	vm, node, thread_index, ptd, n_rx_packets, rxq->buffer_pool_index);
    }
 ...
 */
static_always_inline u32
dpdk_mempool_refill (vlib_main_t *vm, u32 buffer_pool_index,
		     i16 n_buffers_to_refill, u32 *bi, void **buffers)
{
  ...
  n_buffers_to_refill = vlib_buffer_alloc_from_pool (
    vm, bi, n_buffers_to_refill, buffer_pool_index);

  if (PREDICT_TRUE (n_buffers_to_refill > 0))
    {
      vlib_get_buffers_with_offset (vm, bi, buffers, n_buffers_to_refill,
...
/** \brief Scan per thread counter `refill_deplete_per_count` per VLIB buffer
     pool for any refill of buffers required from VLIB buffer pool to
   rte_mempool
   ...
   to a VLIB buffer pool is greater than configured number of `default refill
   value`, dm->per_thread_data->default_refill_count_per_pool, then dpdk
   mempool corresponding to VLIB pool is refilled by buffers equivalent
   to`default refill value`. `current counter` is also decrement by same
   default refill value
*/
static_always_inline void
  ...
  if (ptd->fp_refill_deplete_enable)
      ...
      vec_foreach (bp, vm->buffer_main->buffer_pools)
	{
	  ...
	  if (ptd->refill_deplete_count_per_pool[bp->index] >=
	    ...
	    {
	      ASSERT (bp->index <
		      vec_len (ptd->default_refill_count_per_pool));
	      ASSERT (bp->index <
		      vec_len (ptd->refill_deplete_count_per_pool));
...
#ifdef DPDK_USE_PMD_COMPAT_POOL_OPS
      n_rx_packets +=
	dpdk_device_input (vm, dm, xd, node, thread_index, pv[i].queue_id, 1);
#else
      n_rx_packets +=
	dpdk_device_input (vm, dm, xd, node, thread_index, pv[i].queue_id, 0);
#endif
    }
