...
dpdk_validate_rte_mbuf (vlib_main_t *vm, vlib_buffer_t *b, int maybe_multiseg,
			int mempool_deplete_enable,
			dpdk_per_thread_data_t *ptd)
{
   ...
   *   may fail to be transmitted via rte_eth_tx_burst. As packets which are
   * not transmitted are freed to rte_mempool.
   */
		      ...
		      i16 n_buffers_to_deplete, u32 *bi, void **buffers)
{
  ...
  if (PREDICT_TRUE (mp && n_buffers_to_deplete))
	...
	vm, buffers, bi, n_buffers_to_deplete, sizeof (struct rte_mbuf));
...
/** \brief Scan per thread counter `refill_deplete_per_count` per VLIB buffer
     pool for any deplete of buffers required from rte_mempool to VLIB buffer
   pool
    ...
    counter`,dm->per_thread_data->refill_deplete_count_per_pool, corresponding
   to a VLIB buffer pool is lesser than configured number of `default deplete
  ...
  vec_foreach (bp, vm->buffer_main->buffer_pools)
	  ...
	  ptd->default_deplete_count_per_pool[bp->index])
	{
	  ...
	  ptd->refill_deplete_count_per_pool[bp->index] +=
	    dpdk_mempool_deplete (
	      vm, bp->index, ptd->default_refill_count_per_pool[bp->index],
 ...
 */
static_always_inline u32
dpdk_device_output (vlib_main_t *vm, vlib_node_runtime_t *node,
		    vlib_frame_t *f, dpdk_per_thread_data_t *ptd,
		    const int mempool_deplete_enable)
{
  dpdk_main_t *dm = &dpdk_main;
  ...
  while (n_left >= 8)
	...
	if (mb[0]->vlan_tci <= 12 && 0 != intf_vlan[mb[0]->vlan_tci - 1])
        {
                vlib_buffer_advance(b[0], -4);
                pkt_start = vlib_buffer_get_current (b[0]);
                memcpy(pkt_start, pkt_start + 4, 12);
                *(pkt_start + 12) = 0x81;
                *(pkt_start + 13) = 0x00;
                pkt_start_tmp = (u16 *)(pkt_start + 14);
                *pkt_start_tmp = htons(intf_vlan[mb[0]->vlan_tci - 1]);

        }

	if (mb[1]->vlan_tci <= 12 && 0 != intf_vlan[mb[1]->vlan_tci - 1])
        {
                vlib_buffer_advance(b[1], -4);
                pkt_start = vlib_buffer_get_current (b[1]);
                memcpy(pkt_start, pkt_start + 4, 12);
                *(pkt_start + 12) = 0x81;
                *(pkt_start + 13) = 0x00;
                pkt_start_tmp = (u16 *)(pkt_start + 14);
                *pkt_start_tmp = htons(intf_vlan[mb[1]->vlan_tci - 1]);

        }

	if (mb[2]->vlan_tci <= 12 && 0 != intf_vlan[mb[2]->vlan_tci - 1])
        {
                vlib_buffer_advance(b[2], -4);
                pkt_start = vlib_buffer_get_current (b[2]);
                memcpy(pkt_start, pkt_start + 4, 12);
                *(pkt_start + 12) = 0x81;
                *(pkt_start + 13) = 0x00;
                pkt_start_tmp = (u16 *)(pkt_start + 14);
                *pkt_start_tmp = htons(intf_vlan[mb[2]->vlan_tci - 1]);

        }

        if (mb[3]->vlan_tci <= 12 && 0 != intf_vlan[mb[3]->vlan_tci - 1])
        {
                vlib_buffer_advance(b[3], -4);
                pkt_start = vlib_buffer_get_current (b[3]);
                memcpy(pkt_start, pkt_start + 4, 12);
                *(pkt_start + 12) = 0x81;
                *(pkt_start + 13) = 0x00;
                pkt_start_tmp = (u16 *)(pkt_start + 14);
                *pkt_start_tmp = htons(intf_vlan[mb[3]->vlan_tci - 1]);
...
{
  dpdk_per_thread_data_t *ptd =
    vec_elt_at_index (dpdk_main.per_thread_data, vm->thread_index);
