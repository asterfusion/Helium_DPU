...
{
  /*used in both tunnel and policy mode for SA expiry*/
  u64 soft_byte_limit;
  u64 hard_byte_limit;
...
find_tunnel_db (ip_address_t *saddr, ip_address_t *daddr, u8 dir, u8 is_ipv6)
{
  ...
  NL_XFRM_DBG ("Tunnel iface found during SA deletion index: %x",
	       tun->sw_if_index);
  ...
  pool_foreach_index (sai, ipsec_sa_pool)
      ...
      if (found)
	{
	  if (dir)
	    ...
	    life->sa_in_tunnel = 1; // its enabled only for inb sa
	  return sa;
	}
    }
  return NULL;
			...
			u32 phy_sw_if_index)
{
  ...
  rv = vnet_sw_interface_update_unnumbered (*sw_if_index, phy_sw_if_index, 1);
  if (rv)
    {
      NL_XFRM_ERR ("Error setting ip address on tunnel");
      return -1;
    }
  return 0;
}
  ...
  if (dir == 0)
    {
      ...
      if (!sa) // outb sa is not yet configured
	{
	  return;
	}
      sa_out = sa->id;
  ...
  else
    {
      sa_out = sa_id;
      sa = get_reverse_sa_by_tun_ip (saddr, daddr, is_ipv6, dir);
      if (!sa) // inb sa is not yet configured
	{
	  return;
	}
      sa_in = sa->id;
